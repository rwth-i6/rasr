...
private:
    ...
    static Base* creator0_() {
        return new Derived();
    }
    template<class Derived, class T>
    static Base* creator1_() {
        return new Derived(arg1_<Derived, T>());
    }
    template<class Derived, class T1, class T2>
    static Base* creator2_() {
        return new Derived(arg1_<Derived, T1>(), arg2_<Derived, T2>());
    }
...
public:
    IoRef()
            : Precursor() {}
    IoRef(const Ref<Base>& r)
            : Precursor(r) {}
    template<class S>
    IoRef(const Ref<S>& r)
            : Precursor(r) {}
    explicit IoRef(Base* p)
            : Precursor(p) {}

    IoRef& operator=(const Ref<Base>& r) {
        Precursor::operator=(r);
        return *this;
    }
    ...
    template<class Derived, class T>
    static void registerClass(const T& arg) {
        arg1_<Derived, T>()                             = arg;
        creators_()[((Derived*)0)->Derived::typeName()] = creator1_<Derived, T>;
    }
    template<class Derived, class T1, class T2>
    static void registerClass(const T1& arg1, const T2& arg2) {
