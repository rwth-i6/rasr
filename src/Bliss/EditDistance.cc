...
void EditDistance::Alignment::write(Core::XmlWriter& xml) const {
    switch (format_) {
        case FormatBliss:
            xml << Core::XmlOpen("alignment") + Core::XmlAttribute("type", "edit-distance") + Core::XmlAttribute("format", "bliss");
        ...
        case FormatNist:
            ...
            xml << Core::XmlOpen("alignment") + Core::XmlAttribute("type", "edit-distance") + Core::XmlAttribute("format", "nist/sclite");
...
// ===========================================================================
struct EditDistance::Trace : public Core::ReferenceCounted,
...
struct EditDistance::Hyp {
    ...
    struct KeyFunction {
        const State& operator()(const Hyp& h) const {
            return h.pos;
        }
    };
    ...
    struct PriorityFunction {
        bool operator()(const Hyp& lhs, const Hyp& rhs) const {
            ...
            */
            return (lhs.cost == rhs.cost) ? (lhs.nRef > rhs.nRef) : (lhs.cost < rhs.cost);
        }
    };
};
...
bool match(const char* ca, const char* cb) {
    if (*ca == '-') {
        ...
        for (; (*ca == *cb) && (*ca != '\0') && (*cb != '\0'); ++ca, ++cb)
            ;
        return ((*ca == *cb) || ((*ca == '-') && (*(++ca) == '\0'))) ? true : false;
    }
}
...
EditDistance::EditCost EditDistance::cost(const Token* a, const Token* b) const {
    if (a) {
        if (b)
            return (a == b) ? std::make_pair(Correct, Cost(0)) : subCost(a, b);
        else
            return delCost(a);
    }
    else if (b) {
...
void EditDistance::align(Fsa::ConstAutomatonRef a, Fsa::ConstAutomatonRef b, Alignment& out) const {
    ...
    std::unordered_map<State, Cost, State::Hash>                                                closed;
    Hyp                                                                                         nh;
...
void ErrorStatistic::write(Core::XmlWriter& os) const {
    os << Core::XmlOpen("statistic") + Core::XmlAttribute("type", "edit-distance");
