#ifndef _LM_SIMPLE_TF_NEURAL_LM_HH
#define _LM_SIMPLE_TF_NEURAL_LM_HH
...
// generic simple TF-NLM: mainly for modern systems with reasonable search space size
// - trade speed for simplicity: always feed-in full sequence and get last output scores
//   although this is usually fast enough for GPU decoding with parallel computation
//   Note: slice last position should be done in model graph based on seq_len
// - low memory consumption as not storing any hidden states, also no need of feed/fetch
class SimpleTFNeuralLm: public AbstractNNLanguageModel
{
  ...
  public:
    SimpleTFNeuralLm(const Core::Configuration& c, Bliss::LexiconRef l);
    ~SimpleTFNeuralLm();

    // history: no reduction to avoid cache update, but if needed e.g. FFNNLM
    // - scoring: truncate input seq in the graph
    // - recombination: separate SimpleHistoryLm
...
#endif // _LM_SIMPLE_TF_NEURAL_LM_HH
