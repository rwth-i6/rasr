...
Lattice::ConstWordLatticeRef LatticeExtractor::extract(Lattice::ConstWordLatticeRef lattice, Bliss::SpeechSegment*) {
 ...
 */
Lattice::ConstWordLatticeRef LatticeRescorer::extract(Lattice::ConstWordLatticeRef lattice, Bliss::SpeechSegment* segment) {
...
Lattice::ConstWordLatticeRef AcousticLatticeRescorer::work(Lattice::ConstWordLatticeRef lattice,
...
Lattice::ConstWordLatticeRef AlignmentLatticeRescorer::work(Lattice::ConstWordLatticeRef lattice, Bliss::SpeechSegment* segment) {
    alignmentGenerator_->setSpeechSegment(segment);
    AlignmentLatticeRescorerAutomaton* f      = new AlignmentLatticeRescorerAutomaton(lattice, alignmentGenerator_);
    Lattice::WordLattice*              result = new Lattice::WordLattice;
    result->setWordBoundaries(lattice->wordBoundaries());
...
private:
    ...
    const f32                                pronunciationScale_;
    const f32                                fallBackValue_;
...
public:
    LmLatticeRescorerAutomaton(Lattice::ConstWordLatticeRef,
                               ...
                               f32 pronunciationScale = 0, f32 fallBackValue = Core::Type<f32>::max);
...
Fsa::Weight LmLatticeRescorerAutomaton::score(Fsa::StateId s, const Fsa::Arc& a) const {
    ...
    if (!(hist == histories_[a.target()])) {
        languageModel_->error() << "Mismatch between lattice and language model: "
                                ...
                                << a.target() << "' ('" << languageModel_->formatHistory(hist) << "' vs. '" << languageModel_->formatHistory(histories_[a.target()]) << "').\n"
                                                                                                                                                                        "Possible causes: 1) lattice is time-conditioned,\n"
                                                                                                                                                                        "2) lattice has been generated by using another language model.";
    }
    bool notFinite = Math::isnan(score) || score > Core::Type<f32>::max || score < Core::Type<f32>::min;
    if (notFinite)
        Core::Application::us()->log("inf lm score replaced by fall back score ") << score << ", " << fallBackValue_;
    return Fsa::Weight(notFinite ? fallBackValue_ : score);
}

const Core::ParameterFloat LmLatticeRescorer::paramFallBackValue(
        "fall-back-value",
        "fall back value if lm score is inf",
        Core::Type<f32>::max);

/*
 * LatticeRescorer: lm
 */
LmLatticeRescorer::LmLatticeRescorer(const Core::Configuration& c, bool initialize)
        : Precursor(c),
          fallBackValue_(paramFallBackValue(c)) {
    if (initialize) {
        ModelCombination modelCombination(select("model-combination"), ModelCombination::useLanguageModel);
        modelCombination.load();
        languageModel_ = Core::Ref<const Lm::ScaledLanguageModel>(modelCombination.languageModel().get());
    }
    log("using language model fall back value: ") << fallBackValue_;
}

Lattice::ConstWordLatticeRef LmLatticeRescorer::work(Lattice::ConstWordLatticeRef lattice, Bliss::SpeechSegment*) {
    LmLatticeRescorerAutomaton* f      = new LmLatticeRescorerAutomaton(lattice, languageModel_, 0, fallBackValue_);
    Lattice::WordLattice*       result = new Lattice::WordLattice;
 ...
 */
CombinedLmLatticeRescorer::CombinedLmLatticeRescorer(const Core::Configuration& c)
        : Precursor(c, false) {
    ModelCombination modelCombination(select("model-combination"), ModelCombination::useLanguageModel);
    modelCombination.load();
    languageModel_      = Core::Ref<const Lm::ScaledLanguageModel>(modelCombination.languageModel().get());
...
Lattice::ConstWordLatticeRef CombinedLmLatticeRescorer::work(Lattice::ConstWordLatticeRef lattice, Bliss::SpeechSegment*) {
    Fsa::ConstAutomatonRef lm     = Fsa::ConstAutomatonRef(new LmLatticeRescorerAutomaton(
            lattice, languageModel_, pronunciationScale_, fallBackValue_));
    Lattice::WordLattice*  result = new Lattice::WordLattice;
    result->setWordBoundaries(lattice->wordBoundaries());
...
LatticeReader::LatticeReader(const Core::Configuration& c, Bliss::LexiconRef lexicon)
        : Precursor(c),
          archiveReader_(0) {
...
Lattice::ConstWordLatticeRef LatticeReader::extract(Lattice::ConstWordLatticeRef lattice, Bliss::SpeechSegment* segment) {
    verify(archiveReader_);
    Lattice::ConstWordLatticeRef result = archiveReader_->get(segment->fullName(), fsaPrefix_);
...
Lattice::ConstWordLatticeRef ApproximateDistanceLatticeRescorer::work(Lattice::ConstWordLatticeRef lattice, Bliss::SpeechSegment* segment) {
 ...
 */
ArchiveApproximateWordAccuracyLatticeRescorer::ArchiveApproximateWordAccuracyLatticeRescorer(const Core::Configuration& c, Bliss::LexiconRef lexicon)
        : Precursor(c, lexicon),
          builder_(0) {
    builder_ = new ArchiveApproximateWordAccuracyLatticeBuilder(select("approximate-word-accuracy-lattice-builder"), lexicon);
}
...
Fsa::ConstAutomatonRef ArchiveApproximateWordAccuracyLatticeRescorer::getDistanceFsa(Lattice::ConstWordLatticeRef lattice, Bliss::SpeechSegment* segment) {
    verify(builder_);
...
OrthographyApproximateWordAccuracyLatticeRescorer::OrthographyApproximateWordAccuracyLatticeRescorer(const Core::Configuration& c, Bliss::LexiconRef lexicon)
        : Precursor(c, lexicon),
          builder_(0) {
    builder_ = new OrthographyApproximateWordAccuracyLatticeBuilder(select("approximate-word-accuracy-lattice-builder"), lexicon);
}
...
Fsa::ConstAutomatonRef OrthographyApproximateWordAccuracyLatticeRescorer::getDistanceFsa(Lattice::ConstWordLatticeRef lattice, Bliss::SpeechSegment* segment) {
    verify(builder_);
    return builder_->createFunctor(segment->fullName(),
                                   segment->orth(),
                                   lattice)
 ...
 */
ApproximatePhoneAccuracyLatticeRescorer::ApproximatePhoneAccuracyLatticeRescorer(const Core::Configuration& c, Bliss::LexiconRef lexicon)
        : Precursor(c, lexicon) {}

ApproximatePhoneAccuracyLatticeRescorer::~ApproximatePhoneAccuracyLatticeRescorer() {}

ArchiveApproximatePhoneAccuracyLatticeRescorer::ArchiveApproximatePhoneAccuracyLatticeRescorer(const Core::Configuration& c, Bliss::LexiconRef lexicon)
        : Precursor(c, lexicon),
          builder_(0) {
    builder_ = new ArchiveApproximatePhoneAccuracyLatticeBuilder(select("approximate-phone-accuracy-lattice-builder"), lexicon);
}
...
Fsa::ConstAutomatonRef ArchiveApproximatePhoneAccuracyLatticeRescorer::getDistanceFsa(Lattice::ConstWordLatticeRef lattice, Bliss::SpeechSegment* segment) {
    verify(builder_);
    alignmentGenerator_->setSpeechSegment(segment);
    return builder_->createFunctor(segment->fullName(),
                                   segment->fullName(),
...
OrthographyApproximatePhoneAccuracyLatticeRescorer::OrthographyApproximatePhoneAccuracyLatticeRescorer(const Core::Configuration& c, Bliss::LexiconRef lexicon)
        : Precursor(c, lexicon),
          builder_(0) {
    builder_ = new OrthographyApproximatePhoneAccuracyLatticeBuilder(select("approximate-phone-accuracy-lattice-builder"), lexicon);
}
...
Fsa::ConstAutomatonRef OrthographyApproximatePhoneAccuracyLatticeRescorer::getDistanceFsa(Lattice::ConstWordLatticeRef lattice, Bliss::SpeechSegment* segment) {
    verify(builder_);
    alignmentGenerator_->setSpeechSegment(segment);
    return builder_->createFunctor(segment->fullName(),
                                   segment->orth(),
