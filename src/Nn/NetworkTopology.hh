...
public:
    ...
    s32 topologicalId() const {
        return topologicalId_;
    }
    void setTopologicalId(u32 value) {
        topologicalId_ = value;
    }

    const NeuralNetworkLayer<T>* layer() const {
        return layer_;
    }
    NeuralNetworkLayer<T>* layer() {
        return layer_;
    }

    u32 nConnections() const {
        return connections_.size();
    }
    u32 connection(u32 i) const {
        require_lt(i, connections_.size());
        return connections_.at(i);
    }
    void addConnection(u32 elem) {
        connections_.push_back(elem);
    }

    u32 nPredecessors() const {
        return predecessors_.size();
    }
    u32 predecessor(u32 i) const {
        require_lt(i, predecessors_.size());
        return predecessors_.at(i);
    }
...
public:
    ...
    bool hasLayer(const std::string& layerName) const {
        return layerMapping_.find(layerName) != layerMapping_.end();
    }
    ...
    u32 nFeatureStreams() const {
        return featureStreamConnections_.size();
    }
    ...
    const NetworkTopologyElement<T>* layerElement(u32 i) const {
        require_lt(i, layers_.size());
        return &(layers_.at(i));
    }
    NetworkTopologyElement<T>* layerElement(u32 i) {
        require_lt(i, layers_.size());
        return &(layers_.at(i));
    }
