...
bool readAtt(const Resources<_Automaton>& resources, StorageAutomaton<_Automaton>* f, std::istream& i) {
    ...
    while (i) {
        ...
        switch (n) {
            ...
            case 3:  // acceptor arc
                ...
                a->setOutput(a->input());
                if (*error != '\0') {
                    if (!hasAlphabets && !sinput)
                        sinput = new Fsa::StaticAlphabet();
                    if (strcmp(tmp3, "eps") == 0)
                        a->setInput(Fsa::Epsilon);
                    else
                        a->setInput(hasAlphabets ? input->index(tmp3) : sinput->addSymbol(tmp3));
                    if (!hasAlphabets && !soutput)
                        soutput = new Fsa::StaticAlphabet();
                    if (strcmp(tmp4, "eps") == 0)
                        a->setOutput(Fsa::Epsilon);
                    else
                        a->setOutput(hasAlphabets ? output->index(tmp3) : soutput->addSymbol(tmp3));
                }
                break;
                ...
                if (*error != '\0') {
                    ...
                    if (*error != '\0') {
                        ...
                        if (strcmp(tmp4, "eps") == 0)
                            ...
                            a->setOutput(hasAlphabets ? output->index(tmp4) : soutput->addSymbol(tmp4));
                    }
                }
                else
                    f->setType(Fsa::TypeTransducer);
    ...
    if (soutput)
        f->setOutputAlphabet(Fsa::ConstAlphabetRef(soutput));
    ...
    if (what & Fsa::storeStates) {
        ...
        while (bi) {
            ...
            if (idAndTags & Fsa::StateTagFinal)
                if (!f->semiring()->read(sp->weight_, bi))
                    return false;
            u32 narcs;
            if (!(bi >> narcs))
                return false;
            for (u32 cnt = 0; cnt < narcs; ++cnt) {
                ...
                if (!(bi >> a->target_))
                    return false;
                if (!f->semiring()->read(a->weight_, bi))
                    return false;
                if (!(bi >> a->input_))
                    return false;
                if (f->type() == Fsa::TypeTransducer) {
