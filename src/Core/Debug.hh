...
#if defined(SPRINT_RELEASE_BUILD)
#define DBG_LEVEL -1  // always ignore DBG_LEVEL in SPRINT_RELEASE_BUILD mode
#endif
...
#define DBG(level)          \
    if (DBG_LEVEL >= level) \
    (Core::Application::us()->debugChannel()) << "DBG(" << ((int)(level)) << "/" << (int)(DBG_LEVEL) << ") [" << __FILE__ << ":" << __LINE__ << ":" << __PRETTY_FUNCTION__ << "]"
...
#define DBGX(level)         \
    if (DBG_LEVEL >= level) \
    (Core::Application::us()->debugXmlChannel()) << Core::XmlOpen("debug") + Core::XmlAttribute("level", (int)(level)) + Core::XmlAttribute("max-level", (int)(DBG_LEVEL)) + Core::XmlAttribute("file", __FILE__) + Core::XmlAttribute("line", __LINE__) + Core::XmlAttribute("function", __PRETTY_FUNCTION__)
...
void printWarning(const char* msg, ...);  // uses Core::Application::us() if available, otherwise stderr
void printLog(const char* msg, ...);      // uses Core::Application::us() if available, otherwise stderr
...
#define DBG(level)          \
    if (DBG_LEVEL >= level) \
    fprintf(stdout, "(%d/%d) %s:%d:%s ", (int)(level), (int)(DBG_LEVEL), __FILE__, __LINE__, __PRETTY_FUNCTION__)
...
#define WARNING(format, ...)                                                       \
    fprintf(stderr, "WARNING %s:%d:%s ", __FILE__, __LINE__, __PRETTY_FUNCTION__); \
    fprintf(stderr, format, ##__VA_ARGS__)
