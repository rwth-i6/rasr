...
bool AbstractMixtureSetEstimator::accumulate(Core::BinaryInputStreams& is,
...
void AbstractMixtureSetEstimator::removeDensitiesWithLowWeight(Weight minObservationWeight,
...
DensityIndex AbstractMixtureSetEstimator::densityIndex(MixtureIndex                     mixtureIndex,
    ...
    if (assigningFeatureScorer_) {
        ...
        AssigningFeatureScorer::AssigningScorer scorer = assigningFeatureScorer_->getAssigningScorer(feature);
...
void AbstractMixtureSetEstimator::write(Core::XmlWriter& os) {
    ...
    for (MixtureIndex i = 0; i < mixtureEstimators_.size(); ++i) {
        if (mixtureLabels_.size() > 0) {
            mixtureEstimators_[i]->write(os, indexMaps.densityMap(), mixtureLabels_[i]);
        }
        else {
            mixtureEstimators_[i]->write(os, indexMaps.densityMap());
        }
...
bool AbstractMixtureSetEstimator::accumulateDensity(Core::BinaryInputStreams& is,
                                                    Core::BinaryOutputStream& os) {
    return GaussDensityEstimator::accumulate(is, os);
}

bool AbstractMixtureSetEstimator::accumulateMixture(Core::BinaryInputStreams& is,
                                                    Core::BinaryOutputStream& os) {
