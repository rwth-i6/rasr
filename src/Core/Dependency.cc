...
private:
    ...
    void setDependencyValue(const std::string& v) {
        dependency_.setValue(v);
    }

    void setDependencyDate(const std::string& d) {
        dependency_.setDate(d);
    }
...
public:
    Parser(const Configuration& c, DependencySet& dependencySet)
        ...
        collect(dependency);
        dependency->addTransition(0, 1, collect(new XmlStringBuilderElement("value", this, XmlStringBuilderElement::handler(&Self::setDependencyValue))));
        dependency->addTransition(1, 2, collect(new XmlStringBuilderElement("date", this, XmlStringBuilderElement::handler(&Self::setDependencyDate))));
        dependency->addFinalState(2);

        XmlRegularElement* newDependencySet = new XmlRegularElementRelay("dependency-set", this, 0, 0);
    ...
    bool buildFromStream(std::istream& is) {
        return (parseStream(is) == 0);
    }
};
}  // namespace
...
void DependencySet::add(const std::string& name, const DependencySet& dependencySet) {
    ...
    for (Dependencies::const_iterator d = dependencySet.dependencies_.begin(); d != dependencySet.dependencies_.end(); ++d) {
...
void DependencySet::add(const std::string& name, const bool value) {
    if (value)
        add(name, Dependency("true"));
    else
        add(name, Dependency("false"));
}
...
bool DependencySet::operator==(const DependencySet& s) const {
    ...
    for (Dependencies::const_iterator d = dependencies_.begin(); d != dependencies_.end(); ++d) {
        ...
        if (found == s.dependencies_.end()) {
            Application::us()->warning("dependency named '%s' with value '%s' generated on %s not in second set",
            ...
            if (d->second.value() != found->second.value()) {
                Application::us()->warning("dependency named '%s' with value '%s' generated on %s differs from value '%s' generated on %s",
                                           d->first.c_str(), d->second.value().c_str(),
...
bool DependencySet::satisfies(const DependencySet& required) const {
    ...
    for (d = required.dependencies_.begin(); d != required.dependencies_.end(); ++d) {
        ...
        if (found == dependencies_.end()) {
            Application::us()->warning("dependency named '%s' with value '%s' generated on %s not found",
                                       d->first.c_str(), d->second.value().c_str(),
                                       d->second.date().c_str());
            isEqual = false;
        }
            ...
            if (d->second.value() != found->second.value()) {
                Application::us()->warning("dependency named '%s' with required value '%s' generated on %s differs from found value '%s' generated on %s",
                                           d->first.c_str(), d->second.value().c_str(),
                                           d->second.date().c_str(), found->second.value().c_str(),
                                           found->second.date().c_str());
                isEqual = false;
            ...
            if (d->second.value() != found->second.value()) {
                if (d->first == "acoustic-model.stream-extraction") {
                    ...
                    if (dValue < foundValue) {
                        Application::us()->warning("dependency named '%s' with feature value '%s' generated on %s is smaller than acoustic model value '%s' generated on %s; should be greater equal.",
                                                   d->first.c_str(), d->second.value().c_str(),
                                                   d->second.date().c_str(), found->second.value().c_str(),
                                                   found->second.date().c_str());
                        isEqual = false;
                    ...
                    Application::us()->warning("dependency named '%s' with feature value '%s' generated on %s differs from acoustic model value '%s' generated on %s",
                                               d->first.c_str(), d->second.value().c_str(),
                                               d->second.date().c_str(), found->second.value().c_str(),
                                               found->second.date().c_str());
                    isEqual = false;
