...
void AdvancedTreeSearchManager::restart() {
  ...
  sentenceEnd_.reset();
//   weStats_.clear(acousticPruningThreshold_);
//   featureTime_.clear();
}
...
//   if( startTreesIntervalProbabilistic_ )
//   {
//     if( weStats_.cachedTimeframe != time_ )
//     {
//       Score score = ss_->relativeWordEndScore();
//       if( score != Core::Type<Score>::max )
//       {
//         weStats_.cachedTimeframe = time_;
//         verify( score >= 0 );
//         weStats_.scoreHistogram += score;
//
//         if( weStats_.lastWETimeframe > time_ ||
//             time_ - weStats_.lastWETimeframe >= startTreesInterval_ )
//         {
//           weStats_.lastWETimeframe = time_;
//         }else{
//           Score threshold = weStats_.scoreHistogram.quantile( (weStats_.scoreHistogram.total() / (startTreesInterval_ * startTressIntervalProbabilisticFactor_ )) );
//           if( score <= threshold )
//             weStats_.lastWETimeframe = time_;  // Accept this timeframe
//         }
//       }else{
// //         std::cout << "failed retrieving word-end score" << std::endl;
//         weStats_.lastWETimeframe = time_;
//       }
//       ss_->statistics->customStatistics("word end computation timeframes") += weStats_.lastWETimeframe == time_ ? 1 : 0;
//     }
//
//     return weStats_.lastWETimeframe == time_;
//   }else{
  ...
  while ( !stack.empty() ) {
    ...
    for ( Ref<Trace> arcTrace = trace; arcTrace; arcTrace = arcTrace->sibling ) {
      ...
      if( arcTrace->pronunciation == epsilonLemmaPronunciation() )
      {
        ...
        // The corrective score offset can be negative when using log-linear models
/*        if(correction.scoreOffset < 0)
          std::cerr << "WARNING: negative corrective score offset: " << correction.scoreOffset << std::endl;*/
...
Core::Ref<const LatticeAdaptor> AdvancedTreeSearchManager::buildLatticeForTrace( Ref<Trace> trace ) const
{
  ...
  while ( !stack.empty() ) {
    ...
    wordBoundaries->set(
      currentState->id(),
      Lattice::WordBoundary( trace->time, trace->transit ) );
...
Core::Ref<const LatticeAdaptor> AdvancedTreeSearchManager::getPartialWordLattice()
{
  ...
  if( ( time_ % 100 ) == 0 && time_ - currentSegmentStart_ > ( onlineSegmentationLength_ * 1.0 / onlineSegmentationTolerance_ ) * 100 )
  {
    ...
    if( bestGapLength )
    {
      log() << "online segmentation: using gap: " << bestGap.first->predecessor->time << " -> " << bestGap.second->time << " (" << bestGap.second->time - bestGap.first->predecessor->time << ")";
