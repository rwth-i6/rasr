...
void WindowingFirFilter::getFilterResponse(std::vector<Sample> &h) {
    ...
    for(u32 n = 0; n <= M_ / 2; n++) {
        ...
        for(u8 k = 1; k <= cutOff_.size(); k++) {
            ...
            h[n] = h[M_ - n] +=
                (cutOff_[k - 1].first - (k != cutOff_.size() ? cutOff_[k].first : 0.0)) *
...
Core::XmlWriter& WindowingFirFilterParameter::dump(Core::XmlWriter &o) const {
    ...
    for(u8 i = 0; i < cut_off_.size(); i ++) {
        o << Core::XmlEmpty("cut-off")
            + Core::XmlAttribute("gain", cut_off_[i].first)
            + Core::XmlAttribute("omega", cut_off_[i].second);
    }
...
bool WindowingFirFilterParameter::read(BinaryInputStream &i) {
    u32 size; i >> size; cut_off_.resize(size);
...
Core::ParameterFloat WindowingFirFilterNode::paramLength
("length", "length of output", 5.0);

Core::ParameterBool WindowingFirFilterNode::paramRemoveDelay
("remove-delay", "removes padding from the beginning and the end of the output", false);

Core::ParameterString WindowingFirFilterNode::paramCutOff
("cut-off", "gain1:omega1;gain2:omega2;... ", "");

Core::ParameterFloat WindowingFirFilterNode::paramOvershoot
("overshoot", "max amplitude oscillation in pass band and in stop band interval", .01, 0);

Core::ParameterFloat WindowingFirFilterNode::paramTransitionRegionWidth
("transition-region-width", "width of transition region in Hz", 400, 0);
