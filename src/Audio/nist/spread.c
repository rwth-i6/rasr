...
int sp_mc_read_data(void *buffer, size_t num_sample, SP_FILE *sp)
{
	...
	return_err(proc,104,0,"Read on a file not opened for read");
#ifdef isnotansi
    if (num_sample < 0)
    ...
    if (spifr->status->user_compress != SP_wc_none) 
 	return_err(proc,110,0,"Unable to Read file in compressed mode");
...
{
    volatile int decomp_into_memory=TRUE;
    int wav_bytes=0;
    SPIFR *spifr = sp->read_spifr;
    FOB *fob_in=0, *fob_out=0;
    char *buff=0;
    int blen;
	...
	spifr->status->temp_filename = sptemp();
	...
	if (sp_verbose > 15)
	    fprintf(spfp,"Proc %s: Closing waveform file \"%s\"\n",
		   proc, spifr->status->external_filename);
	
	if (fclose(spifr->waveform->sp_fp))
	    return_err(proc, 403, 0,
		       rsprintf("Unable to close waveform file \"%s\"",
...
/*	fclose(fob_in->fp);  already closed */
	fob_in->fp = FPNULL;
    }
    fob_destroy(fob_in);	    
