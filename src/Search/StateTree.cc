...
public:
    ...
    struct Ordering;
    friend struct PronunciationSuffix::Ordering;
    struct Equality;
    friend struct PronunciationSuffix::Equality;
    struct Hash;
    friend struct PronunciationSuffix::Hash;
 ...
 */
inline const Am::Allophone* StateTree::getAllophone(const PronunciationSuffix& s) const {
 ...
 */
void StateTree::advancePronunciationSuffix(const PronunciationSuffix&                                  _s,
...
public:
    ...
    bool isSorted() const {
        return consolidation_ >= sorted;
    }
    bool isHashValid() const {
        return consolidation_ >= hashValid;
    }
    ...
    bool hasUniqueHead() const {
        return hasUniqueHead_;
    }
...
void StateTree::SuffixSet::partition(SuffixSetList& conts) const {
    ...
    for (ItemList::const_iterator i = items_.begin(); i < items_.end(); ++i) {
        ...
        if (model_->isPathRecombinationInFanInEnabled_) {
            ...
            if ((i->phoneme == 0 && i->subState == model_->nSubStates(*i) - 1 && i->phoneState == model_->nPhoneStates(*i) - 1) || (i->phoneme == 1 && i->subState == 0 && i->phoneState == 0))
...
void StateTree::SuffixSet::advance(ItemList& endingItems) {
    ...
    for (ItemList::iterator i = items_.begin(); i != items_.end(); ++i)
        model_->advancePronunciationSuffix(*i,
...
// ?
void StateTree::SuffixSet::leap(std::vector<const Bliss::Pronunciation*>& leaps) {
    leaps.clear();
    for (ItemList::iterator i = items_.begin(); i != items_.end(); ++i) {
...
StateTree::CoarticulationStructure::CoarticulationStructure(const StateTree* sm, Bliss::LexiconRef l) {
 ...
 */
struct StateTree::BatchRequest : public BuildRequest {
    ...
    virtual u32 hash() const {
        return suffixes.hash() + parentsSuffixes.hash();
    }
    ...
    virtual bool equals(const BuildRequest* _r) const {
        ...
        require(r);
        return ((suffixes == r->suffixes) && (parentsSuffixes == r->parentsSuffixes));
    }
};
...
struct StateTree::LeapRequest : public BatchRequest {
    std::vector<std::vector<const Bliss::Pronunciation*>> leaps;

    LeapRequest(StateTree* m, StateId s)
            : BatchRequest(m, s) {
        verify(0);
        type = leapRequest;
...
const StateTree::StateDesc::ModelIndex StateTree::invalidAcousticModel = Core::Type<StateTree::StateDesc::ModelIndex>::max;
...
StateTree::StateTree(const Core::Configuration&         c,
    ...
    if (acousticModel_->nStateTransitions() >= Core::Type<StateDesc::ModelIndex>::max) {
    ...
    if (acousticModel_->nEmissions() >= Core::Type<StateDesc::ModelIndex>::max) {
        criticalError("Too many mixtures. Current state tree implementation handles max %d mixtures",
                      Core::Type<StateDesc::ModelIndex>::max);
    }
    ...
    if (!filename.empty()) {
        ...
        if (!reader.read(*this, filename)) {
            ...
            log("wrote state tree to \"%s\"", filename.c_str());
        }
        else {
    ...
    if (dc.isOpen())
        draw(dc, phonemeInventory());
}

StateTree::~StateTree() {
    delete coarticulationStructure_;
    coarticulationStructure_ = 0;
    initialPhonemes_         = 0;
}
    ...
    for (StateId s = 0; s < nStates(); ++s) {
        ...
        for (tie(ss, ss_end) = successors(s); ss < ss_end; ++ss)
            for (tie(sss, sss_end) = successors(ss); sss < sss_end; ++sss) {
                ...
                result &= r;
                if (!r)
                    warning("2nd order mesh structure s=%d, ss=%d, sss=%d, ss2=%d",
                            s, ss, sss, ss2);
            }
        bool r = (ss2 == ss2_end);
        if (!r)
            warning("2nd order mesh structure s=%d, ss2=%d, ss2_end=%d",
                    s, ss2, ss2_end);
        result &= r;
    }
 ...
 * lemma pronunciation
 */
void StateTree::createExits(const SuffixSet::ItemList& endingItems,
                            ExitList&                  exits) {
    Exit exit;
...
bool StateTree::shouldLeap(const SuffixSet& c, Depth d) const {
    ...
    if (c.hasLeaped())
        return false;
    if (d < earlyRecombinationLimit_)
        return false;
    if ((s32)c.nStatesRemaining() < earlyRecombinationLimit_)
        return false;
    return true;
}
    ...
    for (StateId si = 0; si < nStates(); ++si) {
        ...
        for (const_ExitIterator i = s.exits.begin(); i != s.exits.end(); ++i) {
            if (i->pronunciation) {
                ...
                if (phi)
                    os << " /" << i->pronunciation->pronunciation()->format(phi) << "/";
                os << " -> " << i->transitEntry;
            }
        }
        os << "\"]" << std::endl;
