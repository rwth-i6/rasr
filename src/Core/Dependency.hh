...
public:
    Dependency() {}
    Dependency(const std::string& value) {
        setValue(value);
    }
    ...
    unsigned int getChecksum() const {
        unsigned int ret = 0;
        for (unsigned int a = 0; a < value_.length(); a++)
            ret += (unsigned int)value_[a];
        return ret;
    }
};
...
public:
    void clear() {
        dependencies_.erase(dependencies_.begin(), dependencies_.end());
    }
    ...
    friend std::ostream& operator<<(std::ostream& o, const DependencySet& s) {
        for (DependencySet::Dependencies::const_iterator d = s.dependencies_.begin(); d != s.dependencies_.end(); ++d)
