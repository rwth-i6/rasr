...
header = mtrf_malloc((unsigned int) (hsize - sizeof fh + 1));
if (header == CNULL) {
...
h = (struct header_t *) mtrf_malloc((unsigned int) sizeof(struct header_t));
if (h != HDRNULL) {
	h->fc = fc;
	h->fv = fv;
}
...
f = (struct field_t *) mtrf_malloc((unsigned int) sizeof(struct field_t));
if (f == FNULL)
	return FNULL;

p1 = mtrf_malloc((unsigned int) (strlen(fieldname) + 1));
if (p1 == CNULL) {
	mtrf_free((char *) f);
	return FNULL;
}

p2 = mtrf_malloc((unsigned int) (vlen + 1));
if (p2 == CNULL) {
	mtrf_free((char *) f);
	mtrf_free(p1);
	return FNULL;
...
struct field_t **spx_get_field_vector(int elements)
{
    ...
    return (struct field_t **) mtrf_malloc((unsigned int) vbytes);
}
