...
inline void LinearAndEluLayer<T>::_forward(const std::vector<NnMatrix*>& input, NnMatrix& output, bool reset) {
    PrecursorLinear::_forward(input, output, reset);
    PrecursorElu::_forward(output, output);
}

template<typename T>
inline void LinearAndEluLayer<T>::forward(const std::vector<NnMatrix*>& input, NnMatrix& output) {
    PrecursorLinear::forward(input, output);
}
...
template<typename T>
inline void LinearAndEluLayer<T>::initComputation(bool sync) const {
    PrecursorLinear::initComputation(sync);
}

template<typename T>
inline void LinearAndEluLayer<T>::finalize() {
    PrecursorLinear::finalize();
    PrecursorElu::finalize();
}

template<typename T>
u32 LinearAndEluLayer<T>::getNumberOfFreeParameters() const {
    return PrecursorLinear::getNumberOfFreeParameters();
}
//=============================================================================
