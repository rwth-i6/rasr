...
With ``lm-lookahead`` enabled, both look-aheads are added to the same within-word extensions and both are removed
again at the word end. At a word end, the LM-lookahead history is advanced with the word-LM tokens the search
actually applies, i.e. with the known or unknown word a completed fallback piece sequence resolves to, not with the
(empty) token sequence of the piece lemma itself.
