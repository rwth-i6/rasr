...
template <class F> class FsaMapperStateIteratorBase;
template <class F> class FsaMapperArcIteratorBase;
...
public:
    ...
    virtual bool Done() const { return !fsa_->getState(s_); }
    virtual StateId Value() const { return fsa_->getState(s_)->id(); }
...
public:
    ...
    virtual bool Done() const { return (i >= arcs_.size()); }
    virtual const Arc& Value() const { return arcs_[i]; }
    virtual void Next() { ++i; }
    virtual size_t Position() const { return i; }
    virtual void Reset() { i = 0; }
    virtual void Seek(size_t a) { i = a; }
    virtual uint32 Flags() const { return FstLib::kArcValueFlags; }
