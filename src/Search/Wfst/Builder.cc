...
Core::Ref<const Am::AcousticModel> Resources::acousticModel() const {
    if (!models_->acousticModel()) {
        models_->setAcousticModel(Am::Module::instance().createAcousticModel(Core::Configuration(models_->getConfiguration(), "acoustic-model"),
                                                                             lexicon(),
...
void Automaton::copyAttributes(Automaton* dest) const {
    for (Attributes::const_iterator a = attributes_.begin(); a != attributes_.end(); ++a)
