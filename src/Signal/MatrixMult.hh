...
bool MatrixMultiplicationNode<T>::configure() {
    ...
    auto inputAttributes = std::make_shared<Flow::Attributes>();
...
bool MatrixMultiplicationNode<T>::updateMatrix(const std::string& filename) {
    ...
    if (matrixCache_) {
        ...
        if (m != matrixCache_->end())
            ...
            if (currentMatrix_)
                matrixCache_->insert(std::make_pair(filename, currentMatrix_));
        }
    }
    else {
        currentMatrix_     = loadMatrix(filename);
...
template<typename T>
typename MatrixMultiplicationNode<T>::MatrixRef MatrixMultiplicationNode<T>::loadMatrix(const std::string& filename) {
    MatrixRef result;
    if (filename.empty()) {
        ...
        if (!Math::Module::instance().formats().read(filename, *result)) {
