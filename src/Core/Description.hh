...
public:
    ...
    public:
        ...
        bool operator==(const Stream& s) const {
            return attributes_ == s.attributes_;
        }
    };
...
public:
    ...
    const Stream& operator[](size_t index) const {
        require(index < streams_.size());
        return streams_[index];
    }

    Core::Description::Stream& mainStream() {
        return getStream(0);
    }
    const Core::Description::Stream& mainStream() const {
        require(!streams_.empty());
...
template<class ValueType>
bool Description::Stream::verifyValue(const std::string& name, const ValueType& expected, bool critical) const {
    ValueType value;
    getValue(name, value);
    if (value != expected) {
