...
/*
 * Score accessor that computes CTC prefix scores on the fly
 */
...
/*
 * Score accessor that wraps a legacy Mm::FeatureScorer::Scorer
 */
class FeatureScorerScoreAccessor : public ScoreAccessor {
public:
    FeatureScorerScoreAccessor(Mm::FeatureScorer::Scorer const& scorer, TimeframeIndex time);

    Score          getScoreForLabel(LabelIndex labelIndex) const override;
    TimeframeIndex getTime() const override;

private:
    Mm::FeatureScorer::Scorer scorer_;
    TimeframeIndex            time_;
};
