...
#if 0
            // The semantic of special symbols in the input is still to be defined
            //
            // Example:
            //
            //  Left automaton:   O --1:*ANY*--> O
            //  Right automaton : O --*ANY*:1--> O
            //
            for (typename _State::const_reverse_iterator sal = sl->rbegin();
                 (sal != sl->rend()) && (sal->output() > Fsa::LastLabelId); ++sal) {
                label = sal->input();
                switch (sal->output()) {
                case Fsa::Any:
                    for (ar = sr->begin(); ar < sr->end(); ++ar) {
                        //if (type() == Fsa::TypeAcceptor) label = ar->output();
                        if (fl_->type() == Fsa::TypeAcceptor) label = ar->input();
                        sp->newArc(insertState(sal->target(), 0, ar->target()),
                                   semiring()->extend(sal->weight(), ar->weight()), label, ar->output());
                    }
                    break;
                case Fsa::Failure:
                    for (al = sl->begin(), ar = sr->begin(); (al < sl->end()) && (ar < sr->end()); ++al) {
                        if (ar->input() > Fsa::LastLabelId) break;
                        if (al->output() >= ar->input())
                            for (; ((ar < sr->end()) && (al->output() == ar->input())); ++ar);
                        if (al->output() > ar->input()) {
                            if (type() == Fsa::TypeAcceptor) label = Fsa::Failure;
                            sp->newArc(insertState(sal->target(), 0, sr->id()),
                                       semiring()->extend(sal->weight(), ar->weight()), label, Fsa::Failure);
                        }
                    }
                    break;
                case Fsa::Else:
