...
namespace Fsa {
    ...
    private:
        ...
        public:
            Trace init(const StaticAutomaton *fsa) {
                ...
                //std::cout << "init " << sp << " " << sp->id() << std::endl;
            ...
            Trace update(Trace trace, Trace recombineTrace, Score score) {
                ...
                //std::cout << "update " << recombineTrace << " (" << sp->id() << ")" << std::endl;
                Score diffScore = score - Score(sp->weight_);
                //std::cout << trace << " " << diffScore << std::endl;
        ...
        public:
            ...
            Trace update(Trace trace, Trace recombineTrace, Score score) {
                ...
                } else sp->newArc(trace, Weight(f32(diffScore)), Epsilon);
#if 0
                const State *_sp = fastState(trace);
                for (State::const_iterator a = _sp->begin(); a != _sp->end(); ++a)
                    if (a->input() != Epsilon)
                        sp->newArc(a->target(), Weight(f32(a->weight()) + diffScore), a->input());
                    else for (State::iterator _a = sp->begin(); _a != sp->end(); ++_a)
                        if ((_a->input() == a->input()) &&
                            (f32(fastState(_a->target())->weight_) + f32(_a->weight()) >
                             f32(fastState(a->target())->weight_) + f32(a->weight()))) {
                            _a->target_ = a->target();
                            _a->weight_ = Weight(f32(a->weight()) + diffScore);
                            _a->input_ = _a->output_ = a->input();
                        }
#endif
                if (Score(sp->weight_) < score) sp->weight_ = Weight(score);
        ...
        struct ExpandedToken {
            Score score_;
            Trace trace_;
            ExpandedToken() : score_(Core::Type<Score>::max), trace_(InvalidStateId) {}
            ExpandedToken(Score score, Trace trace) : score_(score), trace_(trace) {}
        };
    ...
    private:
        enum ExpandType { ExpandTypeNone = 0, ExpandTypeUpdated = 1, ExpandTypeNew = 2 };
        ExpandType expandToken(Score score, Trace trace, StateId target, LabelId output) {
            if (output != Epsilon) trace = tracer_->add(trace, output, score);
            if (fsa_->fastState(target)->isFinal()) {
                ...
                //std::cout << "final " << target << " " << expandedTokens_[target].trace_ << std::endl;
            ...
            while (!tokens_.empty()) {
                ...
                for (ActiveTokens::const_iterator t = tokens_.begin(); t != tokens_.end(); ++t) {
                    ...
                    for (State::const_iterator a = sp->begin(); (a != sp->end()) && (a->input() == Epsilon); ++a) {
                        ...
                        //std::cout << "expand from " << sp->id() << " to " << a->target() << " " << t->trace_ << std::endl;
                        ...
                        if (type >= ExpandTypeUpdated) {
                            ...
                            if (type == ExpandTypeNew) expandedStateIds.push_back(a->target());
                        }
                    }
                }
        ...
        template <class Scorer> void expand(const Scorer &scorer, Core::Vector<StateId> &expandedStateIds) {
            ...
            for (ActiveTokens::const_iterator t = tokens_.begin(); t != tokens_.end(); ++t) {
                ...
                for (; a != sp->end(); ++a) {
                    ...
                    //std::cout << "expand from " << sp->id() << " to " << a->target() << std::endl;
        ...
        void prune(const Core::Vector<StateId> &expandedStateIds) {
            ...
            for (Core::Vector<StateId>::const_iterator s = expandedStateIds.begin(); s != expandedStateIds.end(); ++s) {
                ...
                if (token.score_ < minThreshold_) tokens_.push_back(Token(*s, token.score_, token.trace_));
        ...
        void purge() {
            ...
            for (ActiveTokens::iterator t = tokens_.begin(); t != tokens_.end(); ++t) t->trace_ = mapping[t->trace_];
            if (bestFinalTrace_ != InvalidStateId) bestFinalTrace_ = mapping[bestFinalTrace_];
        }

        Trace getBestFinalTrace() const { return bestFinalTrace_; }
    ...
    public:
        ...
        void restart() {
            ...
            if (initial) {
                ...
                if (tracer_) bestFinalTrace_ = tracer_->init(fsa_);
        ...
        template<class Scorer> void feed(const Scorer &scorer) {
            ...
            prune(expandedStateIds);
            //std::cout << "---2---" << std::endl;
            //std::cout << bestFinalTrace_ << std::endl;
            ...
            //std::cout << bestFinalTrace_ << std::endl;
            //addInitialStateToTracer();
            //AutomatonCounts c = count(ConstAutomatonRef(tracer_));
            //std::cout << c.nStates_ << " " << c.nArcs_ << std::endl;
            //write(ConstAutomatonRef(tracer_.get()), "xxx.fsa.gz");
            //getchar();
            if (++time_ % 10 == 0) purge();
        }
        ...
        ConstAutomatonRef getCurrentWordLattice() {
            ...
            //AutomatonCounts c = count(unite(finalAutomata));
            //std::cout << c.nStates_ << " " << c.nArcs_ << std::endl;
            return transpose(unite(finalAutomata));
        }

        u32 nActiveTokens() const { return tokens_.size(); }
