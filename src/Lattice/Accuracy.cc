...
#if 0
Fsa::ConstAutomatonRef ExactPhonemeAccuracyGenerator::calculateAccuracies(
    Fsa::ConstAutomatonRef lemmaPron) {
    Fsa::ConstAutomatonRef phon =
        Fsa::levenshtein(
            correct_,
            mapLemmaPronToPhon(lemmaPron),
            delCost_, insCost_, subCost_, corCost_);

    return mapPhonToLemmaPron(phon);
}
#endif
...
protected:
    virtual Fsa::LabelId label(Fsa::LabelId l) const {
        return l;
    }
    ...
    for (Fsa::State::const_iterator a = sp->begin(); a != sp->end(); ++a) {
        ...
        for (std::vector<Speech::AlignmentItem>::const_iterator al = alignment->begin(); al != alignment->end();) {
            ...
            for (; bl != alignment->end(); ++bl) {
                if (parent_.label(bl->emission) != _label)
                    break;
            }
...
ApproximatePhoneAccuracyAutomaton::AlignmentToActiveStates::AlignmentToActiveStates(
    ...
    for (Speech::Alignment::const_iterator al = alignment.begin(); al != alignment.end();) {
        ...
        for (; bl != (alignment.end() - 1);) {
            ...
            if (parent.label(cl->emission) != _label)
                break;
            bl = cl;
        }
...
private:
    typedef std::unordered_map<Fsa::LabelId, bool> States;
    typedef Core::Vector<States>                   ActiveStates;
...
private:
    ...
    public:
        AlignmentToSoftAccuracies(SoftFramePhoneAccuracyAutomaton& parent)
                : parent_(parent) {}
...
private:
    typedef std::unordered_map<Fsa::LabelId, f32> SoftAccuracy;
