...
bool StaticAlphabet::addIndexedSymbol(const std::string& symbol, LabelId index,
    ...
    if ((size_t(index) < symbols_.size()) && !symbols_[index].name_.empty())
        return false;
...
bool StaticAlphabet::hasIndex(LabelId index) const {
    switch (index) {
        case Epsilon:
        case Any:
        case Failure:
        case Else:
            return true;
        default:
            if (index == InvalidLabelId or static_cast<size_t>(index) >= symbols_.size()) {
                return false;
            }
            return true;
    }
...
std::string StaticAlphabet::symbol(LabelId index) const {
    switch (index) {
        ...
        default:
            if (index == InvalidLabelId)
                return "*static-alphabet-invalid-label*";
            if (size_t(index) >= symbols_.size())
                return Core::form("*static-alphabet-index-out-of-bounds(%i/%zu)*", index, symbols_.size());
            return symbols_[index].name_;
    }
}

LabelId StaticAlphabet::index(const std::string& symbol) const {
    std::unordered_map<std::string, LabelId, Core::StringHash>::const_iterator i =
