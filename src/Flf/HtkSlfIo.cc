...
HtkSlfContext::HtkSlfContext() {
    ...
    if (silId_ == Fsa::InvalidLabelId)
        Core::Application::us()->warning("Lexicon does not provide a pronunciation for silence.");
    unkId_ = Lexicon::us()->unkLemmaPronunciationId();
    if (unkId_ == Fsa::InvalidLabelId)
        Core::Application::us()->warning("Lexicon does not provide a pronunciation for the unknown word.");
    type_         = HtkSlfForward;
    fps_          = 0.0;
    isCapitalize_ = false;
    mergePenalty_ = true;
...
void HtkSlfContext::clear() {
    ...
    wrdPenalty_ = Semiring::Invalid;
    silPenalty_ = Semiring::Invalid;
    epsSymbol_  = "!NULL";
    lmName_.clear();
}
...
HtkSlfContextRef HtkSlfContext::create(const Core::Configuration& config) {
    ...
    if (wrdPenalty != Semiring::Invalid)
        ...
        Core::Application::us()->error("Cannot set silence penalty without setting word penalty");
    // set merge penalties
...
public:
    ...
    bool logComments() const {
        return logComments_;
    }
    void setLogComments(bool logComments) {
        logComments_ = logComments;
    }
...
void HtkSlfBuilder::conformContext(const HtkSlfHeader& header) {
    ...
    wrdPenalty_ = (context_->wordPenalty() == Semiring::One) ? Semiring::One : convert(context_->wordPenalty());
    silPenalty_ = (context_->silPenalty() == Semiring::One) ? Semiring::One : convert(context_->silPenalty());
}
...
HtkSlfBuilder::Status HtkSlfBuilder::link(const StringPairList& props) {
    ...
    Fsa::LabelId labelId;
    if (label.empty()) {
        labelId = (context_->type() == HtkSlfForward) ? nodes_[targetId].labelId : nodes_[sourceId].labelId;
    }
    else {
    ...
    if ((Fsa::FirstLabelId <= labelId) && (labelId <= Fsa::LastLabelId))
        penalty = (labelId == context_->silence()) ? silPenalty_ : wrdPenalty_;
    ...
    while (!stack.isEmpty()) {
        ...
        for (State::const_iterator a = sp->begin(); a != sp->end(); ++a) {
            ...
            const Bliss::Pronunciation* p =
                    ((Fsa::FirstLabelId <= a->input()) && (a->input() <= Fsa::LastLabelId)) ? lpAlphabet->lemmaPronunciation(a->input())->pronunciation() : 0;
            if (p && p->length()) {
                initialPhonemeId = (*p)[0];
                initialPhonemeId =
                        (phonemeInventory->phoneme(initialPhonemeId)->isContextDependent()) ? initialPhonemeId : Bliss::Phoneme::term;
                finalPhonemeId = (*p)[p->length() - 1];
                finalPhonemeId =
                        (phonemeInventory->phoneme(finalPhonemeId)->isContextDependent()) ? finalPhonemeId : Bliss::Phoneme::term;
            }
            else {
            ...
            if ((initialPhonemeId == Bliss::Phoneme::term) || (initialPhonemeId != source.initialPhonemeId))
            ...
            if ((finalPhonemeId == Bliss::Phoneme::term) || (finalPhonemeId != target.finalPhonemeId))
...
public:
    LemmaPronunciationToWordVariant()
            : ToWordVariant() {
...
public:
    HtkSlfHeaderAndMappingBuilder(
        ...
        header_.lmName     = "";
        ScoreId lmId       = f->semiring()->id("lm");
        header_.lmScale    = (f->semiring()->hasId(lmId)) ? f->semiring()->scale(lmId) : Semiring::UndefinedScale;
        ...
        if ((mapping_.finals.size() == 1) && !mapping_.finals.front()->hasArcs() && (f->semiring()->compare(mapping_.finals.front()->weight(), f->semiring()->one()) == 0)) {
