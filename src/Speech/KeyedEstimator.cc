...
void KeyedEstimator::processAlignedFeature(Core::Ref<const Feature> f, Am::AllophoneStateIndex i) {
...
void KeyedEstimator::processAlignedFeature(Core::Ref<const Feature> f, Am::AllophoneStateIndex i, Mm::Weight w) {
    Mm::MixtureIndex mixture = acousticModel()->emissionIndex(i);
    Mm::DensityIndex density = mixtureSet_->mixture(mixture)->densityIndex(
            assigningFeatureScorer_->getAssigningScorer(f)->bestDensity(mixture));
    (*currentAccumulator_)->accumulate((*f)[featureStream_], density, mixture, mixtureSet_, w);
}
...
void KeyedEstimator::combine() {
    ...
    for (std::set<std::string>::iterator key = keysToAdd.begin();
        ...
        log("Combining accumulator '") << *key << "' using weight '" << paramCombinationWeight(config) << "'";
