...
void OnnxModelBase::loadPrior() {
    ...
    log() << "use context-dependent label priors (context-size:" << priorContextSize_ << ")";
    ...
    log() << "successfully loaded " << contextLogPriors_.size() << " context-dependent label priors";
}
...
void OnnxModelBase::encode() {
    ...
    log() << "encode input features (" << inputBuffer_.size() << ", "
          << inputBuffer_[0].size() << ")";

    MappedValueList                    inputs;
    std::vector<Math::FastMatrix<f32>> batchMat;  // single sequence: F x T
    batchMat.emplace_back(inputBuffer_[0].size(), inputBuffer_.size());
    for (u32 t = 0; t < inputBuffer_.size(); ++t) {
        const std::vector<f32>& f = inputBuffer_[t];
        std::copy(f.begin(), f.end(), &(batchMat.front().at(0, t)));
    }
    inputs.emplace_back(std::make_pair(encoder_features_name_, Onnx::Value::create(batchMat, true)));  // transpose to 1 x T x F
    ...
    outputs.front().get(encoder_outputs_, true);  // transposed to 1 x F x T
...
void OnnxFfnnTransducer::decodeBatch(ScoreCache& scoreCache) {
    ...
    MappedValueList       inputs;
    Math::FastMatrix<s32> mat(contextSize_, batchHash_.size());  // H x B
    if (hmmTopology_ && !loopUpdateHistory_) {
        for (u32 bIdx = 0u; bIdx < batchHash_.size(); ++bIdx) {
            const LabelSequence& seq = labelSeqCache_[batchHash_[bIdx]];
            std::copy(seq.begin(), seq.end(), &(mat.at(0, bIdx)));  // Copy seq into matrix as column
        }
    }
    else {
        const HistoryCache& cache = labelHistoryManager_->historyCache();
        for (u32 bIdx = 0u; bIdx < batchHash_.size(); ++bIdx) {
            ...
            std::copy(lhd->labelSeq.begin(), lhd->labelSeq.end(), &(mat.at(0, bIdx)));  // Copy seq into matrix as column
        }
    }
