...
namespace Ftl {
...
#if 0
            void compact(const std::vector<bool> &deletable) {
                // recompress substates and estimate mapping between old and new substates
                Core::Vector<u8> substates;
                std::map<Cursor, Cursor> map;
                for (Core::Vector<u8>::const_iterator i = substates_.begin(); i != substates_.end(); ++i) {
                    // copy hash pointer
                    map[i - substates_.begin()] = substates.size();
                    substates.push_back(sizeof(Cursor), 0);

                    /*! \todo update compressed layout */
                    // copy substates
                    for (i += sizeof(Cursor); *i != 0xff; i += 1 + size(*i)) {
                        u8 partition = *i;
                        u32 s = size(partition);
                        if (deletable[i - substates_.begin()]) {
                            partition &= ~0x01;
                            s -= sizeof(Substates::Cursor);
                        }
                        substates.push_back(partition);
                        substates.push_back(i + 1, i + 1 + s);
                    }
                    substates.push_back(0xff);
                }
                std::swap(substates_, substates);

                // update links inside hash-table
                for (Core::Vector<Cursor>::iterator i = bins_.begin(); i != bins_.end(); ++i) *i = map[*i];
                for (Core::Vector<u8>::iterator i = substates.begin(); i != substates.end(); ++i) {
                    setBytes(substates_.begin() + map[i - substates.begin()], map[*i], sizeof(Cursor));
                    for (; *i != 0xff; i += 1 + size(*i));
                }
            }
#endif
        };
...
#if 0
            FILE *f = ::fopen("substates", "wb");
            ::fwrite(substates_.begin(), 1, substates_.size(), f);
            fclose(f);
            f = ::fopen("states", "wb");
            ::fwrite(&states_[0], sizeof(State_), states_.size(), f);
            fclose(f);
            f = ::fopen("outputshashed", "wb");
            ::fwrite(&outputsHashed_[0], sizeof(Fsa::LabelIdStrings::Id), outputsHashed_.size(), f);
            fclose(f);
            f = ::fopen("outputs", "wb");
            ::fwrite(&outputs_[0], sizeof(Fsa::LabelId), outputs_.size(), f);
            fclose(f);
#endif
...
#if 0
    // After doing an update, the LocalDeterminizeAutomaton was doubled ...
    ...
    template<class _Automaton> typename _Automaton::ConstRef localDeterminize(typename _Automaton::ConstRef f) {
        require(f);
