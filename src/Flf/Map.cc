...
public:
    TransducerLattice(ConstLatticeRef l)
            : Precursor(l) {}
    ...
    virtual std::string describe() const {
        return Core::form("transducer(%s)", fsa_->describe().c_str());
    }
};
// -------------------------------------------------------------------------
...
public:
    MapInputFromLemmaPronunciationToLemmaLattice(ConstLatticeRef l)
            : Precursor(l) {
...
public:
    MapOutputFromLemmaPronunciationToLemmaLattice(ConstLatticeRef l)
            : Precursor(l) {
...
public:
    MapInputFromLemmaToSingleLemmaPronunciationLattice(ConstLatticeRef l)
            : Precursor(l) {
        isAcceptor_ = (l->type() == Fsa::TypeAcceptor);
...
public:
    MapOutputFromLemmaToSingleLemmaPronunciationLattice(ConstLatticeRef l)
            : Precursor(l) {
        verify(l->type() != Fsa::TypeAcceptor);
        lexicon_   = Lexicon::us().get();
        lAlphabet_ = Lexicon::us()->lemmaAlphabet();
...
public:
    ...
    static LabelMapRef create(const Core::Configuration& config, Fsa::ConstAlphabetRef defaultAlphabet) {
        ...
        Fsa::ConstAlphabetRef from             = fromAlphabetName.empty() ? defaultAlphabet : Lexicon::us()->alphabet(Lexicon::us()->alphabetId(fromAlphabetName, true));
        std::string           toAlphabetName   = paramToAlphabet(config);
        Fsa::ConstAlphabetRef to               = toAlphabetName.empty() ? from : Lexicon::us()->alphabet(Lexicon::us()->alphabetId(toAlphabetName, true));
