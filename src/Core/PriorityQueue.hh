...
class PriorityQueueBase : public T_Heap {
...
void PriorityQueueBase<H, PF>::downHeap(Index i) {
    ...
    while (i <= Precursor::size() / 2) {
        j = 2 * i;
        if (j < Precursor::size() && precedes_(Precursor::item(j + 1), Precursor::item(j)))
            j = j + 1;
        if (!precedes_(Precursor::item(j), e))
            break;
        Precursor::move(i, j);
        i = j;
    }
...
protected:
    const Item& item(Index i) const {
        return heap_[i];
    }
    void put(Index i, const Item& e) {
        heap_[i] = e;
    }
    void move(Index to, Index from) {
        heap_[to] = heap_[from];
    }
    void append(const Item& e) {
        heap_.push_back(e);
    }
    void deleteLast() {
        heap_.pop_back();
...
template<class T_Item,
         class T_Key,
         class T_KeyFunction,
         template<typename, typename, typename> class T_Map,
...
class PriorityQueue : public PriorityQueueBase<UntracedHeap<T_Item>, T_PriorityFunction> {
...
template<class T_Item, class T_Key = typename T_Item::first_type,
         class T_KeyFunction      = Core::select1st<T_Item>,
         class T_PriorityFunction = Core::binary_compose<std::less<typename T_Item::second_type>, Core::select2nd<T_Item>, Core::select2nd<T_Item>>,
         class T_Hash_Obj         = std::hash<T_Key>>
class TracedPriorityQueue : public PriorityQueueBase<TracedHeap<T_Item, T_Key, T_KeyFunction, DefaultHashMap, T_Hash_Obj>, T_PriorityFunction> {
    typedef PriorityQueueBase<TracedHeap<T_Item, T_Key, T_KeyFunction, DefaultHashMap, T_Hash_Obj>, T_PriorityFunction> Precursor;

public:
    TracedPriorityQueue(u32 maxSize = Type<u32>::max)
            : Precursor(maxSize) {}
