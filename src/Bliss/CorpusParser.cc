...
void CorpusDescriptionParser::initSchema() {
    ...
    // Condition/Speaker must apear before Orth, left and right Context may appear anywhere, but at most once
    // The original automaton (before left/right context) consisted of 5 states, now we need to replicate it
    // 4 times to cover the condition that neither left/right, only left/right and both left/right have been parsed
    for (XmlRegularElement::State offset = 0; offset <= 15; offset += 5) {
        segment->addTransition(offset + 0, offset + 0, description_decl);
        segment->addTransition(offset + 0, offset + 1, segmentConditionDesc);
        segment->addTransition(offset + 0, offset + 1, segmentCondition);
        segment->addTransition(offset + 2, offset + 3, segmentConditionDesc);
        segment->addTransition(offset + 2, offset + 3, segmentCondition);
        segment->addTransition(offset + 0, offset + 2, segmentSpeakerDesc);
        segment->addTransition(offset + 0, offset + 2, segmentSpeaker);
        segment->addTransition(offset + 1, offset + 3, segmentSpeakerDesc);
        segment->addTransition(offset + 1, offset + 3, segmentSpeaker);
        segment->addTransition(offset + 0, offset + 4, orth);
        segment->addTransition(offset + 1, offset + 4, orth);
        segment->addTransition(offset + 2, offset + 4, orth);
        segment->addTransition(offset + 3, offset + 4, orth);
    }
    ...
    for (XmlRegularElement::State s = 0; s < 20; s++) {
        segment->addFinalState(s);
    }
