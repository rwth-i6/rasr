...
#if 0
        if (d < c.nStatesRemaining()) return false;
#endif
...
#if 0
        // Number of batches is not constant due to variations in memory allocation.
        // We therefore do not report it to allow regression tests.
        msg.form("\ntotal: %d states, %d (%d) batches, %d exits (%d bytes)",
                         nStates(),
                         batches_.size(), successorBatches_.size(),
                         total.nExits,
                         states_.size() * sizeof(State)
                         + total.nExits * sizeof(ExitList::_Node)
                         + batches_.size() * sizeof(StateId)
                         + successorBatches_.size() * sizeof(BatchId));
#else
