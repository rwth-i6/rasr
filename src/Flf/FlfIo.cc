...
private:
    ...
    bool setFsaDescriptor(const Core::XmlAttributes& atts, FsaDescriptor& fd) {
        ...
        c = atts["file"];
        if (c)
            Core::strconv(std::string(c), fd.file);
        else
            fd.file.clear();
        c = atts["format"];
        if (c)
            Core::strconv(std::string(c), fd.format);
        else
            fd.format.clear();
    ...
    void inputAlphabetFile(const Core::XmlAttributes atts) {
        ...
        if (c)
            Core::strconv(std::string(c), desc_->inputAlphabetName);
        else
            desc_->inputAlphabetName.clear();
    }
    ...
    void outputAlphabetFile(const Core::XmlAttributes atts) {
        ...
        const char* c = atts["name"];
        if (c)
            Core::strconv(std::string(c), desc_->outputAlphabetName);
        else
            desc_->outputAlphabetName.clear();
    }
    ...
    void boundariesFile(const Core::XmlAttributes atts) {
        ...
        if (c)
            Core::strconv(std::string(c), desc_->boundariesFile.file);
        else
            desc_->boundariesFile.file.clear();
...
void FlfReader::addStructure(StaticLattice& l, const Fsa::StaticAutomaton& f) {
    ...
    Lexicon::AlphabetMap& mapOutputLabel = (context_->fsaType() == Fsa::TypeAcceptor) ? *context_->inputAlphabetMap() : *context_->outputAlphabetMap();
    for (Fsa::StateId fsaSid = 0; fsaSid <= f.maxStateId(); ++fsaSid) {
        ...
        if (fsaSp) {
            State* sp = new State(fsaSp->id(), fsaSp->tags(),
                                  fsaSp->isFinal() ? semiring->clone(semiring->defaultWeight()) : semiring->one());
            for (Fsa::State::const_iterator fsaA = fsaSp->begin(); fsaA != fsaSp->end(); ++fsaA) {
                sp->newArc(fsaA->target(),
...
FlfDescriptorRef FlfWriter::buildDescriptor(
    ...
    desc->inputAlphabetFile = FsaDescriptor(
            "bin", Core::form("%s-input-alphabet.binfsa%s", basename.c_str(), suffix.c_str()));
    if (f->type() != Fsa::TypeAcceptor) {
        ...
        desc->outputAlphabetFile = FsaDescriptor(
                "bin", Core::form("%s-output-alphabet.binfsa%s", basename.c_str(), suffix.c_str()));
    }
    else
        desc->hasOutputAlphabet = false;
    if (f->getBoundaries()->valid()) {
        desc->hasBoundaries  = true;
        desc->boundariesFile = BoundariesDescriptor(Core::form("%s.binwb%s", basename.c_str(), suffix.c_str()));
    }
    else
        desc->hasBoundaries = false;
    ...
    if (desc->scoreFiles.empty()) {
        ...
        desc->structureFile.file   = Core::form("%s.binfsa%s", basename.c_str(), suffix.c_str());
    }
    else {
...
bool FlfWriter::writeDescriptor(FlfDescriptorRef   desc,
    ...
    xml << Core::XmlOpen("head")
        ...
        << Core::XmlClose("head");
    xml << Core::XmlEmpty("structure") + Core::XmlAttribute("format", desc->structureFile.format) + Core::XmlAttribute("file", desc->structureFile.file);
    if (desc->hasInputAlphabet)
        xml << Core::XmlEmpty((desc->hasOutputAlphabet ? "input-alphabet" : "alphabet")) + Core::XmlAttribute("name", desc->inputAlphabetName) + Core::XmlAttribute("format", desc->inputAlphabetFile.format) + Core::XmlAttribute("file", desc->inputAlphabetFile.file);
    if (desc->hasOutputAlphabet)
        xml << Core::XmlEmpty("output-alphabet") + Core::XmlAttribute("name", desc->outputAlphabetName) + Core::XmlAttribute("format", desc->outputAlphabetFile.format) + Core::XmlAttribute("file", desc->outputAlphabetFile.file);
    if (desc->hasBoundaries)
        xml << Core::XmlEmpty("boundaries") + Core::XmlAttribute("format", "bin") + Core::XmlAttribute("file", desc->boundariesFile.file);
    xml << Core::XmlOpen("scores") + Core::XmlAttribute("n", desc->scoreFiles.size());
    ...
    for (ScoreId id = 0; id < desc->scoreFiles.size(); ++id, ++keyIt, ++scaleIt, ++fsaIt) {
        xml << Core::XmlOpen("dim") + Core::XmlAttribute("id", id);
        if (!keyIt->empty())
            xml << Core::XmlFull("name", *keyIt);
        if ((*scaleIt != Semiring::UndefinedScale) && (*scaleIt != Semiring::DefaultScale))
            xml << Core::XmlFull("scale", *scaleIt);
        if (*fsaIt)
            xml << Core::XmlEmpty("fsa") + Core::XmlAttribute("format", fsaIt->format) + Core::XmlAttribute("file", fsaIt->file);
...
bool FlfWriter::write(ConstLatticeRef    f,
    ...
    for (KeyList::const_iterator itKey = desc->keys.begin(), endKey = desc->keys.end();
         itKey != endKey; ++itKey, ++itScoreFile)
        if (*itScoreFile) {
...
FlfArchiveReader::FlfArchiveReader(
    ...
    if (semiring()) {
        ...
        discardFlfHeader      = !(paramSuffix(select("boundaries")).empty() && paramSuffix(select("structure")).empty());
