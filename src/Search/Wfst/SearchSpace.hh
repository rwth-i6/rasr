...
public:
    ...
    void setPruningLimit(u32 limit) { pruningLimit_ = limit; }
    void setPurgeInterval(u32 interval) { purgeInterval_ = interval; }
    void setPruningBins(u32 bins) { stateHistogram_.setBins(bins); }
    void setInitialEpsilonPruning(bool prune) { initialEpsPruning_ = prune; }
    void setEpsilonPruning(bool prune) { epsilonArcPruning_ = prune; }
    void setProspectivePruning(bool prune) { prospectivePruning_ = prune; }
    void setLatticePruning(Score threshold) { latticePruning_ = threshold; }
    void setMergeEpsilonPaths(bool merge) { mergeEpsPaths_ = merge; }
    void setCreateLattice(bool create, LatticeTraceRecorder::LatticeType type) {
        createLattice_ = create;
        latticeType_ = type;
        // book_.setCreateLattice(create);
    }
    void setMergeSilenceLatticeArcs(bool merge) { mergeSilenceArcs_ = merge; }
    ...
    void setIgnoreLastOutput(bool ignore) { ignoreLastOutput_ = ignore; }
    void setStateSequences(const StateSequenceList *list) { stateSequences_ = list; }
    void setWeightScale(f32 scale) { weightScale_ = scale; }
    ...
    u32 nActiveHyps() const { return nActiveHmmStateHyps_; }
...
public:
    struct MemoryUsage
    {
        ...
        u32 sum() const { return bookkeeping + stateSequences + states + arcs + epsilonArcs +
                            stateHyps + arcHyps + hmmStateHyps; }
    };
...
protected:
    ...
    struct StateToHypHash { size_t operator()(const StateToHypElement &e) const { return e.first; } };
    ...
    // typedef std::unordered_map<StateIndex, StateHypIndex> StateToHypMap;
...
public:
    template<class H> static bool isActiveHyp(const H &h) {
