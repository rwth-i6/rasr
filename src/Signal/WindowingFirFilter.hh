...
public:
     ...
     *  if no output signal is extended by M_ samples at the beginning and at the end.
     */
    void setRemoveDelay(bool remove) {
        removeDelay_ = remove;
    }
    bool removeDelay() const {
        return removeDelay_;
    }
};
...
public:
    ...
    virtual Flow::Data* clone() const {
        return new WindowingFirFilterParameter(*this);
    };
    ...
    std::vector<std::pair<f64, f64>>& getCutOff() {
        return cut_off_;
    };
};
...
public:
    static std::string filterName() {
        return "signal-windowing-fir-filter";
    }
