...
private:
    ...
    public:
        Block()
                : objects_(0), compressedObjects_(0), size_(0), allocated_(0), compressedSize_(0) {}
        Block(const Block& b) {
            objects_             = b.objects_;
            b.objects_           = 0;
...
public:
    CompressedMemoryManager(BlockNumber nBlocks = 100)
            : blocksThatAreNotFull_(objectsPerBlock) {
        if (nBlocks <= 0)
            nBlocks = 1;
    ...
    Pointer allocate(size_t n = 1) {
        ...
        if (n < objectsPerBlock) {
            ...
            if (blocks_[b].allocatable() > 0)
                blocksThatAreNotFull_[n].push_front(b);
        }
        else {
            b = newBlock();
            o = blocks_[b].allocate(n);
