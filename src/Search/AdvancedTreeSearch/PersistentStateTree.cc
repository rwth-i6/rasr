...
struct ConvertTree
{
  ...
  ConvertTree( const Search::StateTree* _tree, HMMStateNetwork& _subtrees ) : tree( _tree ), subtrees( _subtrees ), masterTreeIndex( subtrees.allocateTree() ), lostNodeIndices( 0 ) {
  }
...
bool PersistentStateTree::read( int transformation ) {
  ...
  {
    Core::Application::us()->log() << "failed reading state network because of transformation mismatch: " << storedTransformation << " vs requested " << transformation;
    return false;
  }
  ...
  for( StateId node = 1; node < structure.stateCount(); ++node )
  {
    ...
    for( HMMStateNetwork::SuccessorIterator target = structure.successors(node); target; ++target )
      if(target.isLabel() && exits[target.label()].pronunciation != Bliss::LemmaPronunciation::invalidId )
        os << "\\n" << lexicon_->lemmaPronunciation( exits[target.label()].pronunciation )->lemma()->preferredOrthographicForm() << Core::form( " tr=%d", exits[target.label()].transitState );
