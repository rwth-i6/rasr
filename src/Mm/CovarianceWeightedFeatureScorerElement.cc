...
void CovarianceWeightedFeatureScorerElement::calculateInverseSquareRootDiagonal(const std::vector<VarianceType>& diagonal) {
...
void CovarianceWeightedFeatureScorerElement::calculateNormalizationFactor(const std::vector<VarianceType>& diagonal) {
    logNormalizationFactor_ = gaussLogNormFactor(diagonal.begin(), diagonal.end());
}

void CovarianceWeightedFeatureScorerElement::calculateNormalizationFactor(const std::vector<VarianceType>& diagonal, const std::vector<f64>& weights) {
