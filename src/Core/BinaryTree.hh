...
private:
    void checkId(Id id) const {
        require(idToNode.find(id) != idToNode.end()); /* no such id */
    }
...
public:
    ...
    const LeafList& leafList() const {
        return leafList_;
    }
    Id leafNumber(const LeafList::const_iterator p) const {
        return (*p)->leafNumber_;
    }
