...
ShiftAdaptorViterbiEstimator::ShiftAdaptorViterbiEstimator(const Core::Configuration&          c,
...
void ShiftAdaptorViterbiEstimator::init() {
    ...
    while (!stack.empty()) {
        Core::BinaryTree::Id id = stack.top();
        stack.pop();
        shift_[id] = Matrix();
...
void ShiftAdaptorViterbiEstimator::accumulate(const Speech::Alignment&                         alignment,
    ...
    for (size_t item = 0; item < alignment.size(); ++item) {
        if (alignment[item].weight < 0.5)
            continue;
        MixtureIndex mixture = unadaptedAdaptor->acousticModel()->emissionIndex(alignment[item].emission);
        size_t       density = unadaptedFeatureScorer->getAssigningScorer(featureSequence[item])->bestDensity(mixture);

        // Extract the mean vector
        DensityIndex          densityIndex = unadaptedAdaptor->mixtureSet()->mixture(mixture)->densityIndex(density);
        ...
        for (u32 d = 0; d < dimension_; d++) {
            betaAccumulators_[(*leafIndex_)[mixture]][d] += 1.0 / cov.diagonal()[d];
            shiftAccumulators_[(*leafIndex_)[mixture]][d] += ((*featureSequence[item]->mainStream())[d] - mean[d]) / cov.diagonal()[d];
        }
    }
}
...
Core::Ref<Adaptor> ShiftAdaptorViterbiEstimator::adaptor(void) {
    ...
    for (std::set<MixtureIndex>::const_iterator p = silenceMixtures_.begin(); p != silenceMixtures_.end(); ++p) {
        ...
        if (countAccumulators_[silenceLeaf] > minSilenceObservations_) {
            ...
            for (u32 d = 0; d < dimension_; d++)
                adaptationMatrices[silenceId][0][d] = shiftAccumulators_[silenceLeaf][d] / betaAccumulators_[silenceLeaf][d];
        }
        else {
...
#endif
        if (adaptationDumpChannel_.isOpen()) {
            adaptationDumpChannel_ << Core::XmlEmpty("shift") + Core::XmlAttribute("shift-id", mixture) + Core::XmlAttribute("shift-id", adaptationMatrixIndex[mixture]);
        }
    }
    if (adaptationDumpChannel_.isOpen()) {
...
FullAdaptorViterbiEstimator::FullAdaptorViterbiEstimator(const Core::Configuration&          c,
                                                         Core::Ref<const Mm::MixtureSet>     m,
                                                         const Core::Ref<Am::AdaptationTree> adaptationTree)
        : AdaptorEstimator(c, m, adaptationTree) {
    init();
...
FullAdaptorViterbiEstimator::ZAccumulator::ZAccumulator()
        : AccumulatorBase() {}

FullAdaptorViterbiEstimator::ZAccumulator::ZAccumulator(ComponentIndex dim)
        : AccumulatorBase() {
...
FullAdaptorViterbiEstimator::GAccumulator::GAccumulator()
        : AccumulatorBase() {}

FullAdaptorViterbiEstimator::GAccumulator::GAccumulator(ComponentIndex dim)
        : AccumulatorBase() {
...
void FullAdaptorViterbiEstimator::accumulate(const Speech::Alignment&                         alignment,
                                             const std::vector<Core::Ref<const Mm::Feature>>& featureSequence,
                                             Core::Ref<const Am::MixtureSetAdaptor>           unadaptedAdaptor,
                                             Core::Ref<const Mm::AssigningFeatureScorer>      unadaptedFeatureScorer) {
    if (alignment.size() != featureSequence.size())
        return;
    require(alignment.size() == featureSequence.size());
    for (size_t item = 0; item < alignment.size(); ++item) {
        MixtureIndex mixture =
...
#endif
        if (adaptationDumpChannel_.isOpen()) {
            adaptationDumpChannel_ << Core::XmlEmpty("mixture") + Core::XmlAttribute("mixture-id", mixture) + Core::XmlAttribute("matrix-id", adaptationMatrixIndex[mixture]);
        }
    }
    if (adaptationDumpChannel_.isOpen()) {
