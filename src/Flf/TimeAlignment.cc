...
public:
    ...
    void dumpStatistics() {
        if (statisticsChannel_.isOpen()) {
            statisticsChannel_ << Core::XmlOpen("statistics") + Core::XmlAttribute("component", this->name()) + Core::XmlAttribute("name", "alignment");
            statisticsChannel_ << Core::XmlFull("count", statistics_.n) + Core::XmlAttribute("name", "all");
            statisticsChannel_ << Core::XmlFull("count", statistics_.nIntersections) + Core::XmlAttribute("name", "intersections");
            statisticsChannel_ << Core::XmlFull("count", statistics_.nFrameAlignments) + Core::XmlAttribute("name", "fCN alignments");
...
protected:
    ConstLatticeRef align(ConstLatticeRef lHyp, ConstPosteriorCnRef cnRef) {
        verify((lHyp->type() == Fsa::TypeAcceptor) && (Lexicon::us()->alphabetId(lHyp->getInputAlphabet()) == Lexicon::LemmaAlphabetId));
        ...
        verify(((*lastScoresPtr)[hyp.size() - 1] < Semiring::Max) || ((*lastScoresPtr)[hyp.size()] < Semiring::Max));
        ...
        u32 j     = ((*lastScoresPtr)[hyp.size() - 1] < (*lastScoresPtr)[hyp.size()]) ? hyp.size() - 2 : hyp.size() - 1;
        u32 lastJ = j;
