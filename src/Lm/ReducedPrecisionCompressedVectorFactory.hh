...
public:
    ReducedBitsFloatVector(unsigned drop_bits) : drop_bits_(drop_bits), bits_per_val_(sizeof(float) * 8 - drop_bits_) {
    }
...
public:
    ...
    ReducedPrecisionCompressedVectorFactory(Core::Configuration const& config) : Precursor(config), drop_bits_(paramDropBits(config)) {}
