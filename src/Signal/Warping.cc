    ...
    for(index = previousIndex + 1; index < inputSize; ++ index) {
        ...
        if (warpingIndex != previousWarpingIndex) {
            ...
            for(u32 w = previousWarpingIndex + 1; w < warpingIndex; ++ w) {
                f64 relativePosition = f32(w - previousWarpingIndex) /
                    f32(warpingIndex - previousWarpingIndex);

                inverseWarpingFunction_[w] =
                    createInterpolator(interpolationType, previousIndex, index, relativePosition);
                                        ...
                                        size_t inputSize, InterpolationType interpolationType)
{
    ...
    for(u32 warpingIndex = 0; ; ++ warpingIndex) {
        ...
        Warping::InverseItem *interpolator =
            createInterpolator(interpolationType, leftOfIndex, leftOfIndex + 1, relativePosition);
        if (interpolator != 0)
...
Warping::InverseItem* Warping::createInterpolator(InterpolationType type,
