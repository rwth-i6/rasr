...
bool SymmetricEigenvalueProblem::solveWithExpertOrRelativelyRobustDriver(const Math::Matrix<ValueType>& a,
                                                                         Math::Vector<ValueType>&       eigenvalues,
                                                                         Math::Matrix<ValueType>&       eigenvectors) const {
    Matrix<double> lapackA;
    copy(a, lapackA);
    int            nEigenvalues;
...
bool SymmetricEigenvalueProblem::solveWithDivideAndConquerDriver(const Math::Matrix<ValueType>& a,
                                                                 Math::Vector<ValueType>&       eigenvalues,
                                                                 Math::Matrix<ValueType>&       eigenvectors) const {
    Matrix<double> lapackA;
    copy(a, lapackA);
...
bool GenEigenProblemWithSchurDecomposition::solveSymmetric(const Math::Matrix<ValueType>& a,
...
bool GenEigenProblemWithSchurDecomposition::solve(const Math::Matrix<ValueType>& a, const Math::Matrix<ValueType>& b,
...
bool GenEigenProblemWithSchurDecomposition::solveWithExpertDriver(const Math::Matrix<ValueType>& a, const Math::Matrix<ValueType>& b,
                                                                  Math::Vector<std::complex<ValueType>>& eigenvalues,
                                                                  Math::Matrix<std::complex<ValueType>>& leftEigenvectors,
                                                                  Math::Matrix<std::complex<ValueType>>& rightEigenvectors) const {
    Matrix<double> lapackA;
    copy(a, lapackA);
    Matrix<double> lapackB;
    copy(b, lapackB);
...
bool GenEigenProblemWithSchurDecomposition::dggevx(Matrix<double>& a, Matrix<double>& b,
...
double GenEigenProblemWithSchurDecomposition::maxPerMinScaling(const Vector<double>& scales, int ilo, int ihi) const {
    double result = (*std::max_element(scales.begin() + (ilo - 1), scales.begin() + ihi) /
                     *std::min_element(scales.begin() + (ilo - 1), scales.begin() + ihi));
    return result;
}

void GenEigenProblemWithSchurDecomposition::copyAlphas(const Vector<double>& realAlphas, const Vector<double>& imaginaryAlphas,
...
void GenEigenProblemWithSchurDecomposition::copyEigenvalues(const Math::Vector<std::complex<ValueType>>& alphas,
...
std::complex<Math::EigenvalueProblem::ValueType> GenEigenProblemWithSchurDecomposition::calculateEigenvalue(std::complex<ValueType> alpha,
                                                                                                            ValueType               alphaMinimum,
                                                                                                            ValueType               beta,
                                                                                                            ValueType               betaMinimum,
...
void GenEigenProblemWithSchurDecomposition::copyEigenvectors(const Math::Vector<std::complex<ValueType>>& alphas,
                                                             const Matrix<ValueType>&                     v,
...
void GenEigenProblemWithSchurDecomposition::writeConditionNumbers(Core::XmlWriter&                             os,
    ...
    double perturbance = Core::Type<double>::epsilon * sqrt(oneNormOfBalancedA * oneNormOfBalancedA + oneNormOfBalancedB * oneNormOfBalancedB);
    ...
    for (; alpha != alphas.end(); ++alpha, ++beta, ++eigenvalue, ++e, ++v) {
        os << Core::XmlOpen("component")
           ...
           << Core::XmlFull("left-eigenvector-asymptotic-error-bound", asin(leftMaxPerMinScaling * sin(perturbance / *v)))
           << Core::XmlFull("right-eigenvector-asymptotic-error-bound", asin(rightMaxPerMinScaling * sin(perturbance / *v)))
...
//--------------------------------------------------------------------------------------------------------
GenSymmetricDefiniteEigenProblem::GenSymmetricDefiniteEigenProblem(const Core::Configuration& c)
        : Core::Component(c),
          Math::GeneralizedEigenvalueProblem(c),
          Math::Lapack::EigenvalueProblem(c) {}

bool GenSymmetricDefiniteEigenProblem::solve(const Math::Matrix<ValueType>& a,
                                             const Math::Matrix<ValueType>& b,
                                             Math::Vector<ValueType>&       eigenvalues,
                                             Math::Matrix<ValueType>&       eigenvectors) {
    switch (driverType()) {
...
bool GenSymmetricDefiniteEigenProblem::solveWithExpertDriver(const Math::Matrix<ValueType>& a,
                                                             const Math::Matrix<ValueType>& b,
                                                             Math::Vector<ValueType>&       eigenvalues,
                                                             Math::Matrix<ValueType>&       eigenvectors) const {
    Matrix<double> lapackA;
...
bool GenSymmetricDefiniteEigenProblem::dsygvx(Matrix<double>& a,
                                              Matrix<double>& b,
                                              int&            nEigenvalues,
...
bool GenSymmetricDefiniteEigenProblem::solveWithDivideAndConquerDriver(const Math::Matrix<ValueType>& a,
                                                                       const Math::Matrix<ValueType>& b,
                                                                       Math::Vector<ValueType>&       eigenvalues,
                                                                       Math::Matrix<ValueType>&       eigenvectors) const {
    Matrix<double> lapackA;
...
bool GenSymmetricDefiniteEigenProblem::dsygvd(Matrix<double>& a,
                                              Matrix<double>& b,
