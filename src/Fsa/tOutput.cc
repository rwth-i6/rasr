...
bool write(const Resources<_Automaton>& resources, typename _Automaton::ConstRef f,
    ...
    typename Resources<_Automaton>::Format* formatter = (format.empty()) ? resources.getDefaultFormat() : resources.getFormat(format);
    if (!formatter) {
...
bool writeAtt(const Resources<_Automaton>& resources, typename _Automaton::ConstRef f,
    ...
    if (o) {
        ...
        if (progress)
            p = new Core::ProgressIndicator("writing", "states");
...
public:
    ...
    void discoverState(_ConstStateRef sp) {
        ...
        if (!(o_ << idAndTags))
            return;
        if (sp->isFinal())
            Precursor::fsa_->semiring()->write(sp->weight_, o_);
        if (!(o_ << sp->nArcs()))
            return;
        for (typename _State::const_iterator a = sp->begin(); a != sp->end(); ++a) {
            if (!(o_ << a->target()))
                return;
            if (!Precursor::fsa_->semiring()->write(a->weight(), o_))
                return;
            if (!(o_ << a->input()))
                return;
            if (Precursor::fsa_->type() == Fsa::TypeTransducer)
...
bool writeBinary(const Resources<_Automaton>& resources, typename _Automaton::ConstRef f,
    ...
    if (what & Fsa::storeInputAlphabet)
        f->getInputAlphabet()->write(bo);
    if (what & Fsa::storeOutputAlphabet)
        f->getOutputAlphabet()->write(bo);
    if (what & Fsa::storeStates) {
    ...
    void discoverState(_ConstStateRef sp) {
        ...
        for (typename _State::const_iterator a = sp->begin(); a != sp->end(); ++a) {
            ...
            if (a->input() != Fsa::Epsilon) {
                ...
                if (tmp.empty())
                    o_ << Core::XmlFull("in", a->input());
                else
                    o_ << Core::XmlFull("in", tmp);
            }
            if (Precursor::fsa_->type() == Fsa::TypeTransducer)
                if (a->output() != Fsa::Epsilon) {
                    std::string tmp = Precursor::fsa_->getOutputAlphabet()->specialSymbol(a->output());
                    if (tmp.empty())
                        o_ << Core::XmlFull("out", a->output());
                    else
                        o_ << Core::XmlFull("out", tmp);
                }
            if (!Precursor::fsa_->semiring()->isDefault(a->weight()))
...
bool writeXml(const Resources<_Automaton>& resources, typename _Automaton::ConstRef f,
    ...
    if (f->initialStateId() != Fsa::InvalidStateId) {
        xo << Core::XmlOpen("fsa") + Core::XmlAttribute("type", Fsa::TypeChoice[f->type()]) + Core::XmlAttribute("semiring", f->semiring()->name()) + Core::XmlAttribute("initial", f->initialStateId()) << "\n";
    }
    else {
        xo << Core::XmlOpen("fsa") + Core::XmlAttribute("type", Fsa::TypeChoice[f->type()]) + Core::XmlAttribute("semiring", f->semiring()->name()) << "\n";
    }
