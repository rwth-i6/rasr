...
struct VectorRangeCheckOnPolicy {
    template<class T>
    void checkRange(const T& v, size_t n) {
...
Vector<T, P> real(const Vector<std::complex<T>, P>& v) {
...
Core::XmlWriter& operator<<(Core::XmlWriter& os, const Vector<T, P>& v) {
    os << Core::XmlOpen(std::string("vector-") + Core::Type<T>::name) + Core::XmlAttribute("size", v.size());
...
inline bool operator==(const Vector<T1, P1>& v1, const Vector<T2, P2>& v2) {
    if (v1.size() != v2.size())
        return false;
...
template<typename T, class P = VectorRangeCheckOffPolicy>
class ReferenceCountedVector : public Core::ReferenceCounted, public Vector<T, P> {
public:
