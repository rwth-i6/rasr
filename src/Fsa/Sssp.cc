...
#if 0
        Accumulator * collector = (sp->isFinal()) ?
            sr->getCollector(sp->weight_) :
            sr->getCollector();
        for (State::const_iterator a = sp->begin(); a != sp->end(); ++a)
            collector->feed(sr->extend(Weight(potentials_[a->target()]), a->weight()));
        potentials_[s] = f32(collector->get());
        delete collector;
#else
