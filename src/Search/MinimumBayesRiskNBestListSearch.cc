...
namespace Search
{
    ...
    //const Core::ParameterBool MinimumBayesRiskNBestListNaiveSearch::paramCreateRiskNbestlist
    //( "create-risk-nbestlist", "shall nbestlist with risk as scores be created", false );
    ...
    void MinimumBayesRiskNBestListNaiveSearch::setDistanceOneCriterion(
        const Fsa::Weight &distanceOne, const Fsa::Weight &maxDistanceOne)
    {
        distanceOneCriterion_ =
            Fsa::LogSemiring->compare(
                Fsa::LogSemiring->collect(
                    distanceOne,
                    Fsa::LogSemiring->extend(
                        Fsa::Weight(-std::log(2.0)),
                        mapProbability_)),
                Fsa::LogSemiring->collect(
                    Fsa::LogSemiring->one(),
                    maxDistanceOne)) <= 0;
