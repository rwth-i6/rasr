...
void SearchSpaceBase::setTransitionModel(Core::Ref<const Am::AcousticModel> acousticModel)
{
    if (acousticModel->nStateTransitions() > Core::Type<TransitionModelIndex>::max)
        Core::Application::us()->error(
                "maximum number of transition models %d exceeded: %d",
    ...
    entryForwardScore =
            (*transitionModels_[Am::TransitionModel::entryM1])[Am::StateTransitionModel::forward];
    entrySkipScore =
            (*transitionModels_[Am::TransitionModel::entryM1])[Am::StateTransitionModel::skip];
}
...
public:
    ...
    virtual ~SearchSpace() { delete network_; }
    ...
    u32 nStates() const { return network_->nStates(); }
    u32 nArcs() const { return network_->nArcs(); }
    u32 nEpsilonArcs() const { return network_->nEpsilonArcs(); }
...
private:
    ...
    public:
        StateHypIterator (const StateHypotheses &states)
            : iter_(states.begin()), end_(states.end()) { update(); }
        bool done() const { return iter_ == end_; }
        void next() { ++iter_; update(); }
        const StateHyp& value() const { return *iter_; }
        bool hasIncoming() const { return incoming_; }
    private:
        void update() {
            incoming_ = done() ? false :
                (isActiveHyp(iter_->incoming[0]) ||
                 (UseSkips && isActiveHyp(iter_->incoming[1])));
        }
...
private:
    ...
    void activateOrUpdateHmmState(StateHypIndex hmmState, Score score,
    ...
    void expandHmmState(const ArcHyp &arcHyp, u32 hmmState, Score score,
                        const TraceRef &trace);
    ...
    void findEpsilonReachable(const StateHyp &stateHyp,
                              Fsa::Stack<EpsilonArcHyp> *hyps) const;
    ...
    void expandTransition(const ArcHyp &arcHyp, const StateHypIndex prevArcHypEnd,
...
public:
    MemoryUsage memoryUsage() {
        ...
        m.hmmStateHyps = (hmmStateHypotheses_.capacity() +
                newHmmStateHypotheses_.capacity()) * sizeof(HmmStateHyp);
        return m;
    }
            ...
            for (u32 i = 0; i < NumIncomingHyps; ++i) {
                if (isActiveHyp(hyp.incoming[i])) {
                    ...
                    if (update || createLattice_) {
                        TraceRef newTrace = book_->addTrace(
                                best.trace, hyp.incoming[i].trace, Epsilon, 0, time_,
        ...
        for (u32 i = 0; i < NumIncomingHyps; ++i) {
            ...
            if (isActiveHyp(hyp.incoming[i])) {
                if (hyp.incoming[i].score < best.score) {
                    ...
                    best.trace = book_->addTrace(
                            best.trace, hyp.incoming[i].trace, Epsilon, 0,
...
{
    TraceRef t = book_->addTrace(
            TraceRecorder::InvalidTraceRef, TraceRecorder::InvalidTraceRef,
 ...
 */
template<class N, bool S>
inline void SearchSpace<N,S>::activateOrUpdateHmmState(
        StateHypIndex hmmState, Score score, const TraceRef &trace)
{
    ...
    if (hmmStateHypIndex < currentHmmStateHypSize_) {
        ...
        // verify_(hmmStateHyp.state == hmmState);
        ...
        const StateHyp &stateHyp, bool expandArcs, ArcHypIndex *arcHypIndex)
{
    ...
    for (ArcIter aiter(network_, stateHyp.state); !aiter.done(); aiter.next(), ++arcIndex) {
        ...
        if (incoming) {
            ...
            if (hmm->isInitial()) score += entryForwardScore;
        ...
        if (UseSkips && incomingSkip && nStates > 1) {
            ...
            if (hmm->isInitial()) score += entrySkipScore;
        ...
        if (expandArcs) {
            if (*arcHypIndex < activeArcs_.size()) {
                ...
                if (arcHyp.arc == arcIndex && arcHyp.state == stateHyp.state) {
                    ...
                    verify_(arcHyp.hmm == hmm && arcHyp.output == arc.olabel &&
                            arcHyp.target == arc.nextstate);
        ...
        if (currentHmmStateHypSize_ !=  currentHmmStateHypBase_) {
            ...
            addArcHyp(currentHmmStateHypSize_, stateHyp.state, arc.nextstate, hmm,
                    arc.olabel, arcWeight, arcIndex);
        }
    }
}
...
template<class N, bool S>
inline void SearchSpace<N,S>::updateContextStateScore(
        StateToScoreMap &scores, StateIndex state, Score score) const
{
 ...
 */
template<class N, bool S>
Score SearchSpace<N,S>::quantileStateScore(
        HmmStateHypotheses::const_iterator begin, HmmStateHypotheses::const_iterator end,
        Score minScore, Score maxScore, u32 nHyps)
{
...
SearchSpace<N,S>::getStateHyp(StateIndex stateIndex)
{
    ...
    if (i == StateToHypMap::InvalidCursor) {
        stateToHyp_.insert(
                StateToHypElement(stateIndex, activeStates_.size()));
 ...
 */
template<class N, bool S>
inline bool SearchSpace<N,S>::activateOrUpdateIncomingHyp(
        IncomingHyp &incoming, TraceRef trace, Label output, const StateSequence *input,
        TimeframeIndex traceTime, Score score, Score arcScore, bool wordEnd)
{
    const bool update = score < incoming.score;
    if (update || createLattice_) {
        TraceRef newTrace = book_->addTrace(
                incoming.trace, trace, output, input, traceTime,
        ...
        const ArcHyp &arcHyp, const StateHypIndex stateHypsBegin, const Score threshold)
{
    ...
    if (UseSkips) {
        ...
        if (!hasLastState[0]) return;
    }
    StateHyp *nextStateHyp = 0;
    for (u32 i = 0; i < NumIncomingHyps; ++i) {
        if (!hasLastState[i]) continue;
...
template<class N, bool S>
inline typename SearchSpace<N,S>::StateHyp*
SearchSpace<N,S>::expandTransHyp(
        const HmmStateHyp &hmmStateHyp, const ArcHyp &arcHyp,
        ...
        const bool isFinalPhone, const bool isWordEnd, const bool isLastState, StateHyp *nextStateHyp)
{
    ...
    if (isLastState) {
        ...
        if (score < threshold) {
            if (!nextStateHyp) nextStateHyp = getStateHyp(arcHyp.target);
        ...
        if ((!isFinalPhone || isLastState) && (skipTdp < Core::Type<Score>::max)) {
            ...
            if (score < threshold) {
                ...
                if (activateOrUpdateIncomingHyp(nextStateHyp->incoming[incomingTarget],
                        hmmStateHyp.trace, arcHyp.output, arcHyp.hmm, time_, score, arcHyp.score, isFinalPhone)) {
                    if (outputIsWordEnd_)
                        stateTransitionModels_[arcHyp.target] = hmm.state(hmmState).transition_;
                    if (wordEndPruning_)
                        setWordEndHyp(arcHyp.target, isWordEnd);
...
void SearchSpace<N,S>::findEpsilonReachable(const StateHyp &stateHyp, Fsa::Stack<EpsilonArcHyp> *hyps) const
{
    for (EpsilonArcIterator aiter(network_, stateHyp.state); !aiter.done(); aiter.next()) {
        ...
        hyps->push(EpsilonArcHyp(arc.nextstate, arc.olabel,
                Network::arcWeight(arc, weightScale_), stateHyp.incoming));
    }
}
...
void SearchSpace<N,S>::expandEpsilonArcs(Score threshold, bool anticipatedPruning)
{
    ...
    if (MergePaths) visitedStates = new StateToScoreMap();
            ...
            for (u32 i = 0; i < NumIncomingHyps; ++i) {
                if (hasIncoming[i]) {
                    ...
                    if (!visited && score < currentTreshold) {
                        ...
                        if (MergePaths) (*visitedStates)[arc.target] = score;
                    }
                }
            }
    ...
    if (MergePaths) delete visitedStates;
}

template<class N, bool S>
inline void SearchSpace<N,S>::findEpsilonPaths(
        const EpsilonArcHyp &arc, const StateToScoreMap *visitedStates,
        Score threshold, const bool mergePaths,
        Fsa::Stack<EpsilonArcHyp> *statesToExplore) const
{
    for (EpsilonArcIterator aiter(network_, arc.target); !aiter.done(); aiter.next()) {
        ...
        if (!visited && score < threshold) {
            ...
            if (nextArc.olabel != Epsilon && arc.output != Epsilon) {
                ...
                for (u32 i = 0; i < NumIncomingHyps; ++i) {
                    statesToExplore->back().trace[i] = book_->addTrace(
                            InvalidTraceRef, arc.trace[i], arc.output, 0,
...
void SearchSpace<N,S>::feed(const Mm::FeatureScorer::Scorer &scorer)
{
    ...
    if (!twoPassPruning_ && currentHmmStateHypSize_ > pruningLimit_) {
        histogramTreshold = quantileStateScore(
                newHmmStateHypotheses_.begin(), newHmmStateHypotheses_.begin() + currentHmmStateHypSize_,
    ...
    if (twoPassPruning_ && hmmStateHypotheses_.size() > pruningLimit_) {
        histogramTreshold = quantileStateScore(
                hmmStateHypotheses_.begin(), hmmStateHypotheses_.end(), 0, threshold, pruningLimit_);
...
SearchSpaceBase* SearchSpaceBase::createSearchSpace(
        bool allowSkips, const Core::Configuration &c) {
