...
public:
    std::pair<ConstConfusionNetworkRef, ConstLatticeRef> getNormalizedCn(ScoreId confidenceId, bool mapping = false) const {
        ...
        Collector*      sumCol   = createCollector(Fsa::SemiringTypeLog);
        Collector*      colStart = createCollector(Fsa::SemiringTypeLog);
        Collector*      colEnd   = createCollector(Fsa::SemiringTypeLog);
        for (u32 i = 0; i < slotPtrs_.size(); ++i) {
...
private:
    ...
    struct SortArcIdByLabel {
        ...
        SortArcIdByLabel(const CenterFrameCn::ArcList& arcs)
                : arcs(arcs) {}
...
private:
    ...
    Id updateFrames(IdList::iterator beginFrameId, IdList::iterator& endFrameId) {
        ...
        Id                            bestFrameId = -1;
        Frame::Properties             bestProps;
        bestProps.nonEpsScore          = Core::Type<Score>::max;
        bestProps.nonEpsCenterDevScore = Core::Type<Score>::max;
        ...
        for (IdList::const_iterator itArcId = frame.arcIds.begin(), endArcId = frame.arcIds.end();
            ...
            if (arcId != -1) {
                ...
                for (IdList::const_iterator itDiscardFrameId = arc.frameIds.begin(), endDiscardFrameId = arc.frameIds.end();
                     itDiscardFrameId != endDiscardFrameId; ++itDiscardFrameId)
                    if (*itDiscardFrameId != frameId) {
                        Frame& discardFrame = frames_[*itDiscardFrameId];
                        // delete arc from frame
                        IdList::iterator itDiscardArcId = discardFrame.arcIds.begin();
                        for (; *itDiscardArcId != arcId; ++itDiscardArcId)
                            verify_(itDiscardArcId != discardFrame.arcIds.end());
                        *itDiscardArcId      = -1;
