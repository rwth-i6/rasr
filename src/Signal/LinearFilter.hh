...
public:
    ...
    void setB(const std::vector<T>& B_tilde) {
        B_tilde_ = B_tilde;
        u_.resize(B_tilde.size());
        for (u8 i = 0; i < u_.size(); i++)
            u_[i] = 0;
    }
    void setA(const std::vector<T>& A_tilde) {
        A_tilde_ = A_tilde;
        y_.resize(A_tilde.size());
        for (u8 j = 0; j < y_.size(); j++)
            y_[j] = 0;
    }
...
public:
    ...
    virtual Data* clone() const {
        return new LinearFilterParameter(*this);
    };

    virtual Core::XmlWriter& dump(Core::XmlWriter& o) const;

    std::vector<_float>& getB() {
        return B_tilde_;
    };
    std::vector<_float>& getA() {
        return A_tilde_;
    };
    std::vector<_float>& getY0() {
        return y0_;
    };
};
