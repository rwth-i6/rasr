...
#ifdef MODULE_ADAPT_ADVANCED
    Core::IoRef<Mm::AbstractAdaptationAccumulator>::registerClass
        <Mm::BandMllrEstimator>(config, adaptationTree_);
    Core::IoRef<Mm::AbstractAdaptationAccumulator>::registerClass
        <Mm::SemiTiedEstimator>(config, adaptationTree_);
#endif
}
...
#ifdef MODULE_ADAPT_ADVANCED
    case semiTiedMllr:
        currentAccumulator_ = new Accumulator(new Mm::SemiTiedEstimator(
                config, mixtureSet, adaptationTree_));
        break;
    case bandMllr:
        currentAccumulator_ = new Accumulator(new Mm::BandMllrEstimator(
                config, mixtureSet, adaptationTree_));
        break;
#endif
    default:
