...
void ConvertMixtureSetEstimator::setMixtureSet(Core::Ref<const MixtureSet> mixtureSet) {
    ...
    for (DensityIndex i = 0; i < mixtureSet->nDensities(); ++i) {
        required_cast(ConvertGaussDensityEstimator*, indexMaps.densityMap()[i].get())
                ->setDensity(mixtureSet->density(i));
    }

    verify(mixtureSet->nMeans() == indexMaps.meanMap().size());
    for (MeanIndex i = 0; i < mixtureSet->nMeans(); ++i) {
        required_cast(ConvertMeanEstimator*, indexMaps.meanMap()[i].get())
                ->setMean(mixtureSet->mean(i));
    }
    ...
    for (CovarianceIndex i = 0; i < mixtureSet->nCovariances(); ++i) {
        required_cast(ConvertCovarianceEstimator*, indexMaps.covarianceMap()[i].get())
                ->setCovariance(mixtureSet->covariance(i), meanSetMap);
    }
}
