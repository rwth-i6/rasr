...
private:
    ...
    u32       getPeakWidth(const std::vector<Amplitude>& v, const u32 peakPos, const Amplitude widthHeight) const;
    u32       getMountainWidth(const std::vector<Amplitude>& v, const u32 peakIndex, const Amplitude widthHeight) const;
    f32       getWidthConfidence(const std::vector<Amplitude>& v, const u32 peakIndex, const Amplitude peakValue, const Amplitude averageHeight) const;
    void      init();
...
public:
    void setContinuousMinPosition(const f32 continuousMinPosition) {
        continuousMinPosition_ = continuousMinPosition;
        needInit_              = true;
    }

    f32 continuousMinPosition() const {
        return continuousMinPosition_;
    }
    u32 minPosition();

    void setContinuousMaxPosition(const f32 continuousMaxPosition) {
        continuousMaxPosition_ = continuousMaxPosition;
        needInit_              = true;
    }

    f32 continuousMaxPosition() const {
        return continuousMaxPosition_;
    }
    u32 maxPosition();

    void setContinuousMaxWidth(const f32 continuousMaxWidth) {
        continuousMaxWidth_ = continuousMaxWidth;
        needInit_           = true;
    }

    void setContinuousHeightAverageWidth(const f32 continuousHeightAverageWidth) {
        continuousHeightAverageWidth_ = continuousHeightAverageWidth;
        needInit_                     = true;
    }

    void setWidthHeightFactor(const f32 widthHeightFactor) {
        widthHeightFactor_ = widthHeightFactor;
    }
    void setMountainWidth(const bool mountainWidth) {
        mountainWidth_ = mountainWidth;
    }

    void setSampleRate(const f64 sampleRate) {
