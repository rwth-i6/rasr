...
void Convolution::setSymmetricResponse(const std::vector<Data>& responsePositiveSide, u32 maximalSignalSize) {
    ...
    if (!responsePositiveSide.empty()) {
        std::copy(responsePositiveSide.begin(), responsePositiveSide.end(), responseSpectrum_.begin());
        std::copy(responsePositiveSide.begin() + 1, responsePositiveSide.end(), responseSpectrum_.rbegin());
    }
...
void Convolution::apply(std::vector<Data>& signal) {
    ...
    Math::transformAlternatingComplexToAlternatingComplex(responseSpectrum_.begin(),
                                                          responseSpectrum_.end(),
                                                          signalSpectrum_.begin(),
                                                          signalSpectrum_.begin(),
                                                          std::multiplies<std::complex<Data>>());
