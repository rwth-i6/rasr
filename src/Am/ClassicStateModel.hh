...
private:
    ...
    struct AllophonePtrEq {
        bool operator()(const Allophone* const a, const Allophone* const b) const {
            return *a == *b;
        }
    };
    typedef std::unordered_map<const Allophone*, Fsa::LabelId, AllophonePtrHash, AllophonePtrEq> AllophoneMap;
...
private:
    ...
    typedef std::unordered_set<Bliss::Phoneme::Id> PhonemeIdSet;
...
protected:
    ...
    std::pair<AllophoneMap::iterator, bool> insert(const Allophone& allo) const;
    std::pair<AllophoneMap::iterator, bool> createAndInsert(const std::vector<bool>& cd, const Bliss::Pronunciation& p, s32 b, s32 c, s32 e, s16 boundary) const;
...
public:
    ...
    bool operator==(const AllophoneState& alloState) const {
        return (allo_ == alloState.allo_) && (state_ == alloState.state_);
    }

    const Allophone* allophone() const {
        require(allo_);
        return allo_;
    }
    s16 state() const {
...
public:
    ...
    const Phonology& phonology() const {
        return *phonologyRef_;
    }
    ConstPhonologyRef getPhonology() const {
        return phonologyRef_;
    }

    const Bliss::PhonemeInventory& phonemeInventory() const {
        return *piRef_;
    }
    Core::Ref<const Bliss::PhonemeInventory> getPhonemeInventory() const {
        return piRef_;
    }

    const AllophoneAlphabet& allophoneAlphabet() const {
        return *allophoneAlphabetRef_;
    }
    ConstAllophoneAlphabetRef getAllophoneAlphabet() const {
        return allophoneAlphabetRef_;
    }

    const AllophoneStateAlphabet& allophoneStateAlphabet() const {
        return *allophoneStateAlphabetRef_;
    }
    ConstAllophoneStateAlphabetRef getAllophoneStateAlphabet() const {
        return allophoneStateAlphabetRef_;
    }
    AllophoneState allophoneState(const Allophone* allo, s16 state) const;

    const HmmTopologySet& hmmTopologySet() const {
        return *hmmTopologySetRef_;
    }
    HmmTopologySetRef getHmmTopologySet() const {
        return hmmTopologySetRef_;
    }
