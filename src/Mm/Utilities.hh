...
namespace Mm {
    ...
    public:
#if 0
        /** This is the legacy function used to quantize
         *  the features in the SIMD feature scorer.
         *  It has been replaced by the version below
         *  which does not have a systematic offset.
         */
        QuantizedType operator()(ValueType x) {
            static const int offset =
                ((int)Core::Type<QuantizedType>::max + (int)Core::Type<QuantizedType>::min) / (int)2 + 1;
            return clip((int)floor(x) + offset);
        }
#else
        QuantizedType operator()(ValueType x) {
            static const int offset =
