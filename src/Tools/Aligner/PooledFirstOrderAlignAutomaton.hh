 ...
 */
#ifndef POOLED_FIRST_ORDER_ALIGN_AUTOMATON
#define POOLED_FIRST_ORDER_ALIGN_AUTOMATON
...
namespace Fsa {
    ...
    public:
        ...
        PooledFirstOrderAlignAutomaton(Core::Configuration &config,
                                       const std::string& source,
                                       const std::string& target,
                                       const TransitionProbs& transitionProbs,
                                       const Translation::ConditionalLexiconRef lex,
...
//                      << std::endl;
            ...
            if (i < I && doVertical) { // when not at the TOP of lattice, make vertical movement
                if (j > 0) {
                    ...
                    f64 weight(lexicon_->getProb(0,x) * factorLexicon_);
                    if (transitionProbs_)
        ...
        virtual std::string describe() const { return std::string("pooledFirstOrderAlignAutomaton()"); }
    };
}
#endif
