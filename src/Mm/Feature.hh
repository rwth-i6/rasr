...
public:
    ...
    bool operator==(const Feature& r) const {
        for (size_t streamIndex = 0; streamIndex < nStreams(); ++streamIndex)
            if (*(*this)[streamIndex] != *r[streamIndex])
                return false;
        return true;
    }
    ...
    void setNumberOfStreams(size_t nStreams) {
        return streams_.resize(nStreams);
    }
 ...
 * Stream description is a set of attributes describing a feature stream.
