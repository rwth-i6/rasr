...
bool stateHasFailArc(typename _Automaton::ConstStateRef state) {
...
template<typename _ConstSemiringRef>
bool isTropicalSemiring(_ConstSemiringRef semiring) {
    return false;
}

template<>
bool isTropicalSemiring<Fsa::ConstSemiringRef>(Fsa::ConstSemiringRef semiring) {
    return (semiring == Fsa::TropicalSemiring);
}

template<typename _Weight>
std::string weight2str(const _Weight& w) {
...
template<>
std::string weight2str<Fsa::Weight64>(const Fsa::Weight64& w) {
    std::ostringstream buffer;
    buffer.precision(30);
    buffer << (f64)w;
    return buffer.str();
...
public:
    ...
    const Fsa::Stack<Fsa::StateId>& getProcessedStates() const {
        return processedStates_;
    }
    inline _Weight getPotential(Fsa::StateId s) const {
        return potential_[s];
    }
    inline _Weight getFailPotential(Fsa::StateId s) const {
        return failPotential_[s];
    }
    bool empty() const {
        return potential_.empty();
    }
...
private:
    ...
    struct AddArcTropical : AddArc {
        inline static _Weight failArc(const _Arc* arc, DirectHits& directHits, _Accumulator* collector, const bool firstLevel, _SsspBackward4SpecialSymbols* owner) {
            ...
            update     = owner->semiring_->extend(sssp4SpecialSymbolsHelper::convertWeight<_AutomataWeight, _Weight>(arc->weight()), failWeight);
    ...
    struct AddArcNegCollect : AddArc {
        ...
        inline static _Weight failArc(const _Arc* arc, DirectHits& directHits, _Accumulator* collector, const bool firstLevel, _SsspBackward4SpecialSymbols* owner) {
            ...
            if (firstLevel) {
                _Weight update = owner->semiring_->extend(sssp4SpecialSymbolsHelper::convertWeight<_AutomataWeight, _Weight>(arc->weight()), failWeight);
                collector->feed(update);
            }
    ...
    template<class _AddArc>
    _Weight processState(const Fsa::StateId s, DirectHits& directHits) {
...
private:
    struct AddArc {
        ...
        inline static void regularArcWithDirectHits(const _Arc*                    arc,
                                                    DirectHits&                    directHits,
                                                    typename DirectHits::iterator& darc,
                                                    const _Weight&                 sourceForwardPotential,
                                                    ForwardPotential&              fpot,
                                                    _SsspForward4SpecialSymbols*   owner) {
            // while: darc->output < farc->output
            while ((darc != directHits.end()) and owner->sorter_(*(*darc), *arc))
                ++darc;
            // now: darc->output >= farc->output
        ...
        inline static void failArc(const _Arc*                  arc,
                                   DirectHits&                  directHits,
                                   const _Weight&               sourceForwardPotential,
                                   const bool                   firstLevel,
                                   _SsspForward4SpecialSymbols* owner) {
            Fsa::StateId ts = arc->target();
            owner->processState<AddArc>(owner->fsa_->getState(ts),
                                        directHits,
                                        owner->semiring_->extend(sssp4SpecialSymbolsHelper::convertWeight<_AutomataWeight, _Weight>(arc->weight()), sourceForwardPotential));
        }
    };

    struct AddArcNegCollect {
        inline static void regularArcWithoutDirectHits(const _Arc*                  arc,
                                                       const _Weight&               sourceForwardPotential,
                                                       ForwardPotential&            fpot,
                                                       _SsspForward4SpecialSymbols* owner) {
            Fsa::StateId ts         = arc->target();
    ...
    template<class _AddArc>
    void processState(_ConstStateRef state, DirectHits& directHits, _Weight sourceForwardPotential) {
...
public:
    ...
    inline const ForwardPotential& forwardPotential(Fsa::StateId s) const {
        return *forwardPotentials_[s];
    }
    bool empty() const {
        return forwardPotentials_.empty();
    }
};

template<class _Automaton,
         class _SsspForward4SpecialSymbols  = SsspForward4SpecialSymbols<_Automaton>,
         class _SsspBackward4SpecialSymbols = SsspBackward4SpecialSymbols<_Automaton>>
...
template<class _Automaton, class _SsspSemiring, class _SsspWeight>
typename _Automaton::ConstRef posterior4SpecialSymbols(typename _Automaton::ConstRef f, typename _Automaton::Weight& totalInv, s32 tol) {
...
public:
    ...
    virtual std::string describe() const {
        return "best4SpecialSymbols(" + Precursor::fsa_->describe() + ")";
    }
};
...
public:
    ...
    virtual std::string describe() const {
        return "removeFailure4SpecialSymbols(" + Precursor::fsa_->describe() + ")";
    }
};
