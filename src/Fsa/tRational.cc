...
typename _Automaton::ConstRef identity(
    ...
    result->setProperties(Fsa::PropertyLinear | Fsa::PropertyAcyclic,
                          Fsa::PropertyNone);
    result->addProperties(Fsa::PropertySortedByInput | Fsa::PropertySortedByOutput);
...
template<class _Automaton>
class ClosureAutomaton : public ModifyAutomaton<_Automaton> {
...
template<class _Automaton>
typename _Automaton::ConstRef kleeneClosure(
        typename _Automaton::ConstRef f) {
    return typename _Automaton::ConstRef(new KleeneClosureAutomaton<_Automaton>(f));
}

template<class _Automaton>
class ComplementAutomaton : public SlaveAutomaton<_Automaton> {
    typedef SlaveAutomaton<_Automaton> Precursor;
...
template<class _Automaton>
typename _Automaton::ConstRef complement(
        typename _Automaton::ConstRef f) {
    return typename _Automaton::ConstRef(new ComplementAutomaton<_Automaton>(f));
}

template<class _Automaton>
class ConcatUnionAutomaton : public _Automaton {
...
protected:
    ...
    static Fsa::ConstAlphabetRef uniteAlphabets(
        ...
        for (u32 k = 0; k < alphabets.size(); ++k) {
            ...
            for (Fsa::Alphabet::const_iterator j = alphabet->begin(); j != alphabet->end(); ++j) {
...
public:
    ...
    ConcatUnionAutomaton(const Core::Vector<_ConstAutomatonRef>& fsa)
        ...
        for (typename Core::Vector<_ConstAutomatonRef>::const_iterator i = fsa.begin(); i != fsa.end(); ++i) {
        ...
        for (typename Core::Vector<_ConstAutomatonRef>::const_iterator i = fsa.begin(); i != fsa.end(); ++i)
            if (semiring_ != (*i)->semiring()) {
                std::cerr << "mixed semirings during union. continuing, but behaviour will be unpredictable." << std::endl;
                break;
            }
...
template<class _Automaton>
class ConcatAutomaton : public ConcatUnionAutomaton<_Automaton> {
    ...
    virtual _ConstStateRef getState(Fsa::StateId s) const {
        ...
        if (s != 0) {
            ...
            if ((k < Precursor::fsa_.size() - 1) && (sp->isFinal())) {
                sp->newArc(Precursor::superStateId(k + 1, Precursor::fsa_[k + 1]->initialStateId()), sp->weight_, Fsa::Epsilon);
                sp->unsetFinal();
            }
        }
        else {
            sp = new _State(s);
            sp->newArc(Precursor::superStateId(0, Precursor::fsa_[0]->initialStateId()),
...
template<class _Automaton>
typename _Automaton::ConstRef concat(typename _Automaton::ConstRef f1, typename _Automaton::ConstRef f2) {
...
template<class _Automaton>
typename _Automaton::ConstRef unite(
...
template<class _Automaton>
class FusionAutomaton : public ConcatUnionAutomaton<_Automaton> {
    typedef ConcatUnionAutomaton<_Automaton> Precursor;
    ...
    virtual ConstStateRef getState(Fsa::StateId s) const {
        ...
        if (s != 0) {
            ...
            for (u32 k = 0; k < Precursor::fsa_.size(); ++k) {
                ...
                for (typename State::const_iterator ka = ks->begin(); ka != ks->end(); ++ka) {
...
template<class _Automaton>
typename _Automaton::ConstRef fuse(
 ...
 */
template<class _Automaton>
class TransposeDfsState : public DfsState<_Automaton> {
...
template<class _Automaton>
typename _Automaton::ConstRef transpose(
...
// mark all productive states as final
template<class _Automaton>
class AllPrefixesAutomaton : public ModifyAutomaton<_Automaton> {
    typedef ModifyAutomaton<_Automaton> Precursor;
...
public:
    ...
    AllPrefixesAutomaton(_ConstAutomatonRef f)
            : Precursor(f) {
        ProductiveStateFinder psf(f, productiveStates_);
        psf.dfs();
    }

    virtual void modifyState(_State* sp) const {
        if (sp->id() < productiveStates_.size() and productiveStates_[sp->id()]) {
            if (not sp->isFinal()) {
    ...
    virtual std::string describe() const {
        return "allPrefixes(" + Precursor::fsa_->describe() + ")";
    }

private:
    class ProductiveStateFinder : public DfsState<_Automaton> {
    public:
        typedef DfsState<_Automaton>     Precursor;
        typedef typename _Automaton::Arc _Arc;
        ...
        virtual void discoverState(_ConstStateRef sp) {
            Fsa::StateTag tags = sp->tags();
            if (tags & Fsa::StateTagFinal) {
                markProductive(sp->id());
            }
        }

        virtual void exploreNonTreeArc(_ConstStateRef from, const _Arc& a) {
            if (a.target() < productiveStates_.size() and productiveStates_[a.target()]) {
                markProductive(from->id());
            }
        }

    private:
        std::vector<bool>& productiveStates_;

        void markProductive(Fsa::StateId s) {
            if (productiveStates_.size() <= s) {
                productiveStates_.resize(s + 1, false);
            }
            productiveStates_[s] = true;
            Fsa::StateId current = s;
            Fsa::StateId pred;
            while ((pred = this->predecessor(current)) != current) {  // initial state has itself as predecessor
                if (not productiveStates_[pred]) {
                    productiveStates_[pred] = true;
                }
                else {
                    break;
                }
...
template<class _Automaton>
typename _Automaton::ConstRef allPrefixes(
        typename _Automaton::ConstRef f) {
    return ref(new AllPrefixesAutomaton<_Automaton>(f));
}

// add an edge from the start-state to all productive states
template<class _Automaton>
class AllSuffixesAutomaton : public ModifyAutomaton<_Automaton> {
    typedef ModifyAutomaton<_Automaton> Precursor;
...
public:
    ...
    AllSuffixesAutomaton(_ConstAutomatonRef f)
            : Precursor(f) {
        ReachableStateFinder rsf(f, reachableStates_);
        rsf.dfs();
    }

    virtual void modifyState(_State* sp) const {
        if (sp->id() == this->initialStateId()) {
            for (size_t s = 0ul; s < reachableStates_.size(); s++) {
                if (reachableStates_[s] and s != sp->id()) {
                    sp->newArc(s, this->semiring()->one(), Fsa::Epsilon);
                }
            }
        }
    }
    ...
    virtual std::string describe() const {
        return "allSuffixes(" + Precursor::fsa_->describe() + ")";
    }

private:
    class ReachableStateFinder : public DfsState<_Automaton> {
    public:
        typedef DfsState<_Automaton>     Precursor;
        typedef typename _Automaton::Arc _Arc;
        ...
        virtual void discoverState(_ConstStateRef sp) {
            if (sp->id() >= reachableStates_.size()) {
                reachableStates_.resize(sp->id() + 1);
            }
            reachableStates_[sp->id()] = true;
        }

    private:
        std::vector<bool>& reachableStates_;
    };
...
template<class _Automaton>
typename _Automaton::ConstRef allSuffixes(
        typename _Automaton::ConstRef f) {
    return ref(new AllSuffixesAutomaton<_Automaton>(f));
}
}  // namespace Ftl
