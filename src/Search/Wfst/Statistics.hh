...
namespace Search { namespace Wfst { namespace Statistics {
...
public:
    ...
    SearchSpaceData(const SearchSpaceBase &ss) : ss_(ss) {}
...
{
public:
    SearchSpaceStatistic(const std::string &name) : stat_(name) {}
...
public:
    DetailedStatistic(AbstractStatistic *base) : base_(base) {}
    virtual ~DetailedStatistic() { delete base_; }
    void log(Core::XmlChannel &channel) const {
        base_->log(channel);
    }
    void reset() {
...
{
public:
    enum EventType { beforePruning, afterPruning, afterArcExpansion, nEvents };
...
public:
    DefaultCollector(const SearchSpace *ss) : searchSpace_(ss)
    {
...
public:
    ...
    void processHmmStateHyp(const DetailedStatistic::ArcHyp &arc, const DetailedStatistic::HmmStateHyp &hyp,
...
public:
    DetailedCollector(const SearchSpace *ss) : DefaultCollector(ss)
    {
        countModels();
