...
private:
    ...
    public:
        ...
        private:
            ...
            void uncompress() {
                ...
                if (partition != 0xff) {
                    ++tmp;
                    if (partition & 0x80)
                        substate_.predecessor_ = Fsa::getBytesAndIncrement(tmp, sizeof(Cursor));
                    else
                        substate_.predecessor_ = NoPredecessor;
        ...
        public:
            bool valid() const {
                return (container_.substates_[current_] != 0xff);
            }
            typename Substate::Cursor cursor() const {
                return current_;
            }
            Self& operator++() {
                current_ += skip_;
    ...
    public:
        ...
        size_t nExternalPredecessors() const {
            return predecessors_.size();
        }
        void resizeExternalPrecessors(size_t size) {
            predecessors_.resize(size);
        }
        ...
        Cursor addPredecessor(Cursor p1, Cursor p2) {
            ...
            if (!(p2 & Overflow))
                predecessors_[p1].push_back(p2);
            else
                predecessors_[p1].insert(predecessors_[p1].end(), predecessors_[p2 & ~Overflow].begin(),
        ...
        Cursor predecessor(Cursor pos) const {
            ...
            if (partition & 0x80)
                predecessor = Fsa::getBytesAndIncrement(tmp, sizeof(Cursor));
            return predecessor;
        }
        ...
        std::pair<Cursor, bool> insert(const Core::Vector<Substate>& v) {
            ...
            for (typename Core::Vector<Substate>::const_iterator s = v.begin(); s != v.end(); ++s) {
                ...
                if (s->output_ > 0)
                    partition |= Fsa::estimateBytes(s->output_) << 2;
                if (previous == s->weight_)
                    partition |= 0x02;
                else if (!semiring_->isDefault(s->weight_))
                    partition |= 0x01;
                substates_.push_back(partition);
                if (partition & 0x80)
                    Fsa::appendBytes(substates_, s->predecessor_, sizeof(Cursor));
                Fsa::appendBytes(substates_, state, ((partition >> 5) & 0x03) + 1);
                Fsa::appendBytes(substates_, s->output_, (partition >> 2) & 0x07);
                if (partition & 0x01)
                    semiring_->compress(substates_, s->weight_);
                previous = s->weight_;
            }
        ...
        size_t size() const {
            return substates_.size();
        }
        const_iterator begin(Cursor start) const {
            return const_iterator(*this, start + sizeof(Cursor));
        }
        ...
        bool operator()(const State_& s1, const State_& s2) const {
            if (s1.output_ != s2.output_)
                ...
                for (; sub1.valid() && sub2.valid(); ++sub1, ++sub2) {
                    if (sub1->state_ != sub2->state_)
                        return false;
                    if (d_.semiring_->compare(sub1->weight_, sub2->weight_) != 0)
                        return false;
                    if (sub1->output_ != sub2->output_)
                        return false;
                }
...
public:
    ...
    // subsets
    void subsetStart() const {
        tmpSubstates_.resize(0);
    }
    void subsetInsert(const Substate& s) const {
        tmpSubstates_.push_back(s);
    }
            ...
            if ((*outputs_.begin(sub->output_) != Fsa::InvalidLabelId) || (!semiring_->isDefault(sub->weight_))) {
                ...
                if (sub->predecessor_ & Substates::Overflow) {
                    for (typename Substates::const_predecessor_iterator p = substates_.predecessorBegin(sub->predecessor_);
                         p != substates_.predecessorEnd(sub->predecessor_); ++p)
                        S.push(*p);
                }
                else if (sub->predecessor_ != Substates::NoPredecessor)
                    S.push(sub->predecessor_);
                while (!S.empty()) {
                    ...
                    if (substates_.state(substate) == sub->state_) {
                        ...
                        if (substate & Substates::Overflow)
                            for (typename Substates::const_predecessor_iterator p = substates_.predecessorBegin(substate);
                                 p != substates_.predecessorEnd(substate); ++p)
                                S.push(*p);
                        else if (substate != Substates::NoPredecessor)
                            S.push(substate);
                    }
                }
            }
    ...
    typename Substates::Cursor subsetStop() const {
        ...
        if (disambiguate_)
            nExternalPredecessors = substates_.nExternalPredecessors();
        ...
        if (disambiguate_)
            subsetMarkSiblingsWithAmbiguousCycles();
    ...
    Fsa::StateId insertState(typename Substates::Cursor subset, Fsa::LabelIdStrings::Id output) const {
        ...
        if (id > Fsa::StateIdMask)
            std::cerr << "determinize: out of state ids" << std::endl;
        return id;
    }
            ...
            if (*outputs_.begin(output) == Fsa::InvalidLabelId) {  // zero length string
                    ...
                    if (!sub->disconnect_) {
                        ...
                        for (typename _State::const_iterator a = _sp->begin(); a != _sp->end(); ++a) {
                            ...
                            if (a->output() != Fsa::Epsilon)
                                tmpOutputs_.push_back(a->output());
                        ...
                        Fsa::StateId            target = insertState(subsetStop(), *outputs_.begin(sub->output_) == Fsa::InvalidLabelId ? sub->output_ : id);
                        Fsa::LabelId            label  = *outputs_.begin(sub->output_) == Fsa::InvalidLabelId ? Fsa::Epsilon : *outputs_.begin(sub->output_);
                ...
                for (typename Core::Vector<Arc_>::const_iterator a = tmpArcs_.begin(); a != tmpArcs_.end();) {
                    ...
                    if (outputArc.size() > 1) {
                        ...
                        if (outputArc.size() > 2)
                            output = outputs_.append(outputArc.begin() + 1);
                    }
                    else
                        arc->output_ = Fsa::Epsilon;
                    ...
                    for (a = first; a < last; ++a) {
                        ...
                        if (isPrefix(outputArc, start))
                            output = outputs_.append(start + outputArc.size() - 1);
    ...
    void dumpOutput(Fsa::LabelIdStrings::Id output, std::ostream& o) const {
        ...
        if (*outputs_.begin(s) == Fsa::InvalidLabelId)
            o << "eps";
        else {
    ...
    virtual std::string describe() const {
        return "determinize(" + fsa_->describe() + ")";
    }
};
...
public:
    ...
    virtual std::string describe() const {
        return "local-determinize(" + Precursor::fsa_->describe() + ")";
    }
};
...
public:
    ...
    virtual std::string describe() const {
        return "replaceDisambiguationSymbols(" + Precursor::fsa_->describe() + ", " + Precursor::fsa_->getInputAlphabet()->symbol(replacement_) + ")";
    }
};
