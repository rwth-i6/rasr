...
bool DemoNode::configure() {
    if (demoChannel_.isOpen()) {
        ...
        demoChannel_ << (Core::XmlEmpty("") + Core::XmlAttribute("nInput", nInputs()) + Core::XmlAttribute("nOutput", nOutputs()));
    }
    ...
    for (PortId port = 0; port < nInputs(); ++port) {
        ...
        if (!putOutputAttributes(port, attr)) {
            if (demoChannel_.isOpen()) {
                demoChannel_ << (Core::XmlEmpty("forwarding of attributes failed") + Core::XmlAttribute("port", port));
            }
            status = false;
        }
    }
    if (demoChannel_.isOpen()) {
        demoChannel_ << (Core::XmlEmpty("forwarding status of attributes") + Core::XmlAttribute("status", status));
...
bool DemoNode::work(PortId output) {
    ...
    if (!getData(output, data)) {
        ...
        if (demoChannel_.isOpen()) {
            demoChannel_ << (Core::XmlEmpty("data provided") + Core::XmlAttribute("output port", output))
                         << Core::XmlClose("work method of " + fullName());
        }
        return success;
    }
    ...
    // putData method.
    if (demoChannel_.isOpen()) {
        demoChannel_ << (Core::XmlEmpty("providing data from input port") + Core::XmlAttribute("port", output));
    }
    if (!putData(output, data.get())) {
        if (demoChannel_.isOpen()) {
            demoChannel_ << (Core::XmlEmpty("no data provided") + Core::XmlAttribute("output port", output))
                         << Core::XmlClose("work method of " + fullName());
        }
        return false;
    }
