...
PeakDetection::PeakDetection()
        : minPosition_(0),
          maxPosition_(0),
          maxWidthHalf_(0),
          heightAverageWidthHalf_(0),
          widthHeightFactor_(.5),
          mountainWidth_(false),
          sampleRate_(0),
          needInit_(true) {}
...
u32 PeakDetection::getMaximumIndex(const std::vector<Amplitude>& v) const {
    ...
    return std::max_element(v.begin() + minPosition_, v.begin() + maxPosition_ + 1) - v.begin();
}
    ...
    for (u32 peakBegin = 1; peakBegin + 1 < v.size(); ++peakBegin) {
        if (v[peakBegin - 1] < v[peakBegin] && v[peakBegin] >= v[peakBegin + 1]) {
            ...
            if (peakEnd + 1 < v.size() && v[peakEnd] > v[peakEnd + 1] &&
                ...
                maximumPeakPosition = std::min(std::max(maximumPeakPosition, minPosition_), maxPosition_);
            }
...
f32 PeakDetection::getWidthConfidence(const std::vector<Amplitude>& v,
    ...
    u32 width = mountainWidth_ ? getMountainWidth(v, peakIndex, widthHeight) : getPeakWidth(v, peakIndex, widthHeight);
...
ParameterFloat PeakDetectionNode::paramMinPosition("min-position", "min peak position in continous units depending on previous nodes");

ParameterFloat PeakDetectionNode::paramMaxPosition("max-position", "max peak position in continous units depending on previous nodes");

ParameterFloat PeakDetectionNode::paramMaxWidth("max-width", "width with which bandwith is normed (in continous units depending on previous nodes)");

ParameterFloat PeakDetectionNode::paramHeightAverageWidth("height-average-width",
                                                          "width over which heigth average is taken (in cont. units depending on prev. nodes, 0: [min-peak-pos..max-peak=pos])",
                                                          0, -2);

ParameterFloat PeakDetectionNode::paramWidthHeightFactor("width-height-factor",
