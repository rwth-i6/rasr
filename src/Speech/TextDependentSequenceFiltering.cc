...
AlignmentToSequenceSelectionNode::Set::Set(const Core::Configuration& c)
    ...
    Core::Ref<Am::AcousticModel> acousticModel = Am::Module::instance().createAcousticModel(
            c, lexicon, Am::AcousticModel::noEmissions | Am::AcousticModel::noStateTying | Am::AcousticModel::noStateTransition);
    if (!acousticModel)
        criticalError("Failed to create the acoustic model.");
...
bool AlignmentToSequenceSelectionNode::configure() {
    ...
    std::shared_ptr<Flow::Attributes> attributeSelection(new Flow::Attributes);
    getInputAttributes(0, *attributeSelection);
    std::shared_ptr<Flow::Attributes> attributeRatio(new Flow::Attributes(*attributeSelection));
...
bool AlignmentToSequenceSelectionNode::work(Flow::PortId p) {
    ...
    if (getData(0, in)) {
        ...
        if (alignment.size() > 0)
            ratio->data() = (f32)filteredFeatureCount / (f32)alignment.size();
        else
            ratio->data() = Core::Type<f32>::max + 1;
