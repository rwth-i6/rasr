...
bool StringExpression::value(std::string& v, bool ignore_unknown_parameters) const {
    std::string result;
    size_t      idx = 0;
    for (Tokens::const_iterator t = tokens_.begin(); t != tokens_.end(); ++t) {
        ...
        else if (not ignore_unknown_parameters) {
            return false;
        }
        else {
            for (Variables::const_iterator iter_v = variables_.begin(); iter_v != variables_.end(); ++iter_v) {
                TokenReferences const& tokref = iter_v->second;
                for (TokenReferences::const_iterator iter_r = tokref.begin(); iter_r != tokref.end(); ++iter_r) {
                    if (*iter_r == idx) {
                        result += "$(" + iter_v->first + ")";
                        goto search_end;
                    }
