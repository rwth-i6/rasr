...
typedef std::unordered_set<AugmentedDensityEstimator,
                           AugmentedDensityEstimatorHash,
                           AugmentedDensityEstimatorEquality>
        DensitySet;
...
public:
    ...
    typedef std::unordered_map<ObjectRef, Value, hashReference<Object>, std::equal_to<ObjectRef>> Map;
...
public:
    ...
    void insert(ObjectRef o, Value v) {
        map_[o] = v;
    }
    typename Map::const_iterator find(ObjectRef o) const {
        return map_.find(o);
    }
 ...
 */
class CovarianceToDensitySetMap : public ReferenceValueMap<AbstractCovarianceEstimator, DensitySet> {
...
CovarianceToDensitySetMap::CovarianceToDensitySetMap(const AbstractMixtureSetEstimator::MixtureEstimators& mixtureEstimators) {
    MixtureEstimators::const_iterator m = mixtureEstimators.begin();
    for (; m != mixtureEstimators.end(); ++m) {
        Core::Ref<EbwDiscriminativeMixtureEstimator> mixtureEstimator(required_cast(EbwDiscriminativeMixtureEstimator*, m->get()));
...
Xi::Xi(const Core::Configuration& c,
...
IterationConstant Xi::denominator(Core::Ref<AbstractMixtureEstimator> mixtureEstimator) {
    ...
    for (; d != mixtureEstimator->densityEstimators().end(); ++d) {
        Core::Ref<EbwDiscriminativeMeanEstimator> meanEstimator(required_cast(EbwDiscriminativeMeanEstimator*,
                                                                              (*d)->mean().get()));
...
protected:
    ...
    IterationConstant sigmaMinimum() const {
        return sigmaMinimum_;
    }
 ...
 * calculate D_k_min
 */
IterationConstant Icb::minimumDk(CovarianceToDensitySetMap::const_iterator covarianceAndDensitySet,
                                 const CovarianceToMeanSetMap::MeanSet&    meanSet) {
    Core::Ref<EbwDiscriminativeCovarianceEstimator> covarianceEstimator(required_cast(EbwDiscriminativeCovarianceEstimator*,
                                                                                      covarianceAndDensitySet->first.get()));
...
GlobalIcb::GlobalIcb(const Core::Configuration&       c,
    ...
    CovarianceToDensitySetMap::const_iterator it = cim.begin();
    for (; it != cim.end(); ++it) {
        const CovarianceToMeanSetMap::MeanSet& meanSet = meanSetMap[Core::Ref<AbstractCovarianceEstimator>(it->first)];
...
public:
    ...
    virtual ~GlobalRwthIterationConstants() {
        delete ic_;
    }
};
...
void GlobalRwthIterationConstants::initialize(
        ...
        const CovarianceToMeanSetMap&                meanSetMap) {
    verify(!ic_);
    ic_ = new InternalIc::GlobalIcb(select("icb"), mixtureEstimators, cim, stepSize_, meanSetMap);
}
...
protected:
    virtual void initialize(const MixtureEstimators&,
                            const InternalIc::CovarianceToDensitySetMap&,
                            const CovarianceToMeanSetMap&);
    virtual void set(const InternalIc::AugmentedDensityEstimator&);

public:
    LocalRwthIterationConstants(const Core::Configuration&);
    virtual ~LocalRwthIterationConstants() {
        delete ic_;
    }
};
...
void LocalRwthIterationConstants::initialize(const MixtureEstimators&                     mixtureEstimators,
                                             const InternalIc::CovarianceToDensitySetMap& cim,
                                             const CovarianceToMeanSetMap&                meanSetMap) {
    verify(!ic_);
    ic_ = new InternalIc::LocalIcb(select("icb"), mixtureEstimators, cim, stepSize_, meanSetMap);
}
...
public:
    ...
    virtual ~CambridgeIterationConstants() {
        delete ic_;
    }
};
...
Sum CambridgeIterationConstants::maximumRoot(Core::Ref<EbwDiscriminativeMeanEstimator>       meanEstimator,
                                             Core::Ref<EbwDiscriminativeCovarianceEstimator> covarianceEstimator) const {
    verify(meanEstimator and covarianceEstimator);

    require(false);  // beck: why is this here ????
...
void CambridgeIterationConstants::set(const InternalIc::AugmentedDensityEstimator& d) {
    ...
    Core::Ref<EbwDiscriminativeCovarianceEstimator> covarianceEstimator(required_cast(EbwDiscriminativeCovarianceEstimator*,
                                                                                      d.densityEstimator->covariance().get()));
...
void IterationConstants::dumpIterationConstants(
        const MixtureEstimators& mixtureEstimators) {
    for (MixtureIndex mix = 0; mix < mixtureEstimators.size(); ++mix) {
        const AbstractMixtureEstimator::DensityEstimators& densityEstimators = mixtureEstimators[mix]->densityEstimators();
        for (DensityIndex dns = 0; dns < densityEstimators.size(); ++dns) {
            Core::Ref<EbwDiscriminativeMeanEstimator> meanEstimator(required_cast(EbwDiscriminativeMeanEstimator*,
                                                                                  densityEstimators[dns]->mean().get()));
...
void IterationConstants::set(const MixtureEstimators&      mixtureEstimators,
                             const CovarianceToMeanSetMap& meanSetMap) {
