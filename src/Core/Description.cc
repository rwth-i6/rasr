...
//=============================================================================================
void Description::Stream::getDependencies(const std::string& prefix, std::string& value) const {
...
bool Description::verifyNumberOfStreams(size_t expectedNumber, bool critical) const {
    if (expectedNumber != nStreams()) {
        Application::us()->error("%s: number of streams does not match: %zu expected and %zu exist.",
