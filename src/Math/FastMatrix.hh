...
public:
    ...
    T& operator() (u32 row, u32 column) { return at(row, column); }
    T const& operator() (u32 row, u32 column) const { return at(row, column); }
...
inline void FastMatrix<T>::addKroneckerDelta(const FastVector<S>& alignment, const FastVector<U>& offset, const FastMatrix<U>& maxIndex, const T scale) {
    ...
    if (typeid(S) != typeid(u32) || typeid(U) != typeid(u32)) Core::Application::us()->warning("FastMatrix<T>::kroneckerDelta expects alignment, mixture, and offset vector of type u32");
