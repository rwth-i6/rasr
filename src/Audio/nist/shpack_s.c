...
extern int read_wav_data(FOB *fp, short int **pwaveform, struct header_t *header, int check_checksum);
...
/*
    ...
    pos=fob_ftell(fpin);
    read_shortpack_header(fpin, 0, &filesize, &swap);
...
void read_shortpack_header(FOB *ifp, int arg, int *pfilesize, int *pswap)
{
  int flags;
