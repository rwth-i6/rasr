...
public:
    ...
    LatticeWeight() {}
    LatticeWeight(const LatticeWeight& w) : PairWeight(w) {}
    LatticeWeight(const PairWeight& w) : PairWeight(w) {}
    LatticeWeight(TropicalWeight w1, TropicalWeight w2) : PairWeight(w1, w2) {}
...
inline int Compare(const LatticeWeight &w, const LatticeWeight &v) {
    ...
    if (f1 < f2) return 1;
    else if (f1 > f1) return -1;
    else if (w.LmScore() < v.LmScore()) return 1;
    else if (w.LmScore() > v.LmScore()) return -1;
    else return 0;
}
...
public:
    ...
    Lattice() : outputType_(DefaultOutput) {}
    explicit Lattice(const Lattice &o) :
            VectorFst(o), wordBoundaries_(o.wordBoundaries_), outputType_(DefaultOutput) {}
    explicit Lattice(const VectorFst &o) : VectorFst(o), outputType_(DefaultOutput) {}
    explicit Lattice(const FstLib::Fst<LatticeArc> &o) : VectorFst(o), outputType_(DefaultOutput) {}
    virtual ~Lattice() {}
        ...
        if (!wordBoundaries_.empty()) {
            ...
            for (StateId s = 0; s < wordBoundaries_.size(); ++s) {
                if (d != sorted.end() && s == *d) {
                    ...
                    if (s != nstates) wordBoundaries_[nstates] = wordBoundaries_[s];
                    ++nstates;
                }
            }
    ...
    void setOutputType(OutputType t) { outputType_ = t; }
private:
