...
#if 0
    class LogCollectLmScores : public TopologicalSortState {
    private:
        ConstLanguageModelRef lm_;
        ScoreId lmId_;
        Score lmScale_;
        ScoreList::const_iterator beginScale_, endScale_;
        HistoriesList histories_;
        Core::Ref<const Bliss::LemmaAlphabet> lAlphabet_;
        Core::Ref<const Bliss::LemmaPronunciationAlphabet> lpAlphabet_;
        Collector *logCollector_;

    protected:
        virtual void exploreState(ConstStateRef sr) {
            HistoryHashList::const_iterator
                begin_histories = histories_[sr->id()].begin(),
                end_histories = histories_[sr->id()].end();
            verify(begin_histories != end_histories);
            ScoreList multipliers(semiring_->scales().size(), ::log(Score(histories_.size())));
            ScoreList::iterator itMult = multipliers.begin();
            for (ScoreList::const_iterator itScale = beginScale_; itScale != endScale_;
                 ++itScale, ++itMult) *itMult /= *itScale;
            if (sr->isFinal()) {
                ScoresRef w(sr->weight());
                Scores::iterator itW = w->begin();
                for (ScoreList::const_iterator itMult = multipliers.begin(), endMult = multipliers.end();
                     itMult != endMult; ++itMult, ++itW) *itW -= *itMult;
                for (HistoryHashList::const_iterator itHistory = begin_histories;
                     itHistory != end_histories; ++itHistory)
                    logCollector_->feed(lmScale_ * lm_->sentenceEndScore(*itHistory));
                w->set(lmId_, Score(logCollector_->get()) / lmScale_);
                logCollector_->reset();
            }
            for (State::const_iterator a = sr->begin(); a != sr->end(); ++a) {
                if ((Fsa::FirstLabelId <= a->input()) && (a->input() <= Fsa::LastLabelId)) {
                    ...
                    for (HistoryHashList::const_iterator itHistory = begin_histories;
                         itHistory != end_histories; ++itHistory) {
                        Lm::Score lmScore(0);
                        Lm::History history(*itHistory);
                        Lm::addLemmaScore(
                            lm_, lmScale_,
                            (lAlphabet_ ?
                             lAlphabet_->lemma(a->input()) :
                             lpAlphabet_->lemmaPronunciation(a->input())->lemma()),
                            lmScale_, history, lmScore);
                        logCollector_->feed(lmScore);
                        histories_[a->target()].insert(history);
                    }
                    w->set(lmId_, Score(logCollector_->get()) / lmScale_);
                    logCollector_->reset();
                } else {
                    verify(a->input() == Fsa::Epsilon);
                    a->weight()->set(id_, Semiring::One);
                    for (HistoryHashList::const_iterator itHistory = begin_histories;
                         itHistory != end_histories; ++itHistory)
                        histories_[a->target()].insert(*itHistory);
                }
            }
            // histories_[sr->id()].erase();
        }

    public:
        LogCollectLmScores(StaticLatticeRef s, ConstLanguageModelRef lm, ScoreId id, ConstSemiringRef semiring) :
            TopologicalSortState(s), lm_(lm), id_(id) {
            verify(s->getTopologicalSort());
            require(s->getTopologicalSort()->maxSid != Fsa::InvalidStateId);
            verify(semiring && (semiring->type() == Fsa::SemringTypeLog));
            require(s->semiring()->hasId(id));
            lmScale_ = semiring->scale(id);
            beginScale_ = semiring->scales()->begin();
            endScale_ = semiring->scales()->end();
            histories_.resize(s->getTopologicalSort()->maxSid + 1);
            histories_[s->getTopologicalSort()->front()].insert(lm_->startHistory());
            switch (getLexiconAlphabetId(s->getInputAlphabet())) {
            ...
            logCollector_ = createCollector(Fsa::SemiringTypeLog);
            traverse();
            s->setDescription(Core::form("collectLmScores(%s;%d,%s)",
                                         s->describe().c_str(), id, semiring->name().c_str()));
        }
        virtual ~LogCollectLmScores() {
            delete logCollector_;
        }
    };

    ConstLatticeRef logCollectLmScores(ConstLatticeRef l, ConstLanguageModelRef lm, ScoreId lmId, ConstSemiringRef semiring) {
        if (!semiring)
            semiring = toLogSemiring(s->semiring());
        l = mapInputToLemmaOrLemmaPronunciation(l);
        sortTopologically(l);
        StaticLatticeRef s = StaticLatticeRef(new StaticLattice);
        deepCopy(l, s.get(), 0);
        s->setBoundaries(l->getBoundaries());
        LogCollectLmScores collectLmScores(s, lm, id);
        return s;
    }
    // -------------------------------------------------------------------------
#endif
