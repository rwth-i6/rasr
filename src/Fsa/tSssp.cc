...
StatePotentials<typename _Automaton::Weight> ssspLoop(
    ...
    while (!q.empty()) {
        ...
        if (p)
            p->notify(q.size(), q.maxStateId() - 1);
    }
    if (p) {
        p->finish();
        delete p;
...
StatePotentials<typename _Automaton::Weight> sssp(
...
template<class _Automaton>
class PushToFinalAutomaton : public ModifyAutomaton<_Automaton> {
...
template<class _Automaton>
typename _Automaton::ConstRef pushToFinal(
...
template<class _Automaton>
class PushToInitialAutomaton : public ModifyAutomaton<_Automaton> {
    typedef ModifyAutomaton<_Automaton> Precursor;
...
public:
    ...
    virtual void modifyState(_State* sp) const {
        ...
        if (s == initialStateId_)
            sp->weight_ = semiring_->one();
        if (sp->isFinal())
            sp->weight_ = semiring_->extend(semiring_->extend(sp->weight_, potentials_[initialStateId_]), semiring_->invert(potentials_[s]));
    }
    virtual std::string describe() const {
...
template<class _Automaton>
typename _Automaton::ConstRef pushToInitial(
        typename _Automaton::ConstRef f, bool progress) {
    return typename _Automaton::ConstRef(new PushToInitialAutomaton<_Automaton>(f, progress));
}

template<class _Automaton>
class PosteriorAutomaton : public ModifyAutomaton<_Automaton> {
    typedef ModifyAutomaton<_Automaton> Precursor;
...
template<class _Automaton>
typename _Automaton::ConstRef posterior(
...
template<class _SrcAutomaton>
class CountPathsAutomaton : public WrapperAutomaton<_SrcAutomaton, CountAutomaton> {
...
template<class _SrcAutomaton>
typename CountAutomaton::ConstSemiringRef
