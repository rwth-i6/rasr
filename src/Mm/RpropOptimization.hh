...
private:
    T minStepSize() const {
        return settings_.minStepSize;
    }
    T maxStepSize() const {
        return settings_.maxStepSize;
    }
    T increasingFactor() const {
        return settings_.increasingFactor;
    }
    T decreasingFactor() const {
        return settings_.decreasingFactor;
    }
    bool useWeightBacktracking() const {
        return settings_.useWeightBacktracking;
    }
    s8 sign(T x) const {
        return (x == 0) ? 0 : ((x > 0) ? 1 : -1);
    }
    s8 direction(ComponentIndex cmp) const {
        return sign(gradient(cmp));
    }
    ...
    void setHold(ComponentIndex cmp) {
        if (stepSizes_[cmp] > 0)
            stepSizes_[cmp] = -stepSizes_[cmp];
    }
    void unsetHold(ComponentIndex cmp) {
        if (stepSizes_[cmp] < 0)
            stepSizes_[cmp] = -stepSizes_[cmp];
    }
    bool isHold(ComponentIndex cmp) const {
        return (stepSizes_[cmp] < 0);
    }
    T stepSize(ComponentIndex cmp) const {
        return Core::abs(stepSizes_[cmp]);
    }
...
public:
    ...
    void setStepSizes(const std::vector<T>& stepSizes) {
        stepSizes_ = stepSizes;
    }
    void setStepSizes(u32 dimension, T stepSize) {
        stepSizes_.clear();
        stepSizes_.resize(dimension, stepSize);
    }
    const std::vector<T>& stepSizes() const {
        return stepSizes_;
    }
    void setPreviousToPrevious(const std::vector<T>& p2p) {
        previousToPrevious_ = p2p;
    }
