...
class XmlBuilderElementTemplate : public CM {
public:
...
class XmlBuilderElement<P, CM, CreateStatic> : public XmlBuilderElementTemplate<CM, P, P, const P&> {
...
public:
    ...
    XmlBuilderElement(const char* _name, XmlContext* _context, Handler _handler = 0)
            : Precursor(_name, _context, _handler) {}
};

template<typename P, class CM>
class XmlBuilderElement<P, CM, CreateUsingNew> : public XmlBuilderElementTemplate<CM, P, std::unique_ptr<P>, std::unique_ptr<P>&> {
    typedef XmlBuilderElementTemplate<CM, P, std::unique_ptr<P>, std::unique_ptr<P>&> Precursor;

public:
    typedef std::unique_ptr<P>& HandlerArgument;
    typedef void (XmlContext::*Handler)(HandlerArgument);
    typedef P Product;
...
template<typename P, class CM>
class XmlBuilderElement<P, CM, CreateByContext> : public XmlBuilderElementTemplate<CM, P, P*, P*> {
    typedef XmlBuilderElementTemplate<CM, P, P*, P*> Precursor;

public:
    typedef P* CreationHandlerResult;
    typedef CreationHandlerResult (XmlContext::*CreationHandler)(const XmlAttributes atts);
    template<class T>
    static CreationHandler creationHandler(CreationHandlerResult (T::*h)(const XmlAttributes atts)) {
...
public:
    ...
    XmlBuilderElement(const char* _name, XmlContext* _context, CreationHandler _creationHandler, Handler _handler = 0)
            : Precursor(_name, _context, _handler),
...
class XmlDataOnlyBuilderElement : public XmlBuilderElementTemplate<XmlEmptyElement, P, P, const P&> {
...
template<typename P>
class XmlChoiceBuilderElement : public XmlDataOnlyBuilderElement<P, ChoiceConversion<P>> {
protected:
...
public:
    XmlChoiceBuilderElement(const char*   _name,
                            XmlContext*   _context,
