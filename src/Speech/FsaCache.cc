...
FsaCache::FsaCache(const Core::Configuration& c, Fsa::StoredComponents whatToStore)
    ...
    if (!path.empty()) {
        ...
        if (!archive_)
            error("Failed to create archive '%s'.", path.c_str());
    }
}
...
Fsa::ConstAutomatonRef FsaCache::read(const std::string& id) {
    ...
    if (reader.isOpen()) {
        ...
        if (!Fsa::readBinary(result.get(), reader)) {
            result.reset();
            error("Failed to read the object \"%s\" from cache \"%s\"", id.c_str(), archive_->path().c_str());
        }
        else {
            if (inputAlphabet_)
                result->setInputAlphabet(inputAlphabet_);
            if (outputAlphabet_)
                result->setOutputAlphabet(outputAlphabet_);
        }
    }
    return result;
}
...
void FsaCache::write(const std::string& id, Fsa::ConstAutomatonRef fsa) {
    ...
    if (!Fsa::writeBinary(fsa, writer, whatToStore_)) {
        error("Failed to write the object \"%s\" to cache \"%s\"", id.c_str(), archive_->path().c_str());
    }
}
