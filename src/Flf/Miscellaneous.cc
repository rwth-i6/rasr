    ...
    virtual ConstLatticeRef filter(ConstLatticeRef l) {
        ...
        if (connected(1)) {
            ...
            if (!segment->hasStartTime() || !segment->hasEndTime()) {
                criticalError("Segment \"%s\" has no start and end time.", segment->segmentId().c_str());
            }
            else {
                startTime = 0;
...
StaticLatticeRef cleanUp(ConstLatticeRef l) {
    ...
    Fsa::ConstAlphabetRef inputA     = l->getInputAlphabet();
    Fsa::ConstAlphabetRef outputA    = isAcceptor ? l->getInputAlphabet() : l->getOutputAlphabet();
        ...
        if (C[sid] == White) {
            ...
            for (State::const_iterator a = sr->begin(); a != sr->end(); ++a) {
                if (isValidTarget(a->target())) {
                    ...
                    if (isValidArc) {
                        if ((C[a->target()] == Gray) || !isValidLabel(inputA, a->input()) || !isValidLabel(outputA, a->output()) || (isAcceptor && (a->input() != a->output())) || !isFinite(semiring, a->weight()))
                            isValidArc = false;
                    }
                }
                else
