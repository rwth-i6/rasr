...
xmlParserInputPtr XmlParser::SAX_resolveEntity_callback(void* ctx, const xmlChar* publicId, const xmlChar* systemId) {
    return static_cast<self*>(ctx)->resolveEntity(
            x2c(publicId), x2c(systemId));
}

void XmlParser::SAX_internalSubset_callback(void* ctx, const xmlChar* name, const xmlChar* ExternalID, const xmlChar* SystemID) {
    static_cast<self*>(ctx)->internalSubset(x2c(name), x2c(ExternalID), x2c(SystemID));
}

void XmlParser::SAX_externalSubset_callback(void* ctx, const xmlChar* name, const xmlChar* ExternalID, const xmlChar* SystemID) {
    static_cast<self*>(ctx)->externalSubset(x2c(name), x2c(ExternalID), x2c(SystemID));
}

xmlEntityPtr XmlParser::SAX_getEntity_callback(void* ctx, const xmlChar* name) {
    return static_cast<self*>(ctx)->getEntity(x2c(name));
}

xmlEntityPtr XmlParser::SAX_getParameterEntity_callback(void* ctx, const xmlChar* name) {
    return static_cast<self*>(ctx)->getParameterEntity(x2c(name));
}

void XmlParser::SAX_entityDecl_callback(void*          ctx,
...
void XmlParser::SAX_notationDecl_callback(void* ctx, const xmlChar* name,
                                          const xmlChar* publicId, const xmlChar* systemId) {
    static_cast<self*>(ctx)->notationDecl(x2c(name), x2c(publicId), x2c(systemId));
}

void XmlParser::SAX_attributeDecl_callback(void* ctx, const xmlChar* elem, const xmlChar* name, int type, int def,
                                           const xmlChar* defaultValue, xmlEnumerationPtr tree) {
    static_cast<self*>(ctx)->attributeDecl(x2c(elem), x2c(name), type, def, x2c(defaultValue), tree);
}

void XmlParser::SAX_elementDecl_callback(void* ctx, const xmlChar* name, int type, xmlElementContentPtr content) {
    static_cast<self*>(ctx)->elementDecl(x2c(name), type, content);
}

void XmlParser::SAX_unparsedEntityDecl_callback(void* ctx, const xmlChar* name, const xmlChar* publicId, const xmlChar* systemId, const xmlChar* notationName) {
    static_cast<self*>(ctx)->unparsedEntityDecl(x2c(name), x2c(publicId), x2c(systemId), x2c(notationName));
}

void XmlParser::SAX_startDocument_callback(void* ctx) {
    static_cast<self*>(ctx)->startDocument();
}

void XmlParser::SAX_endDocument_callback(void* ctx) {
    static_cast<self*>(ctx)->endDocument();
}

void XmlParser::SAX_startElement_callback(void* ctx, const xmlChar* name, const xmlChar** atts) {
    static_cast<self*>(ctx)->startElement(x2c(name), XmlAttributes(atts));
}

void XmlParser::SAX_endElement_callback(void* ctx, const xmlChar* name) {
    static_cast<self*>(ctx)->endElement(x2c(name));
}

void XmlParser::SAX_reference_callback(void* ctx, const xmlChar* name) {
    static_cast<self*>(ctx)->reference(x2c(name));
}

void XmlParser::SAX_characters_callback(void* ctx, const xmlChar* ch, int len) {
    static_cast<self*>(ctx)->characters(x2c(ch), len);
}

void XmlParser::SAX_cdataBlock_callback(void* ctx, const xmlChar* ch, int len) {
    static_cast<self*>(ctx)->cdataBlock(x2c(ch), len);
}

void XmlParser::SAX_ignorableWhitespace_callback(void* ctx, const xmlChar* ch, int len) {
    static_cast<self*>(ctx)->ignorableWhitespace(x2c(ch), len);
}

void XmlParser::SAX_processingInstruction_callback(void* ctx, const xmlChar* target, const xmlChar* data) {
...
void XmlParser::externalSubset(const char* name, const char* ExternalID, const char* SystemID) {
...
void XmlParser::entityDecl(const char* name, int type, const char* publicId, const char* systemId, const char* content) {
}

void XmlParser::attributeDecl(const char* elem, const char* name, int type, int def, const char* defaultValue, xmlEnumerationPtr tree) {
}

void XmlParser::elementDecl(const char* name, int type, xmlElementContentPtr content) {
}

void XmlParser::notationDecl(const char* name, const char* publicId, const char* systemId) {
}

void XmlParser::unparsedEntityDecl(const char* name, const char* publicId, const char* systemId, const char* notationName) {
}
...
void XmlParser::processingInstruction(const char* target, const char* data) {
...
void XmlParser::warnAboutUnexpectedElement(const char* element, const char* context) {
    if (warnAboutUnexpectedElements_)
        warning("Found unexpected element: \"%s\" within \"%s\"", element, context);
}
...
int XmlParser::parseStream(std::istream& is, const std::string& filename) {
    ...
    ctxt_ = xmlCreatePushParserCtxt(const_cast<xmlSAXHandler*>(&SAX_callbacks), static_cast<void*>(this), buffer, nRead, filename.c_str());
    if (ctxt_ == NULL)
        error("Failed to setup XML parser");
...
// ===========================================================================
XmlElement::XmlElement(const char* _name, XmlContext* _context)
        : context_(_context),
          name_(_name) {}
...
bool XmlElement::matches(const char* _name) const {
    return (strcmp(name(), wildcard) == 0) || (strcmp(name(), _name) == 0);
}
...
void XmlDocumentElement::end() {
    if (!hasRootOccured_) {
        ...
        parser()->error("Document ended before root element \"%s\" was found", root_->name());
    }
}
...
XmlElement* XmlDocumentElement::element(const char* name) {
    ...
    if (root_->matches(name)) {
        ...
        /* validityError */
        parser()->error("Root element \"%s\" expected, found \"%s\" instead", root_->name(), name);
        return 0;
    }
}
...
// ===========================================================================
XmlEmptyElement::XmlEmptyElement(const char* _name, XmlContext* _context, bool allowWhitespace)
...
XmlElement* XmlEmptyElement::element(const char* _name) {
    /* validityError */
    parser()->error("Element \"%s\" is declared to have empty content", name());
    return 0;
}
...
// ===========================================================================
XmlEmptyElementRelay::XmlEmptyElementRelay(const char* _name, XmlContext* _context, XmlContext::StartHandler _startHandler /*= 0*/)
...
XmlIgnoreElement::XmlIgnoreElement(const char* _name, XmlContext* _context)
        : XmlElement(_name, _context) {}
...
// ===========================================================================
XmlFlattenElement::XmlFlattenElement(const char* _name, XmlContext* _context)
        : XmlElement(_name, _context) {
    require(parent());
}
...
// ===========================================================================
XmlFlattenElementRelay::XmlFlattenElementRelay(const char* _name, XmlContext* _context, XmlContext::StartHandler _startHandler,
...
// ===========================================================================
XmlParentElement::XmlParentElement(const char* _name, XmlContext* _context)
        : XmlElement(_name, _context) {
...
// ===========================================================================
XmlMixedElement::XmlMixedElement(const char* _name, XmlContext* _context)
        : XmlParentElement(_name, _context) {}

XmlMixedElement::XmlMixedElement(const char* _name, XmlContext* _context, XmlElement* _child, ...)
        : XmlParentElement(_name, _context) {
    va_list ap;
...
// ===========================================================================
XmlMixedElementRelay::XmlMixedElementRelay(const char* _name, XmlContext* _context, XmlContext::StartHandler _startHandler,
                                           XmlContext::EndHandler _endHandler, XmlContext::CharactersHandler _charactersHandler)
...
XmlMixedElementRelay::XmlMixedElementRelay(const char* _name, XmlContext* _context, XmlContext::StartHandler _startHandler,
                                           XmlContext::EndHandler _endHandler, XmlContext::CharactersHandler _charactersHandler, XmlElement* _child, ...)
        : XmlMixedElement(_name, _context) {
    va_list ap;
...
XmlRegularElement::XmlRegularElement(const char* _name, XmlContext* _context)
        : XmlParentElement(_name, _context) {
    state_ = final;
}
...
void XmlRegularElement::draw(std::ostream& os) const {
    ...
    for (t = transitions_.begin(); t != transitions_.end(); ++t) {
        if (t->child) {
            os << t->from << " -> " << t->to << " [label=\"" << t->child->name() << "\"];" << std::endl;
        }
        else {
...
// ===========================================================================
XmlRegularElementRelay::XmlRegularElementRelay(const char* _name, XmlContext* _context, XmlContext::StartHandler _startHandler,
                                               XmlContext::EndHandler _endHandler)
...
void XmlSchemaParser::startElement(const char* _name, const XmlAttributes atts) {
    verify(!parse_stack_.empty());
