...
namespace Signal
{
  ...
  public:
    ...
    public:
      ...
      u8 rowSize() { return row_size_; };
      u32 columnSize() { return column_size_; };
    };
  ...
  protected:
    ...
    virtual bool init() { return !(need_init_ = !reallocateBuffers()); };
    virtual bool reallocateBuffers();
    virtual bool queryWorkspaceSize(u32& size) = 0;
    virtual void freeBuffers();

  public:
    LeastSquares() : nr_sample_(0), nr_parameter_(0), X_(0), y_(0),
      work_(0), lwork_(0), need_init_(true) {};
    virtual ~LeastSquares() { freeBuffers(); };

    void setNumberOfSamples(u32 nr_sample)
    { if (nr_sample_ != nr_sample)
      { nr_sample_ = nr_sample; need_init_ = true; } }

    void setNumberOfParameters(u8 nr_parameter)
    { if (nr_parameter_ != nr_parameter)
      { nr_parameter_ = nr_parameter; need_init_ = true; } }

    virtual void reset() { freeBuffers(); need_init_ = true; }
  ...
  void LeastSquares<T>::freeBuffers()
  {
    ...
    if (y_) { delete y_; y_ = 0; }
    if (work_) { delete work_; work_ = 0;}
  ...
  public: // constructor & destructor
    SvdLeastSquares() : singularValues_(0) {};
  ...
  {
    if (singularValues_) delete singularValues_;
  ...
  {
    if (singularValues_) { delete singularValues_; singularValues_ = 0; }
  ...
  private:
    ...
    bool init()
    { least_squares_.setNumberOfParameters(order_B_ + order_A_); return !(need_init_ = false); }

  public:
    LeastSquaresBuilder(LeastSquares<T>& least_squares) :
      least_squares_(least_squares), order_B_(0), order_A_(0), need_init_(true) {};

    void setOrder_B(u8 order_B) { if (order_B_ != order_B) { order_B_ = order_B; need_init_ = true; } }
    void setOrder_A(u8 order_A) { if (order_A_ != order_A) { order_A_ = order_A; need_init_ = true; } }
  ...
  {
    if (y == 0 || (u && y->size() != u->size()) || ((order_B_ > 0 && u == 0) &&
        (y0 != 0  && y->size() != y0->size())))
      return false;
  ...
  private:
    ...
    T& Ruu(u8 row, u8 column) { return X_(row, column); };
    T& Ruy(u8 row, u8 column) { return X_(row, column + order_B_); };
    T& Ryu(u8 row, u8 column) { return X_(row + order_B_, column); };
    T& Ryy(u8 row, u8 column) { return X_(row + order_B_, column + order_B_); };

    T& fuy(u8 row) { return y_(row); };
    T& fyy(u8 row) { return y_(row + order_B_); };
    ...
    bool CovarianceBuilder<T>::work(const std::vector<T>* u, const std::vector<T>* y, const std::vector<T>* y0)
  {
    ...
    /*
    for(i = 0; i < order_A_ + order_B_; i ++)
      {
        for(k = 0; k < order_A_ + order_B_; k ++)
          cout << X_(i,k) << " ";
        cout << " " << y_(i) << endl;
      }
    */
  ...
  private:
    ...
    T& Ruu(u8 row, u8 column) { return X_(row, column); };
    T& Ruy(u8 row, u8 column) { return X_(row, column + order_B_); };
    T& Rzu(u8 row, u8 column) { return X_(row + order_B_, column); };
    T& Rzy(u8 row, u8 column) { return X_(row + order_B_, column + order_B_); };

    T& fuy(u8 row) { return y_(row); };
    T& fzy(u8 row) { return y_(row + order_B_); };
  ...
  {
    if (y == 0 || (u != 0 && y->size() != u->size()) || (this->order_B > 0 && u == 0) &&
        (z != 0 && y->size() != z->size()))
      return false;
