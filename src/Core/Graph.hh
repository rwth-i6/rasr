...
public:
    ...
    static const NodeId invalidNodeId() {
        return Core::Type<NodeId>::max;
    }
    static const EdgeId invalidEdgeId() {
        return Core::Type<EdgeId>::max;
    }
...
private:
    ...
    template<class T>
    class _OutgoingEdgeIterator : public AdjacentEdgeIterator<T> {
        typedef Graph::AdjacentEdgeIterator<T> Predecessor;
        typedef _OutgoingEdgeIterator          Self;

    public:
        _OutgoingEdgeIterator()
                : Predecessor(0){};
        _OutgoingEdgeIterator(T* e)
                : Predecessor(e){};
        Self& operator++() {
    ...
    template<class T>
    class _IncomingEdgeIterator : public AdjacentEdgeIterator<T> {
        typedef Graph::AdjacentEdgeIterator<T> Predecessor;
        typedef _IncomingEdgeIterator          Self;

    public:
        _IncomingEdgeIterator()
                : Predecessor(0){};
        _IncomingEdgeIterator(T* e)
                : Predecessor(e){};
        Self& operator++() {
...
public:
    ...
    public:
        ...
        EdgePointer outgoing() const {
            return outgoing_;
        }
        EdgePointer incoming() const {
            return incoming_;
        }
        ...
        int nIncomingEdges() const {
            int result = 0;
            for (EdgePointer e = incoming(); e; e = e->linkIncoming())
                ++result;
            return result;
        }
        ...
        int nOutgoingEdges() const {
            int result = 0;
            for (EdgePointer e = outgoing(); e; e = e->linkOutgoing())
                ++result;
            return result;
        }
    ...
    public:
        ...
        NodePointer source() const {
            return source_;
        }
        NodePointer target() const {
            return target_;
        }
        ...
        EdgePointer linkOutgoing() const {
            return linkOutgoing_;
        }
        EdgePointer linkIncoming() const {
            return linkIncoming_;
        }
...
private:
    ...
    public:
        ...
        void begin() {
            current_ = begin_;
            while (current_ < end_ && *current_ == 0)
                ++current_;
        }
        ...
        void end() {
            ...
            do
                --current;
            while (current_ >= begin_ && *current_ == 0);
        }
...
public:
    ...
    public:
        EdgeIterator(typename EdgeList::iterator b, typename EdgeList::iterator e)
                : EdgeIterator_(b, e) {}
        NodePointer currentNode() const {
            return item().source();
        }
    };
    ...
    public:
        const_EdgeIterator(typename EdgeList::const_iterator b, typename EdgeList::const_iterator e)
                : const_EdgeIterator_(b, e) {}
        NodePointer currentNode() const {
            return item().source();
        }
    };
...
public:
    GraphWithInitialFinals() {
        initial_ = 0;
    }

    typename Precursor::NodePointer initial() {
        return initial_;
    }
    const typename Precursor::NodePointer initial() const {
        return initial_;
    }
    void setInitial(typename Precursor::NodePointer n) {
        initial_ = n;
    }
    ...
    typename Precursor::NodeIterator finals() {
        return NodeIterator(finals_.begin(), finals_.end());
    }
    typename Precursor::const_NodeIterator finals() const {
        return const_NodeIterator(finals_.begin(), finals_.end());
    }
    bool isFinal(typename Precursor::NodePointer n) {}
    bool isFinal(const typename Precursor::NodeId& n) {}
    void addFinal(typename Precursor::NodePointer n) {
        finals_.push_back(n);
    }
    void addFinal(const typename Precursor::NodeId& id) {
        addFinal(nodes[id]);
    }
