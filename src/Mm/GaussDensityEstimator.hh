...
public:
    ...
    const Core::Ref<AbstractMeanEstimator> mean() const {
        return meanEstimator_;
    }

    void setCovariance(Core::Ref<AbstractCovarianceEstimator> estimator) {
        covarianceEstimator_ = estimator;
    }
    const Core::Ref<AbstractCovarianceEstimator> covariance() const {
        return covarianceEstimator_;
    }
...
public:
    ...
    const std::vector<Accumulator::SumType>& sum() const {
        return accumulator_.sum();
    }
    virtual Weight weight() const {
        return accumulator_.weight();
    }
...
public:
    typedef std::unordered_set<
            ...
            hashReference<AbstractMeanEstimator>>
            MeanSet;
