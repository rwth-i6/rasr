...
protected:
    ...
    bool firstRun() {
        return paramOldMixtureSetFilename(config).empty();
    }
...
public:
    ...
    void write() const {
        write(paramNewMixtureSetFilename(config));
    }
    void write(Core::XmlWriter& o, std::map<int, std::string>& mixtureLabels) const {
        verify(estimator_ != 0);
        estimator_->setMixtureLabels(mixtureLabels);
        estimator_->write(o);
    }

    size_t dimension() const {
        return estimator_ != 0 ? estimator_->dimension() : 0;
    }
    size_t nMixtures() const {
        return estimator_ != 0 ? estimator_->nMixtures() : 0;
    }

    bool map(const std::string& mappingFilename);
    void setWeightThreshold(Mm::Weight weightThreshold) {
        if (estimator_)
            estimator_->setWeightThreshold(weightThreshold);
    }
    void setAssigningFeatureScorer(
