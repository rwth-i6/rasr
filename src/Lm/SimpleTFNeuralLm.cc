...
SimpleTFNeuralLm::SimpleTFNeuralLm(const Core::Configuration& c, Bliss::LexiconRef l) : 
...
SimpleTFNeuralLm::~SimpleTFNeuralLm()
{
...
void SimpleTFNeuralLm::load() 
{
  ...
  // speed up: compute the scores at init already ? but not always needed (e.g. image building)
}

History SimpleTFNeuralLm::startHistory() const
{
...
History SimpleTFNeuralLm::extendedHistory(const History& h, Token w) const
{
...
Score SimpleTFNeuralLm::score(const History& h, Token w) const
{
...
void SimpleTFNeuralLm::makeBatch(HistoryDescriptor* hd) const
...
void SimpleTFNeuralLm::scoreBatch() const
{ // default initializer always 0 ? just explicitly pad 0 for safety
