...
//      totalInstr_(0),
    totalTime_( 0 ),
//      totalRealTime_(0),
//      instrStats(stats.statistics("Profiling: " + name + ": Instructions")),
    timeStats( stats.customStatistics( "Profiling: " + name + ": Centiseconds" ) )
//      , realTimeStats(stats.statistics("Profiling: " + name + ": Real Centiseconds"))
  {
    if( start )
      this->start();
  }
...
//      clock_gettime(CLOCK_REALTIME, &startrealtime_);
  ...
  void stop() {
    ...
    {
      running_ = false;
//          totalInstr_ += instructions() - startInstr_;
//          startInstr_ = 0;
...
/*          clock_gettime(CLOCK_REALTIME, &temp);
            temp = timeSpecDiff(startrealtime_, temp);
            totalRealTime_ += temp.tv_sec * 100 + temp.tv_nsec / 10000000.0f;*/
    }
  }
  ...
  void stopAndYield( bool print = false ) {
    stop();
//      instrStats += totalInstr_;
    timeStats += totalTime_;
//      realTimeStats += totalRealTime_;
    if( print )
      std::cout << " time: " << totalTime_ << std::endl;
//          std::cout << "Instructions: " << totalInstr_ << " time: " << totalTime_ << " real-time: " << totalRealTime_ << std::endl;
//      totalInstr_ = 0;
    totalTime_ = 0;
//      totalRealTime_ = 0;
  }
...
//     Core::Statistics<f32>& instrStats;
  Core::Statistics<f32>& timeStats;
//     Core::Statistics<f32>& realTimeStats;
};

inline f32 scaledLogAdd( f32 a, f32 b, f32 scale, f32 invertedScale ) {
  if ( b == Core::Type<f32>::max ) return a;
  if ( a == Core::Type<f32>::max ) return b;
  a *= invertedScale;
  b *= invertedScale;
...
{
public:
  AsymmetricIntersectionIterator( const std::vector<T>& array1, const std::vector<T>& array2 ) : a_( array1.size() < array2.size() ? array1 : array2 ), b_( array1.size() < array2.size() ? array2 : array1 ), ready_( false ) {
    stack_.reserve( 50 );
