...
struct Instance {
  Instance(const InstanceKey& _key, Instance* backOffParent) : key(_key),
                                                               inactive(0),
                                                               backOffInstance(0),
                                                               backOffScore(0),
...
struct EarlyWordEndHypothesis {
  ...
  EarlyWordEndHypothesis( TraceId _trace, const SearchAlgorithm::ScoreVector& _score, u32 _exit, PathTrace _pathTrace ) : trace( _trace ), score( _score ), exit( _exit ), pathTrace( _pathTrace ) {
  }
  EarlyWordEndHypothesis() : trace( 0 ), score( 0, 0 ), exit( 0 ) {
  }
};
