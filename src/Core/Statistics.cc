...
template<typename T>
const typename Statistics<T>::Value Statistics<T>::minInit = Type<T>::max;
template<typename T>
const typename Statistics<T>::Value Statistics<T>::maxInit = Type<T>::min;
...
void ChoiceStatistics::write(XmlWriter& os) const {
    ...
    os << XmlOpen("statistic") + XmlAttribute("name", name_) + XmlAttribute("type", "histogram")
       << XmlFull("count", total);
    for (std::vector<Value>::const_iterator v = values.begin(); v != values.end(); ++v) {
...
void Core::HistogramStatistics::writeArray(XmlWriter& os) const {
    os << Core::XmlOpen("statistic-array") + Core::XmlAttribute("name", name()) + Core::XmlAttribute("type", "scalar");
    if (nObservations()) {
        for (u32 i = 0; i < counts_.size(); ++i)
            if (counts_[i])
...
void Core::HistogramStatistics::write(XmlWriter& os) const {
    os << Core::XmlOpen("statistic") + Core::XmlAttribute("name", name()) + Core::XmlAttribute("type", "scalar");
    if (nObservations()) {
