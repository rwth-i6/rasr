...
public:
    NormalizeAutomaton(_ConstAutomatonRef f)
            : Precursor(f) {}
    virtual Fsa::StateId initialStateId() const {
        Fsa::StateId s = Precursor::fsa_->initialStateId();
        if (s != Fsa::InvalidStateId)
            return states_.insert(s);
        return s;
    }
    ...
    virtual std::string describe() const {
        return "normalize(" + Precursor::fsa_->describe() + ")";
    }
...
public:
    TrimAutomaton(_ConstAutomatonRef f, bool progress)
            : Precursor(f) {
        Core::ProgressIndicator* p = 0;
        if (progress)
            p = new Core::ProgressIndicator("connecting", "states");
    ...
    virtual Fsa::StateId initialStateId() const {
        Fsa::StateId s = Precursor::fsa_->initialStateId();
        if ((s < accAndCoacc_.size()) && (accAndCoacc_[s]))
            return s;
...
public:
    ...
    virtual _ConstStateRef getState(Fsa::StateId s) const {
        ...
        _State*            sp     = (fromSp->isFinal()) ? new _State(s, fromSp->tags(), map_(fromSp->weight())) : new _State(s, fromSp->tags(), semiring()->one());
