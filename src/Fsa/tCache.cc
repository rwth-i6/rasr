...
public:
    ...
    virtual _ConstStateRef getState(Fsa::StateId s) const {
        ...
        states_.grow(s);
        if (!states_[s])
            states_[s] = Precursor::fsa_->getState(s);
        ages_.grow(s, 0);
        ages_[s] |= 0x80;
        return states_[s];
    }
    virtual size_t getMemoryUsed() const {
        ...
        for (typename Core::Vector<_ConstStateRef>::iterator i = states_.begin(); i != states_.end(); ++i)
            if ((*i) && ((*i)->refCount() == 1))
                size += (*i)->getMemoryUsed();
        return Precursor::fsa_->getMemoryUsed() + 2 * sizeof(u32) + ages_.capacity() * sizeof(u8) + states_.capacity() * sizeof(_ConstStateRef) + size;
    }
...
Core::Ref<const _Automaton> cache(Core::Ref<const _Automaton> f, u32 maxAge) {
    if (f->hasProperty(Fsa::PropertyCached | Fsa::PropertyStorage))
        return f;
