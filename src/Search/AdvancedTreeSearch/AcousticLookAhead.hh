...
public:
  ...
  struct ApplyPreCachedLookAheadForId {
    ApplyPreCachedLookAheadForId( const AcousticLookAhead& lah ) : lah_( lah ) {
    }
...
// Pairs of time-frame and score (only current if same timeframe)
  ...
  struct CacheForTimeframe {
    ...
    CacheForTimeframe( int maxDepth ) : maxDepth_( maxDepth ), simpleCacheTimeframe_( -1 ) {
      clear();
    }

    struct CacheValue {
      CacheValue() : timeframe( -1 ), cacheKey( 0 ), score( 0.0f ) {
      }
      CacheValue( int t, u32 c, Score s ) : timeframe( t ), cacheKey( c ), score( s ) {
      }
      int timeframe;
      u32 cacheKey;
      Score score;
