...
private:
    struct PhoneBoundaryStateSetDescriptor {
        ...
        bool operator==(const PhoneBoundaryStateSetDescriptor& r) const {
            return (phoneBoundaryFlag == r.phoneBoundaryFlag) && (isCoarticulated == r.isCoarticulated);
        }
        struct Hash {
    ...
    typedef std::unordered_map<PhoneBoundaryStateSetDescriptor, u32,
                               PhoneBoundaryStateSetDescriptor::Hash>
            NumberOfPhoneBoundaryStates;

private:
    u32& nPhoneBoundaryStates(const PhoneBoundaryStateDescriptor& pbsd) {
        return nPhoneBoundaryStates_.insert(std::make_pair(
                                                    PhoneBoundaryStateSetDescriptor(pbsd), 0))
                .first->second;
    }
...
Core::XmlOpen ClassicTransducerBuilder::Statistics::PhoneBoundaryStateSetDescriptor::xmlAttributes(
    ...
    return Core::XmlOpen(xmlOpen) + Core::XmlAttribute("boundary", phoneBoundaryValue) + Core::XmlAttribute("coarticulated", isCoarticulatedValue);
}
...
void ClassicTransducerBuilder::Statistics::writeNumberOfPhoneBoundaryStates(
    ...
    for (NumberOfPhoneBoundaryStates::const_iterator i =
                 nPhoneBoundaryStates_.begin();
         i != nPhoneBoundaryStates_.end(); ++i) {
...
static const Core::ParameterBool paramFilterOutInvalidAllophones(
        "transducer-builder-filter-out-invalid-allophones",
        "Enables filterOutInvalidAllophones for the ClassicTransducerBuilder. "
        "This is preferred but it was not available earlier and it changes the behavior "
        "of the allophone-state-to-phoneme transducer thus it is disabled by default.",
        false);

static const Core::ParameterBool paramFixAllophoneContextAtWordBoundaries(
        "fix-allophone-context-at-word-boundaries",
        "The old behavior would allow sth like .. ng{..+#}@f r{#+..}@i .., "
        "i.e. it looses the context without having silence in between the words. "
...
void ClassicTransducerBuilder::setupWordStart(Fsa::State* s, const PhoneBoundaryStateDescriptor& pbsd) {
    if (pbsd.context.history.empty() && pbsd.context.future.empty() && pbsd.flag == wordStart)
        // This should be unique.
        product_->setInitialStateId(s->id());
    if (pbsd.context.future.empty() && !acceptCoarticulatedSinglePronunciation_) {
...
Fsa::State* ClassicTransducerBuilder::phoneStartState(
        const Allophone* allophone, bool looseCtxMark) {
    ...
    if (allophone->boundary & Allophone::isInitialPhone) {
        pbsd.flag |= wordStart;
        if (!pbsd.context.history.empty() && acceptCoarticulatedSinglePronunciation_)
...
Fsa::State* ClassicTransducerBuilder::phoneEndState(const Allophone* allophone) {
    ...
    if (allophone->boundary & Allophone::isFinalPhone) {
        ...
        if (!pbsd.context.future.empty() && acceptCoarticulatedSinglePronunciation_)
            pbsd.context.future.clear();
        if (fixAllophoneContextAtWordBoundaries_ && pbsd.context.future.empty() && model_->phonemeInventory()->phoneme(allophone->phoneme(0))->isContextDependent())
            // A context dependend allophone which has future context empty must either be the sequence end
            // or followed by a context independent allophone (like silence).
            pbsd.flag |= loosesCtx;
    }
    /**
     *  Note: non-coarticulated phonemes mostly occur as initial/final phoneme (e.i. word start/end)
...
void ClassicTransducerBuilder::buildAllophone(const Allophone* allophone) {
    if (filterOutInvalidAllophones_) {
        // Filter out some invalid allophones.
        // They might occur if you use *.allophones.add-all=true.
        // Note that this is important for a proper allophone-state-to-phoneme-transducer,
        // e.g. consider reading:r eh ..., the transducer would have
        // r{#+#}@i:r eh{#+#}@i:eh ... and r{#+eh}:r eh{r+.}:eh ...
        // and thus will accept an invalid allophone sequence.
        if (model_->phonemeInventory()->phoneme(allophone->phoneme(0))->isContextDependent()) {
            if (model_->phonology()->maximumHistoryLength() > 0 && allophone->context().history.empty())
                if (!(allophone->boundary & Allophone::isInitialPhone))
                    return;
            if (model_->phonology()->maximumFutureLength() > 0 && allophone->context().future.empty())
                if (!(allophone->boundary & Allophone::isFinalPhone))
                    return;
        }
    }
    ...
    buildAllophone(allophone, phoneStartState(allophone, false), phoneEndState(allophone));
    if (fixAllophoneContextAtWordBoundaries_ && (allophone->boundary & Allophone::isInitialPhone) && !model_->phonemeInventory()->phoneme(allophone->phoneme(0))->isContextDependent()) {  // e.g. silence
        require(allophone->context().history.empty());
        require(allophone->context().future.empty());
        buildAllophone(allophone, phoneStartState(allophone, true), phoneEndState(allophone));
    }
}
...
size_t ClassicTransducerBuilder::hashSequence(const AllophoneSuffix& as) {
    ...
    for (s16 state = as.allophoneState.state(); state < as.hmmTopology->nPhoneStates(); ++state) {
...
void ClassicTransducerBuilder::buildAllophoneStates(const Allophone* allophone,
                                                    Fsa::State* start, Fsa::State* end) {
    verify((product_->outputAlphabet() == phonemes_) || (product_->outputAlphabet() == allophones_));
    verify((product_->inputAlphabet() == allophoneStates_) || (product_->inputAlphabet() == emissions_));
    ...
    while (from) {
        ...
        if (++as.subState >= as.hmmTopology->nSubStates()) {
            ...
            as.allophoneState = allophoneStates_->allophoneState(
                    as.allophoneState.allophone(), as.allophoneState.state() + 1);
        }
...
void ClassicTransducerBuilder::buildPronunciation(const Bliss::Coarticulated<
    ...
    if (p.length() == 1) {
        Allophone allophone(phonology(p, 0), Allophone::isInitialPhone | Allophone::isFinalPhone);
    ...
    else if (p.length() > 1) {
        {
            ...
            if (leftContext != Bliss::Phoneme::term && model_->isAcrossWordModelEnabled() && phonemeInventory.phoneme(initialAllophone.phoneme())->isContextDependent()) {
...

        {
            ...
            if (rightContext != Bliss::Phoneme::term && model_->isAcrossWordModelEnabled() && phonemeInventory.phoneme(finalAllophone.phoneme())->isContextDependent()) {
...
Fsa::ConstAutomatonRef ClassicTransducerBuilder::applyTransitionModel(
    ...
    require((ff->inputAlphabet() == allophoneStates_) || (ff->inputAlphabet() == emissions_));
            ...
            for (Fsa::State::iterator a = sp->begin(); a != sp->end(); ++a) {
                if (a->input() != Fsa::Epsilon) {
                    if (!allophoneStates_->isDisambiguator(a->input()))
                        ...
                        a->input_ = emissions_->disambiguator(a->input() - allophoneStates_->nClasses() - 1);
                }
            }
        }
        pi.notify(s);
...
void ClassicTransducerBuilder::finalize() {
     ...
     */
#if 0
    Core::XmlChannel statistics(model_->config, "statistics");

    if (statistics.isOpen()) {
        statistics << "linear state model transducer:" << "\n";
        Fsa::info(product_, statistics);
    }
#endif

#if 0
    if (!nDisambiguators_)
    product_ = Fsa::removeEpsilons(product_);
#endif

    if (shouldApplyTransitionModel_)
        product_ = Fsa::staticCopy(applyTransitionModel(product_));
#if 0
    if (inputType_ == inputAcceptsEmissionLabels)
    applyStateTying();
#endif
    product_ = Fsa::staticCompactCopy(Fsa::trim(product_, true));
#if 0
    if (statistics.isOpen()) {
        statistics << "state model transducer:" << "\n";
        Fsa::info(product_, statistics);
    }
#endif
}
...
typedef std::unordered_map<MinimizedState, Fsa::State*, MinimizedState::Hash,
                           MinimizedState::Equality>
        MinimizedStateMap;
}  // namespace
...
Fsa::ConstAutomatonRef ClassicTransducerBuilder::createMinimizedContextDependencyTransducer(
    ...
    std::unordered_map<Bliss::Phoneme::Id, const Allophone*> nonCoartPhones;
    ...
    for (AllophoneAlphabet::AllophoneList::const_iterator ai =
        ...
        const Bliss::Phoneme* phoneme = model_->phonemeInventory()->phoneme(
                a->phoneme());
        if (!phoneme->isContextDependent() && ((*ai)->boundary & (Allophone::isInitialPhone | Allophone::isFinalPhone)))
    ...
    if (model_->isAcrossWordModelEnabled()) {
        ...
        // non-coart phones
        for (std::unordered_map<Bliss::Phoneme::Id, const Allophone*>::const_iterator
                     i = nonCoartPhones.begin();
             i != nonCoartPhones.end(); ++i)
            initial->newArc(initial->id(), semiring->one(), allophones_->index(i->second), initialPhoneAndSilenceOffset + Fsa::LabelId(i->first));
    }
    ...
    for (AllophoneAlphabet::AllophoneList::const_iterator ai =
        ...
        if (model_->isAcrossWordModelEnabled()) {
            ...
            if ((*ai)->boundary & Allophone::isFinalPhone) {
                ...
                if (((*ai)->future().size() == 0) || ((*ai)->future()[0] == Bliss::Phoneme::term)) {
                    ...
                    if ((*ai)->central() != silenceAllophone->central()) {
                        ...
                        sp->newArc(sFrom, semiring->one(), allophones_->index(*ai), silenceAllophone->central());
                }
                else
            ...
            Fsa::StateId sFrom = stateMap.insert(PhoneContext(phones.begin(),
                                                              phones.end() - 1, (*ai)->boundary & Allophone::isInitialPhone));
            if (!_c->state(sFrom))
                _c->setState(new Fsa::State(sFrom));
            Fsa::State* sp = _c->state(sFrom).get();
    ...
    for (std::vector<Bliss::Phoneme::Id>::const_iterator p =
                 initialNonCoartPhones.begin();
         p != initialNonCoartPhones.end(); ++p) {
        PhoneContext pc(maxHistory + maxFuture, true);
        pc[maxHistory] = Fsa::LabelId(*p);
        ...
        */
            ...
            for (std::vector<NoCoartBoundary>::const_iterator wb =
                         nonCoartBoundaries.begin();
                 wb != nonCoartBoundaries.end(); ++wb) {
