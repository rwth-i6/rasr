...
Score GaussDiagonalMaximumFeatureScorer::distance(const std::vector<FeatureType>&  feature,
    ...
    ComponentIndex eff_dim = dimension & (~3u);  // == dimension - dimension % 4
    while (cmp < eff_dim) {
        __m128 m    = _mm_load_ps(mean.data() + cmp);
        __m128 f    = _mm_load_ps(feature.data() + cmp);
        __m128 isrv = _mm_load_ps(inverseSquareRootVar.data() + cmp);
        __m128 df   = (m - f) * isrv;
        sum += df * df;
        cmp += 4u;
    }
    ...
    switch (dimension - eff_dim) {
        case 3:
            df = (mean[cmp] - feature[cmp]) * inverseSquareRootVar[cmp];
            result += df * df;
            ++cmp;
        case 2:
            df = (mean[cmp] - feature[cmp]) * inverseSquareRootVar[cmp];
            result += df * df;
            ++cmp;
        case 1: df = (mean[cmp] - feature[cmp]) * inverseSquareRootVar[cmp]; result += df * df;
    }
#else
...
GaussDiagonalSumFeatureScorer::GaussDiagonalSumFeatureScorer(const Core::Configuration&  c,
                                                             Core::Ref<const MixtureSet> mixtureSet)
        : Core::Component(c),
...
void GaussDiagonalSumFeatureScorer::calculateScoresAndNumberOfDensities(const CachedAssigningContextScorer* cs,
    ...
    for (size_t dns = 0; dns < nDensities_; ++dns) {
        ...
        Score score = mixture.minus2LogWeights()[dns] +
                      ...
                      distance(c->featureVector_, meanTable_[density.meanIndex()], covariance.inverseSquareRootDiagonal());

        scores_[dns] = 0.5 * score;
    }
}

AssigningFeatureScorer::ScoreAndBestDensity GaussDiagonalSumFeatureScorer::calculateScoreAndDensity(const CachedAssigningContextScorer* cs,
                                                                                                    MixtureIndex                        mixtureIndex) const {
...
void GaussDiagonalSumFeatureScorer::calculateDensityPosteriorProbabilities(const CachedAssigningContextScorer* cs, Score logDenominator,
