...
const Core::ParameterBool SegmentwiseNnTrainer<T>::paramEnableFeatureDescriptionCheck(
        ...
        "the size of the input (does not work reliably on models with multiple input streams)", true);
...
void SegmentwiseNnTrainer<T>::processWordLattice(Lattice::ConstWordLatticeRef lattice, Bliss::SpeechSegment *segment){
    ...
    if (NeuralNetworkTrainer<f32>::weightedAccumulation_) weights_.initComputation();      // move to GPU
...
{
    if (!enableFeatureDescriptionCheck_) return;
    if (featureDescriptionNeedInit_) {
        ...
        if (network().getLayer(0).getInputDimension(0) != nFeatures) {
            this->error("mismatch in dimension: ") <<
                    network().getLayer(0).getInputDimension(0) << " (neural network input dimension) vs. " <<
                    nFeatures << " (dimension of flow features)";
        }
        featureDescriptionNeedInit_ = true;
    } else {
...
void SegmentwiseNnTrainer<T>::backpropagateError(){
    ...
    for (u32 layer = 0; layer < lastLayer; layer++) errorSignal_.at(layer).setToZero();

    // error backpropagation
    for (s32 layer = (s32)lastLayer; layer > (s32)network().lowestTrainableLayerIndex(); layer--) {
        std::vector<NnMatrixf32*> errorSignalOut;
        for (u32 i = 0; i < network().getLayer(layer).nPredecessors(); i++) errorSignalOut.push_back(&(errorSignal_[network().getLayer(layer).getPredecessor(i)]));
...
void SegmentwiseNnTrainer<T>::logTrainingStatistics() const {
    ...
    if (estimator().fullBatchMode() && statistics_){
        ...
        T totalObjectiveFunction = (1.0 - ceSmoothingWeight_) * statistics_->objectiveFunction() / numberOfProcessedSegments_
                + ceSmoothingWeight_ * ceObjectiveFunction_ / numberOfProcessedSegments_;
...
void SegmentwiseNnTrainer<T>::accumulateStatisticsOnLattice(Fsa::ConstAutomatonRef posteriorFsa,
        Core::Ref<const Lattice::WordBoundaries> wordBoundaries, Mm::Weight factor){
...
T SegmentwiseNnTrainer<T>::smoothErrorSignalWithCE(){
    ...
    if (ceSmoothingWeight_ > 0.0) {
        if (!maxoutLayer_) {                                                                       // simple linear+softmax
            ...
            if (priorScale_ > 0) network().getTopLayerOutput().addToAllColumns(prior_, priorScale_); // add prior, previously assumed: priors was merged in bias, output was ~likelihood
            ...
            if (priorScale_ > 0) network().getLayerOutput(maxoutLayer_->getPredecessor(0)).expandAddToAllColumns(maxoutLayer_->getMixture(), maxoutLayer_->getOffset(), prior_, priorScale_);       // add state prior to all hidden variable per state (maximum approx)
            topLayer_->applySoftmax(network().getLayerOutput(maxoutLayer_->getPredecessor(0)));                                                                                                     // and apply softmax
            network().getTopLayerOutput().maxoutvar(maxoutLayer_->getMixture(), maxoutLayer_->getOffset(), network().getLayerOutput(maxoutLayer_->getPredecessor(0)), maxoutLayer_->getMaxindex()); // and redo the maxoutvar (but index should remain the same)
            ...
            if (statistics_->hasGradient()){
                ...
                errorSignal_[maxoutLayer_->getPredecessor(0)].add(network().getLayerOutput(maxoutLayer_->getPredecessor(0)), ceSmoothingWeight_);                                                   // add the CE error signal
                errorSignal_[maxoutLayer_->getPredecessor(0)].addKroneckerDelta(alignment_, maxoutLayer_->getOffset(), maxoutLayer_->getMaxindex(), -ceSmoothingWeight_);                           // hard target is the (maximal) hidden variable of the target state, maximum approximation
            }
        }
