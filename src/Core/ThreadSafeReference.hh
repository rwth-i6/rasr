...
protected:
    explicit ThreadSafeReferenceCounted(u32 rc)
            : referenceCount_(rc) {}
...
public:
    ...
    ThreadSafeReferenceCounted(const ThreadSafeReferenceCounted&)
            : referenceCount_(0) {}
    ThreadSafeReferenceCounted& operator=(const ThreadSafeReferenceCounted&) {
        return *this;
    }
...
protected:
     ...
     */
    void set(Object* o) {
        // it might happen that the object we get is also referenced by the current object
        // thus we need to increment the ref counter on the new object first, only then
        // unreference the old object, this happens for example in Search::SearchSpace::pruneSilenceSiblings
        if (object_ != o) {
            referenceObject(o);
            unreference();
            object_ = o;
...
public:
    ...
    template<class S>
    TsRef& operator=(const TsRef<S>& rhs) {
        set(rhs._get());
        return *this;
    }
    ...
    template<class S>
    bool operator==(const TsRef<S>& r) const {
