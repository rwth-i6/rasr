...
namespace Signal {
    ...
    private:
        ...
        void equalizeQuantiles(Frame &out) const {
            if (piecewiseLinear_){
                ...
                for(u16 d = 0; d < dim(); d++){
                    maximalQuantile = std::max(
                        overestimationFactor_*trainingQuantile_[numberOfQuantiles_*dim()+d],
        ...
        void combineNeighbors(Frame &out) const {
            ...
            for(u16 d = 0; d < dim(); d++)
                (*tmp)[d] = (1.-lambda_[d]-rho_[d]) * (*out)[d] + lambda_[d] * (*out)[std::max((s32)d-1,0)]
                    + rho_[d] * (*out)[std::min((s32)d+1,(s32)dim()-1)];
            for(u16 d = 0; d < dim(); d++)
                (*out)[d] = (*tmp)[d];
        }
        void normalizeMean(Frame &out) const {
            for(u16 d = 0; d < dim(); d++) (*out)[d] -= mean_[d];
        }
        void normalizeVariance(Frame &out) const {
            for(u16 d = 0; d < dim(); d++) (*out)[d] /= dev_[d];
        }
    ...
    protected:
        ...
        u32 dim() const { return mean_.size(); };

    public:
        QuantileEqualization() :
            length_(0), right_(0), numberOfQuantiles_(0),
            changed_(true),
            equalizeQuantiles_(true), combineNeighbors_(false),normalizeMean_(true), normalizeVariance_(false),
            needInit_(true) {}

        virtual ~QuantileEqualization() { reset(); }

        void setQuantileEqualization(bool norm) {
            if (equalizeQuantiles_ != norm) { equalizeQuantiles_ = norm; reset(); }
        }
        void setCombineNeighbors(bool norm) {
            if (combineNeighbors_ != norm) { combineNeighbors_ = norm; reset(); }
        }
        void setPoolQuantiles(bool norm) {
            if (poolQuantiles_ != norm) { poolQuantiles_ = norm; reset(); }
        }
        void setPiecewiseLinear(bool norm) {
            if (piecewiseLinear_ != norm) { piecewiseLinear_ = norm; reset(); }
        }
        void setQuantileEstimation(bool norm) {
            if (estimateQuantiles_ != norm) { estimateQuantiles_ = norm; reset(); }
        }
        void setQuantileFile(std::string filename) {
            if (filename_ != filename) { filename_ = filename; reset(); }
        }
        void setNormalizeMean(bool norm) {
            if (normalizeMean_ != norm) { normalizeMean_ = norm; reset(); }
        }
        void setNormalizeVariance(bool norm) {
            if (normalizeVariance_ != norm) { normalizeVariance_ = norm; reset(); }
        }
        void setLength(u32 length) {
            if (length_ != length) { length_ = length; reset(); }
        }
        void setRight(u32 right) {
            if (right_ != right) { right_ = right; reset(); }
        }
        void setNumberOfQuantiles(u32 numberOfQuantiles) {
            if (numberOfQuantiles_ != numberOfQuantiles) { numberOfQuantiles_ = numberOfQuantiles; reset(); }
        }
        void setOverestimationFactor(f32 overestimationFactor) {
            if (overestimationFactor_ != overestimationFactor) { overestimationFactor_ = overestimationFactor; reset(); }
        }
        void setDeltaAlpha(f32 deltaAlpha) {
            if (deltaAlpha_ != deltaAlpha) { deltaAlpha_ = deltaAlpha; reset(); }
        }
        void setDeltaGamma(f32 deltaGamma) {
            if (deltaGamma_ != deltaGamma) { deltaGamma_ = deltaGamma; reset(); }
        }
        void setDeltaLambdaAndRho(f32 deltaLambdaAndRho) {
            if (deltaLambda_ != deltaLambdaAndRho) { deltaLambda_ = deltaLambdaAndRho; reset(); }
            if (deltaRho_ != deltaLambdaAndRho) { deltaRho_ = deltaLambdaAndRho; reset(); }
        }
        void setBeta(f32 beta) {
            if (beta_ != beta) { beta_ = beta; reset(); }
        }
        bool update(const Frame &in, Frame &out);
        bool flush(Frame &out) { if (needInit_) return false; return update(Frame(), out); }

        virtual void reset() { needInit_ = true; }
    };
    ...
    public:
        static std::string filterName() { return "signal-quantile-equalization"; }
        ...
        virtual void reset() { QuantileEqualization::reset(); }
