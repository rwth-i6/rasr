...
public:
    virtual ConstStateRef getState(StateId s) const {
        return ConstStateRef(state_);
    }
    virtual std::string describe() const {
        return std::string("nBestAutomaton()");
    }
...
public:
    ...
    virtual ConstStateRef getState(StateId s) const {
        return Fsa::ConstStateRef(statemap_.find(s)->second);
    }
    virtual std::string describe() const {
        return std::string("reorderAutomaton(" + fsa_->describe() + ")");
    }
    virtual StateId initialStateId() const {
        return 0;
    }
...
ConstAutomatonRef reorderNbest(std::string sourceSentence, std::string targetSentence, const TranslationLexicon* lex, u32 n = 1) {
    return projectInput(nbest(composeMatching(ConstAutomatonRef(staticCopy(numberTokens(sourceSentence), TropicalSemiring)),
                                              ConstAutomatonRef(new nBestAutomaton(sourceSentence, targetSentence, lex))),
                              n,
                              false));
};

ConstAutomatonRef nbestReorder(std::string sourceSentence, std::string targetSentence, const TranslationLexicon* lex, u32 n = 1) {
    return ConstAutomatonRef(new ReorderAutomaton(nbest(composeMatching(ConstAutomatonRef(staticCopy(numberTokens(sourceSentence), TropicalSemiring)),
                                                                        ConstAutomatonRef(new nBestAutomaton(sourceSentence, targetSentence, lex))),
                                                        n,
                                                        false)));
}
