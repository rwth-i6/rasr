...
struct ForwardBackwardAlignment {
    struct Word {
        Word() : amScore(Core::Type<Score>::max), lmScore(Core::Type<Score>::max) {}
...
protected:
    ...
    std::pair<Flf::ConstLatticeRef, Flf::ConstLatticeRef> recognizeFeatures(const std::deque<FeatureRef>& features)
    {
        ...
        if (meshRescoring_)
        {
            ...
            if (meshCombination_)
                ...
                ret = best(decodeRescoreLm(meshLattice, mc_->languageModel(), recognizer_->describePruning()->masterBeam(), rescoreWordEndLimit_, c.prefix, c.suffix), BellmanFord);
            if (expandTransits_)
    ...
    struct RecognizedSequence {
        RecognizedSequence() : extension(Flf::IncrementalRecognizer::ExtendNone) {}
        Extension extension;
    ...
    RecognizedSequence refine(int offset, const std::deque<FeatureRef>& features, const Bliss::LemmaPronunciation* forceFirstWord, const Bliss::LemmaPronunciation* forceLastWord, const std::vector<const Bliss::Lemma*>& lmPrefix, const std::vector<const Bliss::Lemma*>& lmSuffix, bool isInitial = false, bool isSecond = false)
    {
        ...
        std::cout << "forward-backward refinement on " << offset << " -> " << offset + features.size() << " (enforce " << (forceFirstWord ? forceFirstWord->lemma()->preferredOrthographicForm().str() : "*") << ":" << (forceLastWord ? forceLastWord->lemma()->preferredOrthographicForm().str() : "*") << ") with pruning " << ret.pruning->format() << std::endl;
        ...
        if (forwardMesh)
        {
            ...
            // Debug
//              std::cout << "Debug reversing" << std::endl;
//              std::vector<MeshEntry> reverseTest;
//              e.lattice = backwardMesh;
//              reverseTest.push_back(e);
//              Search::SearchAlgorithm::RecognitionContext c = recognizer_->setContext(Search::SearchAlgorithm::RecognitionContext());
//              recognizer_->setContext(c);
//              forward = best(decodeRescoreLm(mesh(reverseTest), mc_->languageModel(), recognizer_->describePruning()->masterBeam()*2, rescoreWordEndLimit_, c.prefix, c.suffix), BellmanFord);
        }

        timer.stop();
        if(verboseRefinement_)
            std::cout << "local needed time: " << timer.user() << " for duration: " << (features.size() / 100.0) << " local RTF: " << timer.user() / (features.size() / 100.0) << std::endl;

        ForwardBackwardAlignment alignment(features.size(), forward, backward, lmContextLength_, lpAlphabet_, correctForceEqualAlignment_, correctForceEqualScore_ || (correctStrictInitial_ && isInitial), correctIncludeNoise_ || (correctStrictInitial_ && isInitial), verboseRefinement_);
...

        {
            ...
            for(std::vector<const Bliss::Lemma*>::const_reverse_iterator it = lmSuffix.rbegin(); it != lmSuffix.rend(); ++it)
                Lm::addLemmaScore(backwardRecognizer_->mc_->languageModel(), *it, backwardRecognizer_->mc_->languageModel()->scale(), basicBackwardHistory, basicBackwardScore);
            ...
            for(std::deque<ForwardBackwardAlignment::Word>::const_iterator it = alignment.forWords.begin(); it != alignment.forWords.end(); ++it)
            {
                ...
                Lm::addLemmaScore(mc_->languageModel(), it->pron->lemma(), mc_->languageModel()->scale(), forwardHypothesisForwardHistory, forwardHypothesisCentralForwardLMScore);
                if(forwardHypothesisCentralForwardLMScore - old >= 0 && std::abs((forwardHypothesisCentralForwardLMScore - old) - it->lmScore) > NUMERICAL_TOLERANE && verboseRefinement_)
                    std::cout << "WARNING: forward word score " << forwardHypothesisCentralForwardLMScore - old << " and alignment score alignment " << it->lmScore << " differ" << std::endl;

                if(verboseRefinement_)
                    std::cout << "forward " << " (" << it->start << " -> " << it->end << "): " << it->pron->lemma()->preferredOrthographicForm() << " " << it->pron->id() << " [ am " <<  it->amScore << ", lm " << it->lmScore << " ]" << " actual " << forwardHypothesisCentralForwardLMScore - old << std::endl;
            }
            ...
            for(std::deque<ForwardBackwardAlignment::Word>::const_reverse_iterator it = alignment.forWords.rbegin(); it != alignment.forWords.rend(); ++it)
            {
                ...
                Lm::addLemmaScore(backwardRecognizer_->mc_->languageModel(), it->pron->lemma(), backwardRecognizer_->mc_->languageModel()->scale(), forwardHypothesisBackwardHistory, forwardHypothesisBackwardScore);
                if(verboseRefinement_)
                    std::cout << "backward LM score component for " << it->pron->lemma()->preferredOrthographicForm() << ": " << forwardHypothesisBackwardScore - old << std::endl;
            }

            for(std::vector<const Bliss::Lemma*>::const_reverse_iterator it = lmPrefix.rbegin(); it != lmPrefix.rend(); ++it)
                Lm::addLemmaScore(backwardRecognizer_->mc_->languageModel(), *it, backwardRecognizer_->mc_->languageModel()->scale(), forwardHypothesisBackwardHistory, forwardHypothesisBackwardScore);
            ...
            for(std::deque<ForwardBackwardAlignment::Word>::const_iterator it = alignment.backWords.begin(); it != alignment.backWords.end(); ++it)
            {
                ...
                Lm::addLemmaScore(mc_->languageModel(), it->pron->lemma(), mc_->languageModel()->scale(), backwardHypothesisForwardHistory, backwardHypothesisForwardScore);

                if(verboseRefinement_)
                    std::cout << "forward LM score component for " << it->pron->lemma()->preferredOrthographicForm() << ": " << backwardHypothesisForwardScore - old << std::endl;
            }
            ...
            for(std::deque<ForwardBackwardAlignment::Word>::const_reverse_iterator it = alignment.backWords.rbegin(); it != alignment.backWords.rend(); ++it)
            {
                ...
                Lm::addLemmaScore(backwardRecognizer_->mc_->languageModel(), it->pron->lemma(), backwardRecognizer_->mc_->languageModel()->scale(), backwordHypothesisBackwardHistory, backwardHypothesisCentralLMScore);

                if(backwardHypothesisCentralLMScore - old >= 0 && std::abs((backwardHypothesisCentralLMScore - old) - it->lmScore) > NUMERICAL_TOLERANE)
                {
                    if(verboseRefinement_)
                        std::cout << "WARNING: backward word score " << (backwardHypothesisCentralLMScore - old) << " differs from score in alignment: " << it->lmScore << std::endl;
                }

                if(verboseRefinement_)
                    std::cout << "backward " << " (" << it->start << " -> " << it->end << "): " << it->pron->lemma()->preferredOrthographicForm() << " " << it->pron->id() << " [ am " <<  it->amScore << ", lm " << it->lmScore << " ]" << " actual " << backwardHypothesisCentralLMScore - old << std::endl;
            ...
            for(std::vector<const Bliss::Lemma*>::const_reverse_iterator it = lmPrefix.rbegin(); it != lmPrefix.rend(); ++it)
                Lm::addLemmaScore(backwardRecognizer_->mc_->languageModel(), *it, backwardRecognizer_->mc_->languageModel()->scale(), backwordHypothesisBackwardHistory, backwardHypothesisBackwardScore);
            ...
            if (std::abs(forwardHypothesisForwardScore - forwardHypothesisBackwardScore) > NUMERICAL_TOLERANE) {
                std::cout << "WARNING: forward and backward score of forward hypothesis differ: " << forwardHypothesisForwardScore << " vs. " << forwardHypothesisBackwardScore << " difference " << std::abs(forwardHypothesisForwardScore - forwardHypothesisBackwardScore) << std::endl;
                scoreMismatch = true;
            }

            if (std::abs(backwardHypothesisForwardScore - backwardHypothesisBackwardScore) > NUMERICAL_TOLERANE) {
                std::cout << "WARNING: forward and backward score of forward hypothesis differ: " << backwardHypothesisForwardScore << " vs. " << backwardHypothesisBackwardScore << " difference " << std::abs(forwardHypothesisForwardScore - forwardHypothesisBackwardScore) << std::endl;
                scoreMismatch = true;
            }
        ...
        if(ranges.size() && scoreThresholdSatisfied)
        {
            ...
            if(verboseRefinement_)
            {
                if(completeForwardScore == completeBackwardScore)
                    ...
                    std::cout << "ACCEPTING diverging result because the score difference between forward and backward pass is below the threshold: " << std::abs(completeForwardScore - completeBackwardScore) / (features.size() * 0.01f) << " <= " << scoreTolerance_ << std::endl;
            }
        }
        ...
        while(restart)
        {
            ...
            for(std::set<ForwardBackwardAlignment::Range>::iterator rangeIt = ranges.begin(); rangeIt != ranges.end(); ++rangeIt)
            {
                ...
                Search::SearchAlgorithm::RecognitionContext forwardContext, backwardContext;
...

        {
            ...
            while(forwardState->nArcs())
            {
                ...
                {
                    // Insert new lattice
//                      std::cout << "is initial: " << (currentState->id() == newLattice->initialStateId()) << std::endl;
...
//                          std::cout << "adding boundary with time " << boundary.time() << std::endl;
                    ...
                    newBoundaries->set(currentState->id(), forward->boundary(forwardState->id()));
//                      std::cout << "copying boundary with time " << forward->boundary(forwardState->id()).time() << std::endl;
                    currentState->setWeight(forwardState->weight());
                    currentState->setTags(forwardState->tags());
                }
            }
    ...
    ConstLatticeRef postProcess(ConstLatticeRef l)
    {
        ...
        if (addConfidenceScores_ || fwdBwdThreshold_ >= 0 || minArcsPerSecond_ || maxArcsPerSecond_ < Core::Type<f32>::max) {
            ...
            if (fwdBwdThreshold_ >= 0 || minArcsPerSecond_ || maxArcsPerSecond_ < Core::Type<f32>::max) {
                l = pruneByFwdBwdScores(l, fb, fwdBwdThreshold_ < 0 ? (fb->max() - fb->min()) : fwdBwdThreshold_, minArcsPerSecond_, maxArcsPerSecond_);
...
public:
    ...
    void startRecognition(const Bliss::SpeechSegment *segment, bool useDataSource = true) {
        ...
        if(!useDataSource)
        {
            ...
            {           /// @todo Also check compatibility when using the backward-recognizer. However we need all features.
                FeatureRef feature;
    ...
    std::pair<ConstLatticeRef, ConstSegmentRef> recognize() {
        ...
        if(backwardRecognizer_.get())
        {
            ...
            if (preCacheAllFrames_) {
                struct PreCacher : public Search::SearchAlgorithm {
                    PreCacher() : SearchAlgorithm(Core::Configuration()), Core::Component(Core::Configuration()) {}
                        ...
                        if (minArcsPerSecond_) {
                            ...
                            if(counts.nArcs_ < minArcs)
                            {
                                if (i > maxLatticeRegenerations_) {
                                    log() << "not enough arcs: " << counts.nArcs_ << " need at least " << minArcs << ", but NOT regenerating lattice because maximum number of regenerations is already reached (" << i << ")";
                                    break;
                                }else{
            ...
            if(features.size()) {
                std::cout << "global needed time: " << globalTimer_.user() << " for frame-duration: " << (features.size() / 100.0) << " global RTF: " << globalRtf() << " (postprocessing RTF " << (globalTimer_.user() - postProcessingStartTime) / (features.size() / 100.0) << " forward-backward RTF " << refinementTime / (features.size() / 100.0) << ", precaching RTF " << preCachingTime / (features.size() / 100.0) << ")" << std::endl;
                log() << "global needed time: " << globalTimer_.user() << " for frame-duration: " << (features.size() / 100.0) << " global RTF: " << globalRtf() << " (postprocessing RTF " << (globalTimer_.user() - postProcessingStartTime) / (features.size() / 100.0) << " forward-backward RTF " << refinementTime / (features.size() / 100.0) << ", precaching RTF " << preCachingTime / (features.size() / 100.0) << ")";
            }
        ...
        if(featureTimes_.size())
            std::cout << "global needed time: " << timer.user() << " for frame-duration: " << (featureTimes_.size() / 100.0) << " global RTF: " << timer.user() / (featureTimes_.size() / 100.0) << std::endl;
