...
std::optional<LabelScorer::ScoreWithTime> LabelScorer::computeScoreWithTimeInternal(Request const& request, size_t scorerIndex) {
    verify(scorerIndex == 0ul);
    return computeScoreWithTimeInternal(request);
}

std::optional<LabelScorer::ScoresWithTimes> LabelScorer::computeScoresWithTimesInternal(std::vector<Request> const& requests, size_t scorerIndex) {
    verify(scorerIndex == 0ul);
    return computeScoresWithTimesInternal(requests);
}
