...
public:
    EventDrivenParser(const Core::Configuration& c)
            : Core::XmlParser(c) {
        state = 0;
    }
};
...
class MySchemaParser : public Core::XmlSchemaParser {
    ...
    class OrthographyElement : public Core::XmlBuilderElement<std::string, Core::XmlMixedElement, Core::CreateStatic> {
...
public:
    MySchemaParser(const Core::Configuration& c)
            : Core::XmlSchemaParser(c) {
        Core::XmlElement* orth_element = new OrthographyElement(this, OrthographyElement::handler(&Self::orth));

        Core::XmlMixedElement* segment_element = new Core::XmlMixedElementRelay("segment", this);
        segment_element->addChild(orth_element);
        segment_element->ignoreUnknownElements();

        Core::XmlMixedElement* recording_element = new Core::XmlMixedElementRelay("recording", this,
        ...
        Core::XmlMixedElement* corpus = new Core::XmlMixedElementRelay("corpus", this);
