...
#if 0
ODEInt::ODEInt():dxsav(0),kmax(0),toBeDeleted(false){}
ODEInt::ODEInt(int _kmax, int N):dxsav(0),kmax(0),toBeDeleted(true){
    xp_p=new Vec_DP(kmax);
    yp_p=new Mat_DP(N,_kmax);
}
...
Nr::Vec_DP ODEInt::odeint(const Vec_IO_DP &ystart, const DP x1, const DP x2, const DP eps,
                    ...
                    void rkqs(Vec_IO_DP &, Vec_IO_DP &, DP &, const DP, const DP,
                              Vec_I_DP &, DP &, DP &,
                              Nr::DerivativesBase<DP,Vec_I_DP,Vec_O_DP> &))
{
    if(dxsav==0) dxsav=(x2-x1)/10.0;
    const int MAXSTP=10000;
    const DP TINY=1.0e-30;
    int i,nstp;
    DP xsav = 0,x,hnext,hdid,h;
