...
public:
    ...
    Argument operator()(Argument element, s32) {
        return element / upperBound_;
    }
};
...
private:
    ...
    void crossCorrelationWithMultiplication(const std::vector<Data>& x, const std::vector<Data>& y, std::vector<Data>& Rxy);
    void crossCorrelationWithAbsoluteDifference(const std::vector<Data>& x, const std::vector<Data>& y, std::vector<Data>& Rxy) const;

    void normalizeUpperBound(const std::vector<Data>& x, const std::vector<Data>& y, std::vector<Data>& Rxy) const;
...
public:
    ...
    void setPower(const f64 power) {
        power_ = power;
    }
    void setNormalizationType(const NormalizationType normalizationType) {
        normalizationType_ = normalizationType;
    }
    void setUseFastFourierTransform(const bool use) {
        useFastFourierTransform_ = use;
    }
};
...
private:
    ...
    void setContinuousBegin(f64 begin) {
        if (continuousBegin_ != begin) {
            continuousBegin_ = begin;
            needInit_        = true;
        }
    }
    void setContinuousEnd(f64 end) {
        if (continuousEnd_ != end) {
            continuousEnd_ = end;
            needInit_      = true;
        }
    }
...
public:
    static std::string filterName() {
        return "signal-cross-correlation";
    }
