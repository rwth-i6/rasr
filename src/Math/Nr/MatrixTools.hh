...
void invert(Math::Matrix<T, P>& mat) {
...
template<typename T, class P>
T logDeterminant(const Math::Matrix<T, P>& mat) {
    require(mat.nRows() == mat.nColumns());
    ...
    d = 0.0;
    for (u32 i = 0; i < mat.nRows(); i++)
        d += log(fabs(tmp_mat[i][i]));
    return d;
}

template<typename T, class P>
T determinant(const Math::Matrix<T, P>& mat) {
    require(mat.nRows() == mat.nColumns());
