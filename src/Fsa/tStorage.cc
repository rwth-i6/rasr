...
void StorageAutomaton<_Automaton>::setType(Fsa::Type type) {
    type_ = type;
    if (type_ == Fsa::TypeAcceptor)
        output_ = input_;
}
template<class _Automaton>
Fsa::Type StorageAutomaton<_Automaton>::type() const {
    return type_;
}
...
template<class _Automaton>
Core::Ref<const typename _Automaton::Semiring> StorageAutomaton<_Automaton>::semiring() const {
    return semiring_;
}

template<class _Automaton>
Fsa::StateId StorageAutomaton<_Automaton>::initialStateId() const {
    return initial_;
}
