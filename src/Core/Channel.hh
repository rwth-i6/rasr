...
public:
    Channel(const Configuration&, const std::string& name, Default defaultTarget = disabled, TargetType __type = plainTarget);
    virtual ~Channel();
 ...
 */
class XmlChannel : public XmlWriter {
private:
    Channel channel_;

public:
    XmlChannel(const Configuration&, const std::string& name, Channel::Default defaultTarget = Channel::disabled);
    virtual ~XmlChannel();
...
class Channel::Manager : public Configurable,
                         private Core::Mutex {
private:
