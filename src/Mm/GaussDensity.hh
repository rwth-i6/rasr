...
/** GaussDensity
 */
class GaussDensity : public AbstractDensity, public GaussDensityTopology {
...
public:
    LogLinearDensity(ComponentIndex dimension = 0)
            : scales_(dimension) {}
    LogLinearDensity(const Scales& v)
            : scales_(v) {}
    const Scales& scales() const {
        return scales_;
    }
    Scales& scales() {
        return scales_;
...
public:
    Mean(ComponentIndex dimension = 0, MeanType value = 0)
            : Precursor(dimension, value) {}
    Mean(const Mean& v)
            : Precursor(v) {}
    Mean(const Mean& v, const Count& obs)
            : Precursor(v) {}
...
public:
    DiagonalCovariance(ComponentIndex dimension = 0) {
    ...
    Covariance* clone() const {
        return new DiagonalCovariance(*this);
    }

    DiagonalCovariance& operator=(std::vector<VarianceType>& v) {
        buffer_ = v;
        featureWeights_.resize(v.size(), 1.0);
        return *this;
    }
    virtual void setFeatureWeights(const std::vector<f64>& w) {
        featureWeights_ = w;
    }
    virtual void setOffset(ComponentIndex offset) {
    ...
    virtual const std::vector<VarianceType>& diagonal() const {
        return buffer_;
    }
    virtual const std::vector<f64>& weights() const {
