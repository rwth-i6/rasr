...
public:
    ...
    void set(Fsa::StateId id, const WordBoundary& wordBoundary) {
        internal_.grow(id);
        if (internal_[id].valid())
            verify(internal_[id] == wordBoundary);
...
public:
    DfsState()
            : Fsa::DfsState(Fsa::ConstAutomatonRef()) {}
