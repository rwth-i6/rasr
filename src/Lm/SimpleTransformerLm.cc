...
SimpleTransformerLm::SimpleTransformerLm(const Core::Configuration& c, Bliss::LexiconRef l) : Core::Component(c), Precursor(c, l), session_(select("session")), loader_(Tensorflow::Module::instance().createGraphLoader(select("loader"))), graph_(loader_->load_graph()),  // tf::GraphDef, libraries and necessary param names
...
void SimpleTransformerLm::makeBatch(HistoryDescriptor* hd) const {  // sort by length ? general search behavior ensures similar length in the ordered queue
...
void SimpleTransformerLm::scoreBatch() const {  // default initializer always 0 ?
