 ...
 */
template<class T>
class XmlVectorElement : public XmlBuilderElement<std::vector<T>, XmlElement, CreateByContext> {
public:
    typedef XmlBuilderElement<std::vector<T>, XmlElement, CreateByContext> Predecessor;
...
protected:
    ...
    virtual XmlElement* element(const char* elementName) {
        return 0;
    }
...
public:
    ...
    XmlVectorElement(XmlContext* context, CreationHandler newVector, const char* name = tagName.c_str());

    virtual bool matches(const char* _name) const {
        return xmlMatchTagName(*this, _name);
    }
};
...
template<class T>
XmlVectorElement<T>::XmlVectorElement(XmlContext* context, CreationHandler newVector, const char* name)
...
void XmlVectorElement<T>::end() {
    ...
    std::copy(std::istream_iterator<T>(stream), std::istream_iterator<T>(), std::back_inserter(*Predecessor::product_));
