...
public:
  BatchPointerIterator ( BatchIdType batchId, std::vector <NodeIdType>& batches, std::vector <NodeType>& values ) :
    ...
    if ( current_ == currentBorder_ ) NextBatch();
  }
...
/**Appends "slave" directly to "parent" */
  ...
  inline bool checkBatch( const BatchIdType batch ) const {
    verify( !( batch & singleBatchMask_ ) );
    if ( batches_[batch] <= batches_[batch + 2] && batches_[batch + 1] != batch ) return true;
    return false;
  }

  void verifyBatchesChain( BatchIdType batch ) const {
    verify( !( batch & singleBatchMask_ ) );
    if ( !( batch >= 0 && batch < batches_.size() - 2 ) ) Core::Application::us()->log() << "error-batch: " << batch << " batches.size: " << batches_.size() << "\n";
  ...
  BatchBuilder ( std::vector <NodeIdType>& batches, std::vector <NodeType>& nodes ) : batches_( batches ), nodes_( nodes ) {
  }
...
private:
  ...
  inline BatchIdType createNewBatch( NodeIdType from, NodeIdType to, BatchIdType append = invalidBatch_, BatchIdType appendTo = invalidBatch_, bool forceNormalBatch = false ) {
    ...
    if ( appendTo != invalidBatch_ ) appendToLast = this->getLastBatch( appendTo );
...
public:
  ...
  BatchManager ( std::vector <NodeIdType>& batches, std::vector <NodeType>& nodes ) : SelfBuilder( batches, nodes ) {
  }
