...
namespace Search {
        ...
        LinearSearch::Pronunciation::Pronunciation( const Bliss::LemmaPronunciation *const lemma, const Am::AcousticModel* acousticModel ) : lemma_( lemma ) {
        ...
        public:
                ...
                struct Hypo { /*This includes lmScore, so the real score is "score - lmScore" */ Score score; Score lmScore; Book* bkp; LinearSearch::Pronunciation::MixtureItem* mixture; } ;
                typedef std::vector<Hypo> HypoVector;

                WordPronunciationState( const Bliss::LemmaPronunciation * const lemma, const Am::AcousticModel* const acousticModel, Score unigramScore ) : pron_( new LinearSearch::Pronunciation( lemma, acousticModel ) ), position_(0), unigramScore_( unigramScore ), irregularChain_(false) {
        ...
        private:
                ...
                WordPronunciationState& operator= ( const WordPronunciationState& rhs ) { return *this; }
        ...
        LinearSearch::LinearSearch( const Core::Configuration& conf) : Core::Component( conf ), SearchAlgorithm( conf ), singleWordRecognition_( paramSingleWordRecognition_( conf ) ), time_(0) {
        ...
        void LinearSearch::feed( const Mm::FeatureScorer::Scorer& emissionScores ) {
                ...
                for( WordPronunciationStateVector::iterator it = state_.begin(); it != state_.end(); ++it, ++wordNumber ) {
                        ...
                        if( ( singleWordRecognition_ && last && last->hadRegularWord && (*it)->pronunciation()->isRegularWord() )
                                || (*it)->irregularChain() ) {
...
#if 0
                // DEBUG
                std::cout << "BOOKKEEPING chose word " << chosen << " at TIME FRAME " << time_ << std::endl;
                std::cout << "used word exit score " << exit << std::endl;
#endif
    }
    ...
    LinearSearch::Book::Book() : score(Core::Type<Score>::max), lmScore(0), hadRegularWord( false ), bkp(0), time(0) {
    }
    ...
    LinearSearchHistoryData::LinearSearchHistoryData() : success(false) {
    }
