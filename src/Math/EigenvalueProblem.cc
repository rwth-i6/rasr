    ...
    for (size_t i = 0; i < diagonal.size(); ++i) {
        if (diagonal[i] < 0 && Core::abs(diagonal[i]) > diagonalNormalizationTolerance_) {
            error("Failed to normalize eigenvectors because "
                  "because %zu diagonal element %e is negative.",
                  i, diagonal[i]);
            result = false;
        }
    }
    if (result) {
        ...
        for (size_t i = 0; i < diagonal.size(); ++i) {
            if (diagonal[i] == 0)
                ...
                scalingMessage << (eigenvectors.columnNorm(i, 2) / diagonal[i]) << " ";
        }
...
bool EigenvalueProblem::isCorrectSolution(const Matrix<ValueType>& a,
    ...
    Matrix<ValueType> eigenvectorsTimesValues(eigenvectors * makeDiagonalMatrix(eigenvalues));
    return areEqual(aTimesEigenvectors, eigenvectorsTimesValues);
}

bool EigenvalueProblem::areEqual(const Matrix<ValueType>& x, const Matrix<ValueType>& y) {
    std::pair<size_t, size_t> notEqual = x.isAlmostEqual(y, verificationTolerance_);
...
void EigenvalueProblem::sortEigenvalues(Vector<ValueType>& eigenvalues, Matrix<ValueType>& eigenvectors) {
    if (eigenvalueSortType_ != noSort) {
        ...
        std::transform(eigenvalueIndexMap.begin(),
                       eigenvalueIndexMap.end(),
                       permutation.begin(),
                       select2nd<MapItem>());
...
bool EigenvalueProblem::real(const Vector<std::complex<ValueType>>& c, Vector<ValueType>& r) {
...
bool EigenvalueProblem::real(const Matrix<std::complex<ValueType>>& c, Matrix<ValueType>& r) {
...
bool GeneralizedEigenvalueProblem::solveSymmetricAndFinalize(const Matrix<ValueType>& a,
...
bool GeneralizedEigenvalueProblem::finalizeEigenvaluesAndVectors(const Matrix<ValueType>& a,
                                                                 const Matrix<ValueType>& b,
                                                                 Vector<ValueType>&       eigenvalues,
                                                                 Matrix<ValueType>&       eigenvectors) {
    const Matrix<ValueType>* m = &a;
    if (normalizeEigenvectorsUsingB_)
        m = &b;
    return EigenvalueProblem::finalizeEigenvaluesAndVectors(*m, eigenvalues, eigenvectors);
}

bool GeneralizedEigenvalueProblem::isCorrectSolution(const Matrix<ValueType>& a,
                                                     const Matrix<ValueType>& b,
                                                     const Vector<ValueType>& eigenvalues,
                                                     const Matrix<ValueType>& eigenvectors) {
    if ((a.nColumns() != eigenvectors.nRows()) ||
    ...
    Matrix<ValueType> aTimesEigenvectors(a * eigenvectors);
    Matrix<ValueType> bTimesEigenvectorsAndValues(b * eigenvectors * makeDiagonalMatrix(eigenvalues));
