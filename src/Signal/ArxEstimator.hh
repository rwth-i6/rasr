...
public:
    ...
    void setOrder_B(u8 order_B) {
        least_squares_builder_.setOrder_B(order_B_ = order_B);
    };
    ...
    void setF(const std::vector<_float>& F_tilde) {
        F_tilde_   = F_tilde;
        need_init_ = true;
    };
    void setD(const std::vector<_float>& D_tilde) {
        D_tilde_ = D_tilde;
    };

    bool work(const std::vector<_float>* u,
              const std::vector<_float>* y,
              const std::vector<_float>* y0               = 0,
              _float*                    estimation_error = 0,
              std::vector<_float>*       B_tilde          = 0,
              std::vector<_float>*       A_tilde          = 0);

    void reset() {
        freeBuffers();
        least_squares_.reset();
        need_init_ = true;
    }
};
}  // namespace Signal
