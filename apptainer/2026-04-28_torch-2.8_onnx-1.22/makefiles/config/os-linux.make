...
TF_CXXFLAGS += -I/usr/local/lib/python3.12/dist-packages/tensorflow/include
TF_LDFLAGS  += -Wl,--no-as-needed -Wl,--allow-multiple-definition
TF_LDFLAGS  += -lcrypto
TF_LDFLAGS  += -L/usr/local/lib/python3.12/dist-packages/tensorflow
TF_LDFLAGS  += -Wl,-rpath -Wl,/usr/local/lib/python3.12/dist-packages/tensorflow
...
INCLUDES    += -I$(CUDAROOT)/targets/x86_64-linux/include/
LDFLAGS     += -L$(CUDAROOT)/targets/x86_64-linux/lib/ -lcublas -lcudart -lcurand
NVCC        = $(CUDAROOT)/bin/nvcc
NVCCFLAGS   = -gencode=arch=compute_61,code=sm_61 -gencode=arch=compute_75,code=sm_75 -gencode=arch=compute_86,code=sm_86 -gencode=arch=compute_89,code=sm_89
...
PYTHON_PATH =
ifneq (${PYTHON_PATH},)
INCLUDES    += `${PYTHON_PATH}/bin/python3.12-config --includes 2>/dev/null`
LDFLAGS     += `${PYTHON_PATH}/bin/python3.12-config --ldflags --embed 2>/dev/null`
LDFLAGS     += -Wl,-rpath -Wl,${PYTHON_PATH}/lib
else
INCLUDES    += `python3.12-config --includes 2>/dev/null`
INCLUDES    += -I$(shell python3.12 -c 'import numpy as np; print(np.get_include())')
INCLUDES    += `python3.12 -m pybind11 --includes 2>/dev/null`
LDFLAGS     += `python3.12-config --ldflags --embed 2>/dev/null`
