...
class AllPairsShortestDistance::Internal : public std::unordered_map<StateIdPair, Score, StateIdPair::Hash> {
    typedef std::unordered_map<StateIdPair, Score, StateIdPair::Hash> Precursor;
            ...
            for (StateIdScorePairList::const_iterator itFrom = from.begin(), endFrom = from.end(); itFrom != endFrom; ++itFrom) {
                ...
                if (toTime - fromTime <= timeThreshold) {
                    ...
                    if ((itPair == to.end()) || (itPair->sid != d.sid))
                        to.insert(itPair, d);
                    else if (d.score < itPair->score)
                        itPair->score = d.score;
                }
            }
        }
