...
template<class _Automaton>
typename _Automaton::State* StaticAutomaton<_Automaton>::newState(Fsa::StateId tags, const typename _Automaton::Weight& weight) {
    setState(new typename _Automaton::State(states_.size(), tags, weight));
    return states_.back().get();
}

template<class _Automaton>
typename _Automaton::State* StaticAutomaton<_Automaton>::newFinalState(const typename _Automaton::Weight& weight) {
    setState(new typename _Automaton::State(states_.size(), Fsa::StateTagFinal, weight));
    return states_.back().get();
}

template<class _Automaton>
void StaticAutomaton<_Automaton>::setStateFinal(typename _Automaton::State* s, const typename _Automaton::Weight& finalWeight) {
    s->setFinal(finalWeight);
}

template<class _Automaton>
void StaticAutomaton<_Automaton>::setStateFinal(typename _Automaton::State* s) {
...
void StaticAutomaton<_Automaton>::normalize() {
    ...
    for (s = _s = 0; s < states_.size(); ++s) {
        ...
        if (states_[s]) {
            if (first == Fsa::InvalidStateId)
                first = s;
            states_[_s] = states_[s];
            states_[_s]->setId(_s);
            if (states_[s]->nArcs() > 1)
                isLinear = false;
