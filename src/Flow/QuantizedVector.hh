...
template<class T>
class Vector : public virtual Timestamp, public std::vector<T> {
private:
...
public:
    ...
    virtual Core::XmlWriter& dump(Core::XmlWriter& o) const {
        o << Core::XmlOpen open(datatype()->name()) + Core::XmlAttribute("size", size()) + Core::XmlAttribute("start", startTime()) + Core::XmlAttribute("end", endTime());
        if (!empty()) {
...
public:
    ...
    virtual Core::XmlWriter& dump(Core::XmlWriter& o) const {
        o << Core::XmlOpen open(datatype()->name()) + Core::XmlAttribute("name", name()) + Core::XmlAttribute("size", size()) + Core::XmlAttribute("start", startTime()) + Core::XmlAttribute("end", endTime());
        if (!empty()) {
            for (const_iterator i = begin(); i != end() - 1; ++i)
                o << *i << " ";
            o << back();
...
template<class T>
Core::XmlWriter& operator<<(Core::XmlWriter& o, const NamedVector<T>& v) {
    v.dump(o);
    return o;
}
template<class T>
Core::BinaryOutputStream& operator<<(Core::BinaryOutputStream& o, const NamedVector<T>& v) {
    v.write(o);
    return o;
}
template<class T>
Core::BinaryInputStream& operator>>(Core::BinaryInputStream& i, NamedVector<T>& v) {
    v.read(i);
    return i;
}
