...
StatePosteriorFeatureScorerNode::StatePosteriorFeatureScorerNode(const Core::Configuration& config) :
    ...
    Precursor(config)
{
    fs_ = new Mm::StatePosteriorFeatureScorer(
        select("posterior-feature-scorer"),
...
bool StatePosteriorFeatureScorerNode::configure(){
    ...
    if (!configureDatatype(attributes, Feature::FlowFeature::type())) return false;
...
bool StatePosteriorFeatureScorerNode::work(Flow::PortId) {
    ...
    Core::Ref<const StatePosteriorScorer> scorer(
        required_cast(
            const StatePosteriorScorer*,
