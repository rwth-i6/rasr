...
/// ------------ Search algorithm helpers ---------------------------:
        ...
        u32 getLastSyntacticToken( const Core::Ref<Trace> &_trace ) {
          ...
          while ( trace )
          {
            if ( trace->pronunciation && trace->pronunciation != epsilonLemmaPronunciation() && trace->pronunciation->lemma() && trace->pronunciation->lemma()->hasSyntacticTokenSequence() && trace->pronunciation->lemma()->syntacticTokenSequence().size() ) {
