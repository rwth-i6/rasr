...
public:
    FsaTool() : semiring_(TropicalSemiring) {
#ifdef MODULE_OPENFST
        INIT_MODULE(OpenFst)
#endif
        setTitle("fsa");
    ...
    public:
        bool has(const std::string &parameter) const { return (find(parameter) != end()); }
    };
    ...
    int main(const std::vector<std::string> &arguments) {
        if (arguments.size() == 0) std::cerr << getUsage();
        size_t arg = 0;
        Core::Timer *timer[2];
        timer[0] = timer[1] = 0;
            ...
            if (operation == Core::Choice::IllegalValue) {
                ...
                case op_best:
                    ...
                    if (specs.has("first")) {
                        ...
                        if ((*error == '\0') && (s >= 0)) stack_.push(nbest(first_, s));
                        else std::cerr << "best: n must be a positive integer" << std::endl;
                    }
                    break;
                case op_closure:
                    oneOperand();
                    if (specs.has("kleene")) stack_.push(kleeneClosure(first_));
                    else stack_.push(closure(first_));
                    break;
                case op_collect:
                    if (!specs.has("value")) std::cerr << "collect: needs value" << std::endl;
                    else {
                        oneOperand();
                ...
                case op_concat: {
                    ...
                    if (stack_.size() >= size_t(n)) {
                        ...
                        for (u32 i = 0; i < u32(n); ++i) automata.push_back(stack_.pop());
                        stack_.push(concat(automata));
                    } else std::cerr << "concat: not enough automata on stack" << std::endl;
                    break;
                }
                case op_complement:
                    oneOperand(); stack_.push(complement(first_)); break;
                case op_compose:
                    twoOperands();
                    if ((!specs.has("filter")) || (specs["filter"] == "match"))
                        stack_.push(composeMatching(first_, second_));
                    else if (specs["filter"] == "seq") stack_.push(composeSequencing(first_, second_));
                    break;
                case op_determinize:
                    oneOperand();
                    stack_.push(
                    ...
                    break;
                case op_difference:
                    twoOperands(); stack_.push(difference(first_, second_)); break;
                case op_duplicate:
                    oneOperand(); stack_.push(first_); stack_.push(first_); break;
                case op_expm:
                    oneOperand();
                    stack_.push(expm(first_));
                    break;
                case op_extend:
                    if (!specs.has("value")) std::cerr << "extend: needs value" << std::endl;
                    else {
                        oneOperand();
                    ...
                    break;
                case op_invert:
                    oneOperand(); stack_.push(invert(first_)); break;
                case op_levenshtein: {
                    twoOperands();
                    stack_.push(levenshtein(first_, second_));
#if 0
                    LevenshteinInfo info = levenshteinInfo(levenshtein(first_, second_));
                    std::cerr << "del: " << info.del_ << " ins: " << info.ins_ << " sub: " << info.sub_ << std::endl;
#endif
                    break;
                }
                case op_map_input:
                ...
                case op_map_output:
                    ...
                    break;
                case op_minimize:
                    oneOperand(); stack_.push(minimize(first_)); break;
                case op_multiply:
                    if (!specs.has("value")) std::cerr << "multiply: needs value" << std::endl;
                    else {
                        oneOperand();
                    ...
                    break;
                case op_normalize:
                    oneOperand(); stack_.push(normalize(first_)); break;
                case op_partial: {
                    if (!specs.has("id")) std::cerr << "partial: needs initial state [id]" << std::endl;
                    else {
                        oneOperand();
                        char *error;
                        long int s = strtol(specs["id"].c_str(), &error, 10);
                        if ((*error == '\0') && (s >= 0)) stack_.push(partial(first_, s));
                        else std::cerr << "partial: [id] must be a (positive) state id" << std::endl;
                    }
                    break;
                }
                case op_posterior:
                    oneOperand(); stack_.push(posterior(first_)); break;
                case op_posterior_64: {
                    Weight tmp;
                    oneOperand(); stack_.push(posterior64(first_, tmp));
                    log("totalInv: ") << f32(tmp);
                } break;
                case op_posterior_expectation: {
                    Weight tmp;
                    bool vNormalized = specs["v-norm"].empty() or (specs["v-norm"] == "true");
                    twoOperands(); stack_.push(posteriorE(first_, second_, tmp, vNormalized));
                    log("expectation: ") << f32(tmp);
                } break;
                case op_posterior_fail: {
                        Weight tmp;
                        oneOperand(); stack_.push(posterior4SpecialSymbols(first_, tmp));
                        log("totalInv: ") << f32(tmp);
            }	break;
                case op_project:
                    oneOperand();
                    if (specs.has("output")) stack_.push(projectOutput(first_));
                    else stack_.push(projectInput(first_));
                    break;
                case op_prune:
                    if (!specs.has("beam")) std::cerr << "prune: needs [beam] threshold" << std::endl;
                    else {
                        oneOperand();
                ...
                case op_push:
                    oneOperand();
                    if (specs.has("initial")) stack_.push(pushToInitial(first_));
                    else stack_.push(pushToFinal(first_));
                    break;
                case op_random: {
                    oneOperand();
                    char *error;
                    if (specs.has("seed")) {
                        ...
                        if (*error != '\0') std::cerr << "random: invalid seed value" << std::endl;
                    } else {
                        srand48(time(0));
                    }
                    ...
                    if (specs.has("weight")) {
                        ...
                        if ((*error != '\0')) std::cerr << "random: invalid importance sampling weight value" << std::endl;
                    }
                    u32 maximumSize = 0;
                    if (specs.has("limit")) {
                        maximumSize = strtol(specs["limit"].c_str(), &error, 10);
                        if (*error != '\0') std::cerr << "random: invalid maximum size value" << std::endl;
                    }
                    stack_.push(random(first_, samplingWeight, maximumSize));
                    } break;
                case op_remove:
                    oneOperand();
                    if (specs.has("disambiguators")) stack_.push(removeDisambiguationSymbols(first_));
                    else stack_.push(removeEpsilons(first_));
                    break;
                case op_sort:
                    oneOperand();
                    if (specs.has("weight")) stack_.push(sort(first_, SortTypeByWeight));
                    else if (specs.has("input")) stack_.push(sort(first_, SortTypeByInput));
                    else if (specs.has("output")) stack_.push(sort(first_, SortTypeByOutput));
                    else stack_.push(sort(first_, SortTypeByArc));
                    break;
                case op_sync_prune:
                    if (!specs.has("beam")) std::cerr << "sync-prune: needs [beam] threshold" << std::endl;
                    else {
                        oneOperand();
                    ...
                    break;
                case op_transpose:
                    oneOperand(); stack_.push(transpose(first_, progress)); first_.reset(); break;
                case op_trim:
                    oneOperand(); stack_.push(trim(first_, progress)); break;
                case op_unite: {
                    long int n = 2;
                    if (specs.has("n")) {
                        char *error;
                    ...
                    } else std::cerr << "unite: not enough automata on stack" << std::endl;
                    break;
                }
                case op_fuse: {
                    long int n = 2;
                    ...
                    } else std::cerr << "fuse: not enough automata on stack" << std::endl;
                    break;
                }
                case op_draw: {
                    oneOperand();
                    std::string file = "-";
                    if (specs.has("file")) file = specs["file"];
                    Core::CompressedOutputStream dos(file);
                    Fsa::Hint hints = Fsa::HintNone;
                    if (specs.has("best"))     hints |= Fsa::HintMarkBest;
                    if (specs.has("detailed")) hints |= Fsa::HintShowDetails;
                    if (specs.has("linear"))   hints |= Fsa::HintAsProbability;
                ...
                case op_write: {
                    ...
                    std::string file = "-";
                    if (specs.has("file")) file = specs["file"];
                    if (specs.has("input") && specs.has("output")) write(first_, file, storeAlphabets, progress);
                    else if (specs.has("input")) writeAlphabet(first_->getInputAlphabet(), file);
                    else if (specs.has("output")) writeAlphabet(first_->getOutputAlphabet(), file);
                    else if (specs.has("states")) write(first_, file, storeStates, progress);
                    else write(first_, file, storeAll, progress);
                    stack_.push(first_);
                    break;
                }

                case op_cache:
                    oneOperand(); stack_.push(cache(first_)); break;
                case op_copy: {
                    oneOperand();
                ...
                case op_delete:
                    ...
                    break;
                case op_default:
                    if (!specs.has("semiring")) std::cerr << "default: needs identifier" << std::endl;
                    else {
                ...
                case op_semiring:
                    oneOperand();
                    if (!specs.has("semiring")) std::cerr << "semiring: needs identifier" << std::endl;
                    else {
                        ...
                        if (type == SemiringTypeLog) {
                            if (specs.has("tolerance")) {
                                ...
                                if ((*error == '\0') && (tolerance >= 0)) semiring = ConstSemiringRef(new LogSemiring_(tolerance));
                                else {
                                    std::cerr << "best: n must be a positive integer" << std::endl;
                                    semiring = getSemiring(type);
                                }
                            } else semiring = getSemiring(type);
                        } else semiring = getSemiring(type);
                        if (semiring) stack_.push(changeSemiring(first_, semiring));
                    }
                    break;

                case op_count:
                    oneOperand();
                    if (specs.has("paths")) std::cout << countPaths(first_) << std::endl;
                    else {
                        if (!specs.has("label")) std::cerr << "count: needs [label]" << std::endl;
                        else {
                            size_t count = 0;
                            if (specs.has("input"))
                                count = countInput(first_, first_->getInputAlphabet()->index(specs["label"]));
                            else count = countOutput(first_, first_->getOutputAlphabet()->index(specs["label"]));
                ...
                case op_info:
                    ...
                    break;
                case op_memory:
                    oneOperand(); memoryInfo(first_, log()); stack_.push(first_); break;
                case op_time:
                    if (timer[0]) log() << *(timer[0]); break;
                case op_wait:
                    std::cerr << "press <ENTER> to continue" << std::flush; getchar(); break;
                default:
                    break;
                }
            }
            timer[1]->stop();
            if (timer[0]) delete timer[0];
            timer[0] = timer[1];
            timer[1] = 0;
        }
        if (timer[0]) delete timer[0];
...
Core::Choice FsaTool::OperationChoice
("best", op_best,
 "closure", op_closure,
 "collect", op_collect,
 "concat", op_concat,
