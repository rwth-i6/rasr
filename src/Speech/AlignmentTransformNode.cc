...
AlignmentResetWeightsNode::AlignmentResetWeightsNode(const Core::Configuration& c)
        : Core::Component(c),
          Precursor(c),
...
AlignmentMapAlphabet::AlignmentMapAlphabet(const Core::Configuration& c)
        : Core::Component(c),
          Precursor(c),
...
void AlignmentRemoveEmissionScoreNode::transform(Alignment& a) {
    ...
    while (getData(1, in)) {
        ...
        while (currentAlignmentItem != a.end() && currentAlignmentItem->time == currentFeatureId) {
            verify(currentAlignmentItem == a.begin() ? true : (currentAlignmentItem - 1)->time <= currentAlignmentItem->time);
