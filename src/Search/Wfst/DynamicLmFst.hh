...
struct DynamicLmFstOptions: public FstLib::CacheOptions
{
    DynamicLmFstOptions(Core::Ref<Lm::LanguageModel> languageModel = Core::Ref<Lm::LanguageModel>(),
            ...
            : lm(languageModel), outputType(wordType), pronunciationScale(pronScale),
...
private:
    static const uint64 kProperties =
            FstLib::kAcceptor | FstLib::kIDeterministic | FstLib::kODeterministic |
 ...
 */
template<>
class StateIterator<Search::Wfst::DynamicLmFst> :
    public CacheStateIterator<Search::Wfst::DynamicLmFst>
{
public:
 ...
 */
template<>
class ArcIterator<Search::Wfst::DynamicLmFst> :
    public ArcIteratorBase<OpenFst::Arc>
{
public:
    ...
    ArcIterator(const Search::Wfst::DynamicLmFst &fst, StateId s) :
        fst_(fst), state_(s), pos_(1), end_(fst.NumArcs(s)),
        flags_(FstLib::kArcValueFlags), scores_(0)
    {
    ...
    {
        const uint32 scoreOnlyMask =
                (FstLib::kArcNoCache | FstLib::kArcWeightValue |
...
public:
    ...
    DynamicLmFstMatcher(const DynamicLmFst &fst, FstLib::MatchType matchType)
        : fst_(fst.Copy()), arcRead_(true), aiter_(0), mtype_(matchType), state_(FstLib::kNoStateId),
          loop_(FstLib::kNoLabel, 0, Weight::One(), FstLib::kNoStateId) {}

    DynamicLmFstMatcher(const DynamicLmFstMatcher &m, bool safe)
        : fst_(m.fst_->Copy(safe)), arcRead_(true), aiter_(0), mtype_(m.mtype_),
          state_(FstLib::kNoStateId), loop_(m.loop_) {}
