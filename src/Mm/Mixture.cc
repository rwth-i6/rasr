...
#else
    // Do not remove density explicitly but only set mixture weight to zero.
    // This avoids problems with different topologies.
    for (DensityIndex dnsInMix = 0; dnsInMix < nDensities(); ++ dnsInMix) {
        if ((weight(dnsInMix) < _minWeight) and (dnsInMix != dnsMax)) {
            logWeights_[dnsInMix] = -1000;
        }
    }
#endif
    if (normalizeWeights) {
