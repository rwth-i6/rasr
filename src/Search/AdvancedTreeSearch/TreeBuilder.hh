...
public:
  ...
  struct StatePredecessor {
    StatePredecessor( std::set<StateId> _successors = std::set<StateId>(), Search::StateTree::StateDesc _desc = Search::StateTree::StateDesc(), bool _isWordEnd = false ) :
      successors( _successors ), desc( _desc ), isWordEnd( _isWordEnd ),
  ...
  u32 addExit( StateId prePredecessor, StateId predecessor, Bliss::Phoneme::Id leftPhoneme, Bliss::Phoneme::Id rightPhoneme, int depth, Bliss::LemmaPronunciation::Id pron );
  void hmmFromAllophone( HMMSequence& ret, Bliss::Phoneme::Id left, Bliss::Phoneme::Id central, Bliss::Phoneme::Id right, u32 boundary = 0, bool allowNonStandard = true );
  ...
  std::pair<StateId, StateId> extendPhone( StateId currentState, u32 phoneIndex, const std::vector<Bliss::Phoneme::Id>& phones, Bliss::Phoneme::Id left = Bliss::Phoneme::term, Bliss::Phoneme::Id right = Bliss::Phoneme::term );
