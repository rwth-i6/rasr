...
namespace Fsa {
    ...
    private:
        ...
        private:
            ...
            struct ExpandedToken {
                Score score_;
                Trace trace_;
                ExpandedToken() : score_(Core::Type<Score>::max) {}
        ...
        public:
            Slice(const StaticAutomaton *fsa, ExpandedTokens &expandedTokens, StaticAutomaton *trace,
                ...
                fsa_(fsa), inputEpsilon_(fsa->getInputAlphabet()->epsilon()),
                outputEpsilon_(fsa->getOutputAlphabet()->epsilon()), semiring_(fsa->semiring()),
                bestFinalTrace_(InvalidStateId), trace_(trace), expandedTokens_(expandedTokens),
                threshold_(threshold), finalThreshold_(finalThreshold),
                statesBeforePruning_("states before pruning"), statesAfterPruning_("states after pruning") {}
            ...
            template <class Scorer> Score expand(Slice &next, const Scorer &scorer, Time time, Core::Vector<StateId> &expandedTokens) {
                ...
                //minFinalScore_ = Core::Type<Score>::max;
                while (!tokens_.isEmpty()) {
                    ...
                    for (State::const_iterator a = sp->begin(); a != sp->end(); ++a) {
                        ...
                        if (fsa_->fastState(a->target())->isFinal()) {
                            //if (score < minFinalScore_) minFinalScore_ = score;
                ...
                //std::cout << " " << minScore << " " << minFinalScore << " " << next.bestFinalTrace_ << std::endl;
                return minThreshold;
                //return minScore + threshold_;
            }
            ...
            void prune(Slice &next, const Core::Vector<StateId> &expandedTokens, Score minThreshold) {
                ...
                next.tokens_.clear();
                //minFinalScore_ += finalThreshold_;
                //std::cout << minThreshold << " " << minFinalScore_ << std::endl;
                for (Vector<StateId>::const_iterator i = expandedTokens.begin(); i != expandedTokens.end(); ++i) {
                    if (expandedTokens_[*i].score_ < minThreshold)
                        //if (!fsa_->fastState(*i)->isFinal() || (expandedTokens_[*i].score_ < minFinalScore_))
    ...
    public:
        Search(const StaticAutomaton *fsa, Score threshold) :
            fsa_(fsa), trace_(new StaticAutomaton()), traceRef_(trace_), expandedTokens_(fsa->size()),
