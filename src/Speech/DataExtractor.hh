...
class DataExtractor : public CorpusProcessor {
 ...
 */
class FeatureExtractor : public DataExtractor {
...
protected:
    ...
    /** Override this function to achieve the features extracted form data source one-by-one.
     */
 ...
 */
class FeatureVectorExtractor : public FeatureExtractor {
    typedef FeatureExtractor Precursor;

protected:
    virtual void setFeatureDescription(const Mm::FeatureDescription& d) {
        d.verifyNumberOfStreams(1, true);
        setFeatureVectorDescription(d.mainStream());
    }
