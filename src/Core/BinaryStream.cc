...
void BinaryStreamIos::open(const std::string&      name,
                           std::ios_base::openmode mode) {
    mode |= std::ios::binary;
    if (fstream_)
        fstream_->open(name.c_str(), mode);
}
...
bool BinaryOutputStream::write(const T* v, std::streamsize n) {
    if (!swap_)
        return (bool)os_->write((char*)v, sizeof(T) * n);
    T* swapBuffer = new T[n];
    memcpy(swapBuffer, v, sizeof(T) * n);
    swapEndianess<sizeof(T)>(swapBuffer, n);
    bool result = (bool)os_->write((char*)swapBuffer, sizeof(T) * n);
    delete[] swapBuffer;
    return result;
}
...
bool BinaryOutputStream::write(const bool* v, std::streamsize n) {
    ...
    for (std::streamsize i = 0; i < n; i++)
        tmp[i] = (v[i]) ? 0xff : 0;
    bool success = (bool)os_->write((char*)tmp, n);
    delete[] tmp;
    return success;
}
}  // end namespace Core
...
BinaryOutputStream& BinaryOutputStream::operator<<(u8 n) {
    write(&n);
    return *this;
}
BinaryOutputStream& BinaryOutputStream::operator<<(s8 n) {
    write(&n);
    return *this;
}
BinaryOutputStream& BinaryOutputStream::operator<<(s16 n) {
    write(&n);
    return *this;
}
BinaryOutputStream& BinaryOutputStream::operator<<(u16 n) {
    write(&n);
    return *this;
}
BinaryOutputStream& BinaryOutputStream::operator<<(s32 n) {
    write(&n);
    return *this;
}
BinaryOutputStream& BinaryOutputStream::operator<<(u32 n) {
    write(&n);
    return *this;
}
BinaryOutputStream& BinaryOutputStream::operator<<(s64 n) {
    write(&n);
    return *this;
}
BinaryOutputStream& BinaryOutputStream::operator<<(u64 n) {
    write(&n);
    return *this;
}
BinaryOutputStream& BinaryOutputStream::operator<<(f32 n) {
    write(&n);
    return *this;
}
BinaryOutputStream& BinaryOutputStream::operator<<(f64 n) {
    write(&n);
    return *this;
}
...
BinaryOutputStream& BinaryOutputStream::operator<<(size_t n) {
    write(&n);
    return *this;
}
#endif
...
BinaryOutputStream& BinaryOutputStream::operator<<(bool b) {
    write(&b);
    return *this;
}
...
bool BinaryInputStream::read(T* v, std::streamsize n) {
    if (!is_->read((char*)v, sizeof(T) * n))
        return false;
    if (swap_)
        swapEndianess<sizeof(T)>(v, n);
    return true;
}
...
bool BinaryInputStream::read(bool* v, std::streamsize n) {
    u8*  tmp     = new u8[n];
    bool success = (bool)is_->read((char*)tmp, n);
    for (std::streamsize i = 0; i < n; i++)
        v[i] = (tmp[i]) ? true : false;
    delete[] tmp;
    return success;
}
}  // namespace Core
...
BinaryInputStream& BinaryInputStream::operator>>(u8& n) {
    read(&n);
    return *this;
}
BinaryInputStream& BinaryInputStream::operator>>(s8& n) {
    read(&n);
    return *this;
}
BinaryInputStream& BinaryInputStream::operator>>(s16& n) {
    read(&n);
    return *this;
}
BinaryInputStream& BinaryInputStream::operator>>(u16& n) {
    read(&n);
    return *this;
}
BinaryInputStream& BinaryInputStream::operator>>(s32& n) {
    read(&n);
    return *this;
}
BinaryInputStream& BinaryInputStream::operator>>(u32& n) {
    read(&n);
    return *this;
}
BinaryInputStream& BinaryInputStream::operator>>(s64& n) {
    read(&n);
    return *this;
}
BinaryInputStream& BinaryInputStream::operator>>(u64& n) {
    read(&n);
    return *this;
}
BinaryInputStream& BinaryInputStream::operator>>(f32& n) {
    read(&n);
    return *this;
}
BinaryInputStream& BinaryInputStream::operator>>(f64& n) {
    read(&n);
    return *this;
}
...
BinaryInputStream& BinaryInputStream::operator>>(bool& b) {
    read(&b);
    return *this;
}
...
BinaryInputStreams::BinaryInputStreams(const std::vector<std::string>& filenames,
                                       std::ios_base::openmode         mode,
...
BinaryInputStreams::BinaryInputStreams(std::vector<std::istream*>* istreams,
                                       BinaryStreamIos::Endianess  endianess)
