...
public:
    ...
    // labels
    LabelIndex numClasses() const {
        return numClasses_;
    }
    const LabelIndexMap& getLabelIndexMap();
    // special labels: either in the vocab file or configurable (hard-coded naming)
    LabelIndex getBlankLabelIndex() const {
        return getSpecialLabelIndex("<blank>", "blank-label-index");
    }
    LabelIndex getStartLabelIndex() const {
        return getSpecialLabelIndex("<s>", "start-label-index");
    }
    LabelIndex getEndLabelIndex() const {
        return getSpecialLabelIndex("</s>", "end-label-index");
    }
    LabelIndex getUnknownLabelIndex() const {
        return getSpecialLabelIndex("<unk>", "unknown-label-index");
    }
    LabelIndex getNoContextLabelIndex() const;

    // special flags for various models, e.g. attention, segmental, RNN-T
    bool needEndProcess() const {
        return needEndProcessing_ || isPositionDependent_;
    }
    bool isPositionDependent() const {
        return isPositionDependent_;
    }
    virtual bool useRelativePosition() const {
        return false;
    }
    virtual bool useVerticalTransition() const {
        return false;
    }
    ...
    virtual bool bufferFilled() const {
        return eos_ || inputBuffer_.size() >= bufferSize_;
    }
    void setEOS() {
        eos_ = true;
    }
    ...
    virtual void increaseDecodeStep() {
        ++decodeStep_;
    }
    // stopping criteria
    ...
    virtual bool loopUpdateHistory() const {
        return loopUpdateHistory_;
    }
    virtual bool blankUpdateHistory() const {
