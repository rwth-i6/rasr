...
namespace OpenFst {
template<class A> class CompactReplaceFst;
}
...
{
public:
    GrammarRelabeler(const L &l) : l_(l) {}
...
{
public:
    enum GrammarType { TypeAny, TypeVector, TypeConst, TypeCompact,
                       TypeCombine, TypeCompose, TypeDynamic, TypeFailArc };
...
{
public:
    GrammarFstTpl() : fst_(0) {}
    GrammarFstTpl(const F &other) : fst_(other.Copy()) {}
    virtual ~GrammarFstTpl() { delete fst_; }
    ...
    GrammarType type() const { return t; }
protected:
    F *fst_;
};
 ...
 */
class GrammarFst :
    public GrammarFstTpl<FstLib::StdVectorFst, AbstractGrammarFst::TypeVector>
{
    typedef GrammarFstTpl<FstLib::StdVectorFst, AbstractGrammarFst::TypeVector> Parent;
public:
    GrammarFst() {}
    GrammarFst(const FstLib::StdVectorFst &f) : Parent(f) {}
...
public:
    ...
    bool load(const std::string &filename);
    GrammarType type() const { return TypeFailArc; }
    static const OpenFst::Label FailLabel;
};

typedef GrammarFstTpl<FstLib::StdCompactAcceptorFst,
            AbstractGrammarFst::TypeCompact> CompactGrammarFst;

typedef GrammarFstTpl<FstLib::StdConstFst,
            AbstractGrammarFst::TypeConst> ConstGrammarFst;

// Add if needed
// typedef GrammarFstTpl<FstLib::NGramFst<FstLib::StdArc>,
//	    AbstractGrammarFst::TypeNGram> NGramGrammarFst;
...
public:
    ...
    const FstLib::StdFst* getFst() const;
    void reset();
    GrammarType type() const { return TypeCombine; }
protected:
...
public:
    ...
    bool load(const std::string &root);
    void relabel(const GrammarRelabelerBase &relabeler);
    const FstLib::StdFst* getFst() const { return rfst_; }
    void reset();
    GrammarType type() const { return TypeCompose; }
protected:
    static const Core::ParameterInt paramCacheSize;
...
public:
    ...
    void relabel(const GrammarRelabelerBase &relabeler);
    const FstLib::StdFst* getFst() const { return fst_; }
    void reset();
    GrammarType type() const { return TypeDynamic; }
protected:
