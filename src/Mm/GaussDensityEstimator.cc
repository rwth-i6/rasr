...
GaussDensity* GaussDensityEstimator::estimate(const ReferenceIndexMap<AbstractMeanEstimator>&       meanMap,
...
bool GaussDensityEstimator::equalTopology(const GaussDensityEstimator&                          toCompare,
                                          const ReferenceIndexMap<AbstractMeanEstimator>&       meanMap,
...
bool GaussDensityEstimator::accumulate(Core::BinaryInputStreams& is, Core::BinaryOutputStream& os) {
    MeanIndex meanIndex;
    is.front() >> meanIndex;
...
Covariance* CovarianceEstimator::estimate(const CovarianceToMeanSetMap& meanSetMap, VarianceType minimumVariance) {
    if (accumulator_.weight() == 0)
        return new DiagonalCovariance(accumulator_.sum().size());

    const CovarianceToMeanSetMap::MeanSet& meanSet = meanSetMap[Core::Ref<AbstractCovarianceEstimator>((CovarianceEstimator*)this)];
...
void CovarianceEstimator::applyMinimumVariance(VarianceType minimumVariance, DiagonalCovariance& result) const {
    ...
    for (std::vector<VarianceType>::iterator d = result.buffer_.begin(); d != result.buffer_.end(); ++d) {
        if (*d < minimumVariance)
            *d = minimumVariance;
    }
}
...
const CovarianceToMeanSetMap::MeanSet& CovarianceToMeanSetMap::operator[](Core::Ref<AbstractCovarianceEstimator> estimator) const {
