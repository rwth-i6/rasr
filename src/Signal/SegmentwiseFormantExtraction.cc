                                        ...
                                        std::vector<LinearFilterParameter>& out)
{
    ...
    for(u8 i = 1; i < segments.size(); i ++)
        {
            out[i - 1].getB().clear();
            out[i - 1].getY0().clear(); out[i - 1].getY0().resize(order_, 0.0);
...
Core::Choice SegmentwiseFormantExtractionNode::domain_choice(
    "frequency", frequency,
    Core::Choice::endMark());
Core::ParameterChoice SegmentwiseFormantExtractionNode::paramDomain
("domain", &domain_choice, "calculation domain", frequency);

Core::Choice SegmentwiseFormantExtractionNode::segment_estimator_choice
("dinamic-programming", dynamic_programing,
 Core::Choice::endMark());
Core::ParameterChoice SegmentwiseFormantExtractionNode::paramSegmentEstimator
("segment-estimator", &segment_estimator_choice, "segment estimator type", dynamic_programing);

Core::ParameterInt SegmentwiseFormantExtractionNode::paramOrder
("order", "LPC order for one segment", 2, 0);
Core::ParameterInt SegmentwiseFormantExtractionNode::paramMaxNumberOfSegments
("max-number-segment", "maximum number of segments");
Core::ParameterInt SegmentwiseFormantExtractionNode::paramStep
("step", "devide frequency resolution by step", 1, 0);
Core::ParameterInt SegmentwiseFormantExtractionNode::paramMaxFrequency
("max-frequency", "frequency range to search", 5000, 0);
