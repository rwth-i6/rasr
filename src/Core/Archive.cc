...
bool Archive::readFile(const std::string& name, std::string& b) {
    ...
    if (sizes.compressed() > 0) {
        ...
        if (read(name, tmp)) {
            ...
            u32 base = 10;
            if (tmp[3] & 0x04)
                base += (int(tmp[base]) + int(tmp[base + 1])) << 8;  // extra field
            if (tmp[3] & 0x08) {
                for (; (base < compressedSize) && (tmp[base]); base++)
                    ;  // filename
                base++;
            }
            if (tmp[3] & 0x10) {
...
bool Archive::writeFile(const std::string& name, const std::string& b, bool compress) {
    ...
    if (compress) {
         ...
         */
        int zstatus = ::compress2((Bytef*)&compressed[header_length - 2], &compressedSize,
    ...
    bool status = false;
    if (compressed.size())
        status = write(name, compressed, Sizes(b.size(), compressed.size()));
    else
        status = write(name, b, Sizes(b.size(), 0));
...
std::ostream& Core::operator<<(std::ostream& s, const Archive& a) {
    ...
    for (Archive::const_iterator i = a.files(); i; ++i) {
        ...
        if (i.sizes().compressed())
            s << ' ' << std::setiosflags(std::ios::right) << std::setw(prec) << i.sizes().compressed();
        else
            s << " " << std::setw(prec) << " ";
    ...
    if (compressed)
        s << ' ' << std::setiosflags(std::ios::right) << std::setw(prec) << compressed;
    else
        s << " " << std::setw(prec) << " ";
...
Archive::Type Archive::test(const std::string& path) {
    if (path == "-")
        return TypeUnknown;
    if (BundleArchive::test(path))
        return TypeBundle;
    if (DirectoryArchive::test(path))
        return TypeDirectory;
    if (FileArchive::test(path))
        return TypeFile;
    return TypeUnknown;
}
...
public:
    ArchiveWriterBuffer(std::string& buffer)
            : buffer_(buffer) {}
...
ArchiveWriter::~ArchiveWriter() {
    if (isOpen())
        archive_.writeFile(path_, buffer_, compress_);
    delete rdbuf(0);
}
