...
protected:
    ...
    double eigenvalueLowerBound() const {
        return eigenvalueLowerBound_;
    }
    double eigenvalueUpperBound() const {
        return eigenvalueUpperBound_;
    }
    DriverType driverType() const {
        return driverType_;
    }
    bool shouldSupportEigenvectors() const {
        return shouldSupportEigenvectors_;
    }
 ...
 */
class SymmetricEigenvalueProblem : public Math::EigenvalueProblem, public EigenvalueProblem {
private:
    bool solveWithExpertOrRelativelyRobustDriver(const Math::Matrix<ValueType>& a,
 ...
 */
class GenEigenProblemWithSchurDecomposition : public Math::GeneralizedEigenvalueProblem, public EigenvalueProblem {
private:
    /**
...
private:
    bool solveWithExpertDriver(const Math::Matrix<ValueType>&         a,
     ...
     */
    bool dggevx(Matrix<double>& a,
                Matrix<double>& b,
                Vector<double>& realAlphas,
                Vector<double>& imaginaryAlphas,
                Vector<double>& betas,
                Matrix<double>& leftEigenvectors,
                Matrix<double>& rightEigenvectors,
                ...
                double&         rightMaxPerMinScaling) const;
     ...
     *  and @c betas.
     */
    void copyEigenvalues(const Math::Vector<std::complex<ValueType>>& alphas,
     ...
     */
    std::complex<ValueType> calculateEigenvalue(std::complex<ValueType> alpha,
     ...
     */
    void copyEigenvectors(const Math::Vector<std::complex<ValueType>>& alphas,
                          const Matrix<ValueType>&                     v,
                          Math::Matrix<std::complex<ValueType>>&       eigenvectors) const;

    void writeConditionNumbers(Core::XmlWriter&                             os,
 ...
 */
class GenSymmetricDefiniteEigenProblem : public Math::GeneralizedEigenvalueProblem, public EigenvalueProblem {
private:
    bool solveWithExpertDriver(const Math::Matrix<ValueType>& a,
                               const Math::Matrix<ValueType>& b,
                               Math::Vector<ValueType>&       eigenvalues,
