...
public:
    ...
    Matrix<T, P>& operator/=(const T& v) {
        return operator*=((T)1 / v);
    }

    Vector<T, P>& operator[](size_t i) {
        P().checkRange(elem_, i);
        return elem_[i];
    }
    const Vector<T, P>& operator[](size_t i) const {
        P().checkRange(elem_, i);
        return elem_[i];
    }
...
void Matrix<T, P>::setRow(size_t position, const Vector<T, P>& v) {
    require_lt(position, nRows());
    require_eq(v.size(), nColumns());
    elem_[position] = v;
}
...
void Matrix<T, P>::setColumn(size_t position, const Vector<T, P>& v) {
    require_lt(position, nColumns());
    require_eq(v.size(), nRows());
    for (size_t row = 0; row < nRows(); row++)
...
T Matrix<T, P>::productTrace(const Matrix<T, P>& rs) const {
    T trace = (T)0.0f;
    require_eq(nRows_, rs.nColumns_);
    require_eq(nColumns_, rs.nRows_);
...
void Matrix<T, P>::permuteColumns(const std::vector<size_t>& permutation) {
    require_eq(permutation.size(), nColumns());
    Matrix<T, P> tmp = *this;
    for (size_t row = 0; row < nRows(); ++row) {
        for (size_t column = 0; column < nColumns(); column++) {
...
template<class T, class P>
Matrix<T, P> Matrix<T, P>::operator+(const Matrix<T, P>& rs) const {
    require_eq(nRows(), rs.nRows());
    require_eq(nColumns(), rs.nColumns());
...
template<class T, class P>
Matrix<T, P>& Matrix<T, P>::operator-=(const Matrix<T, P>& rs) {
    require_eq(nRows_, rs.nRows_);
    require_eq(nColumns_, rs.nColumns_);
    for (size_t n = 0; n < nRows(); n++)
        for (size_t m = 0; m < nColumns(); m++)
            elem_[n][m] -= rs[n][m];
    return (*this);
...
void Matrix<T, P>::write(Core::XmlWriter& os) const {
    os << Core::XmlOpen(std::string("matrix-") + Core::Type<T>::name) + Core::XmlAttribute("nRows", nRows()) + Core::XmlAttribute("nColumns", nColumns());
...
template<class T, class P>
Vector<T, P> diagonal(const Matrix<T, P>& m) {
    return m.diagonal();
}
...
inline bool operator==(const Matrix<T1, P1>& m1, const Matrix<T2, P2>& m2) {
    if (m1.nRows() != m2.nRows())
        return false;
    if (m1.nColumns() != m2.nColumns())
        return false;
    for (u32 i = 0; i < m1.nRows(); ++i)
        if (m1[i] != m2[i])
            return false;
    return true;
}
...
template<class T, class P = VectorRangeCheckOffPolicy>
class RefernceCountedMatrix : public Core::ReferenceCounted, public Matrix<T, P> {
public:
    RefernceCountedMatrix()
            : Matrix<T, P>() {}
    RefernceCountedMatrix(int n, int m)
            : Matrix<T, P>(n, m) {}
    RefernceCountedMatrix(int n)
            : Matrix<T, P>(n) {}
    template<class S>
    RefernceCountedMatrix(const Matrix<S>& x)
            : Matrix<T, P>(x) {}
};
...
public:
    NameHelper()
            : std::string(std::string("matrix-") + NameHelper<T>()) {}
};
...
public:
    NameHelper()
            : std::string(std::string("ref-counted-matrix-") + NameHelper<T>()) {}
};
}  // namespace Core
