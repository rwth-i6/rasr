...
public:
    struct Transit {
         ...
         */
        u8 boundary;
        Transit(Bliss::Phoneme::Id final    = Bliss::Phoneme::term,
...
public:
    ...
    Speech::TimeframeIndex time() const {
        return time_;
    }
    void setTransit(const Transit& transit) {
        transit_ = transit;
    }
    Transit transit() const {
        return transit_;
    }
    ...
    bool valid() const {
        return time_ != Speech::InvalidTimeframeIndex;
    }

    struct Hash {
        size_t operator()(const Boundary& b) const {
            return size_t(b.time()) ^ ((size_t(b.transit().final) << 25) | (size_t(b.transit().final) >> 7)) ^ ((size_t(b.transit().initial) << 18) | (size_t(b.transit().initial) >> 14)) ^ ((size_t(b.transit().boundary) << 11) | (size_t(b.transit().boundary) >> 11));
        }
    };
    struct Equal {
...
class StaticBoundaries : public Core::Vector<Boundary>,
                         public Boundaries {
...
public:
    ...
    bool operator==(const StaticBoundaries& boundaries) const {
        return (size() != boundaries.size()) ? false : std::equal(begin(), end(), boundaries.begin());
    }
    virtual bool valid() const {
        return true;
    }
