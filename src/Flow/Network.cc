...
void Network::startThread() {
    // if network is threaded, mark all output nodes threaded
    if (isThreaded()) {
        for (u32 i = 0; i < outputs_.size(); i++) {
            outputs_[i].link()->getFromNode()->setThreaded(true);
        }
    }

    // start all nodes
    for (std::list<AbstractNode*>::iterator it = nodes_.begin(); it != nodes_.end(); it++) {
        (*it)->startThread();
    }
...
#if 0
  // clear all queues
  for (u32 i = 0; i < links_.size(); i++) links_[i]->clear();
#endif
      started_ = false;
}
#endif
...
ssize_t Network::getRemainingDataLen(PortId out) {
    Link* link = getPortLink(out);
    require(link);
...
bool Network::dump(const bool initialCall, Core::XmlChannel& dumpChannel, std::set<std::string>* dumped) {
    ...
    if (initialCall)
        dumpChannel << Core::XmlOpen("network") + Core::XmlAttribute("name", getTypeName());

    // remember the filenames to avoid multiple inclusions
    if (!dumped)
        dumped = new std::set<std::string>;
    ...
    if (!initialCall)
        dumpChannel << Core::XmlOpen("network-node") + Core::XmlAttribute("filter", filename()) + Core::XmlAttribute("name", getTypeName());

    // dump <param>
    for (std::list<Network::Parameter>::const_iterator it = params_.begin();
         it != params_.end(); it++) {
        dumpChannel << Core::XmlEmpty("param") + Core::XmlAttribute("name", it->name());
    }

    // dump <in>
    for (std::vector<Port>::const_iterator inputPort = inputs_.begin();
         inputPort != inputs_.end(); ++inputPort) {
        dumpChannel << Core::XmlEmpty("in") + Core::XmlAttribute("name", inputPort->name());
    }

    // dump <out>
    for (std::vector<Port>::const_iterator outputPort = outputs_.begin();
         outputPort != outputs_.end(); ++outputPort) {
        dumpChannel << Core::XmlEmpty("out") + Core::XmlAttribute("name", outputPort->name());
    }
    ...
    for (std::list<AbstractNode*>::const_iterator it = nodes_.begin();
        ...
        if (0 == ((*it)->name().find(Network::inputRepeaterPrefix)))
            continue;
    ...
    for (std::list<Link*>::const_iterator it = links_.begin(); it != links_.end(); ++it) {
        ...
        if ("" == (from_n + to_n))
            continue;
        dumpChannel << Core::XmlEmpty("link") + Core::XmlAttribute("from", from_n + (from_p != "" ? ":" + from_p : "")) + Core::XmlAttribute("to", to_n + (to_p != "" ? ":" + to_p : ""));
    }
