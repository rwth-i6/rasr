...
struct AlignmentToPython {
    ...
    AlignmentToPython() : nSkippedAlignmentFrames_(0), parent_(NULL), features_(NULL) {}
    ...
    void extractSoftAlignment(const Speech::Alignment& alignment, Python::ObjRef& pySoftAlignment) {
        ...
        if(!Python::stdVec2numpy(criticalErrorFunc_, pyI.obj, alignmentTime)) return;
        if(!Python::stdVec2numpy(criticalErrorFunc_, pyJ.obj, alignmentClassIdx)) return;
        if(!Python::stdVec2numpy(criticalErrorFunc_, pyData.obj, alignmentWeight)) return;
...
struct BuildSegmentToOrthMapVisitor : public Bliss::CorpusVisitor {
    BuildSegmentToOrthMapVisitor() : Bliss::CorpusVisitor(), map_(new Core::StringHashMap<std::string>()) {}
...
struct PythonControl::Internal : public Core::Component {
    ...
    Internal(const Core::Configuration &c) :
        Core::Component(c),
        capsule_(NULL), callback_(NULL),
        criterion_(NULL), allophoneStateFsaExporter_(NULL), segmentToOrthMap_(NULL)
    {
    ...
    PyObject* calculateCriterion(PyObject* args, PyObject* kws) {
        ...
        if(segmentName) speechSegment.setName(segmentName);
        if(orthography) speechSegment.setOrth(orthography);
    ...
    bool _readAlignmentFromCacheArchive(const std::shared_ptr<Core::Archive>& a, const std::string& segmentName, Flow::DataPtr<Flow::DataAdaptor<Speech::Alignment> >& alignmentRef) {
    ...
    PyObject* analyzeAlignmentFromCache(PyObject* args, PyObject* kws) {
        ...
        if(!segment_name_c) archive_iter = a->files();
        while(true) {
            if(!segment_name_c) {
    ...
    PyObject* callback(PyObject* args, PyObject* kws) {
        ...
        if(cmd_s == "version") return version();
        if(cmd_s == "version_number") return versionNumber();
        if(cmd_s == "help") return help();
        if(cmd_s == "init_criterion") return initCriterion();
        if(cmd_s == "calculate_criterion") return calculateCriterion(args, kws);
        if(cmd_s == "get_ctc_alignment") return getCtcAlignment(args, kws);
        if(cmd_s == "export_allophone_state_fsa_by_orthography") return exportAllophoneStateFsaByOrthography(args, kws);
        if(cmd_s == "export_allophone_state_fsa_by_segment_name") return exportAllophoneStateFsaBySegName(args, kws);
        if(cmd_s == "get_orthography_by_segment_name") return getOrthographyBySegmentName(args, kws);
        if(cmd_s == "get_alignment_from_cache") return getAlignmentFromCache(args, kws);
        if(cmd_s == "analyze_alignment_from_cache") return analyzeAlignmentFromCache(args, kws);
...
struct NoneFeatureExtractor : public Speech::CorpusProcessor {
    ...
    Core::Ref<Speech::DataSource> dataSource() const { return Core::Ref<Speech::DataSource>(); }
    NoneFeatureExtractor(const Core::Configuration &c) : Core::Component(c), Precursor(c) {}
};
...
public:
    ...
    PythonControlCorpusProcessor(const Core::Configuration &c, PythonControl& control) :
        Core::Component(c), Precursor(c), control_(control),
        firstSegment_(true),
