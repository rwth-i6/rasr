 ...
 * */
template<class T>
class Vector : public Flow::Timestamp, public SingleValueSparseVector<T> {
private:
...
Core::XmlWriter& Vector<T>::dump(Core::XmlWriter& o) const {
    o << Core::XmlOpen(datatype()->name()) + Core::XmlAttribute("size", size()) + Core::XmlAttribute("start", startTime()) + Core::XmlAttribute("end", endTime());
    Precursor::dump(o);
...
public:
    NameHelper()
            : std::string(Core::NameHelper<Sparse::SingleValueSparseVector<T>>()) {}
};
}  // namespace Core
