...
public:
    ...
    virtual void discoverState(_ConstStateRef sp) {
        ...
        for (typename _State::const_iterator a = sp->begin(); a != sp->end(); ++a) {
            ...
            if (semiring->compare(w, minWeight_) < 0)
                minWeight_ = w;
        }
    }
...
template<class _Automaton>
typename _Automaton::ConstRef prunePosterior(typename _Automaton::ConstRef f, const typename _Automaton::Weight& threshold, bool relative) {
    return typename _Automaton::ConstRef(new PosteriorPruneAutomaton<_Automaton>(f, threshold, relative));
}

template<class _Automaton>
typename _Automaton::ConstRef prunePosterior(typename _Automaton::ConstRef f, const typename _Automaton::Weight& threshold,
    ...
    void calculateSlice() const {
        ...
        for (std::vector<Fsa::StateId>::iterator i = states.begin(); i != states.end(); ++i) {
            ...
            if (semiring->compare(potentials_[*i], worst) < 0) {
                ...
                potentials_[*i] = maxWeight_;
        }
    }
    virtual _ConstStateRef getState(Fsa::StateId s) const {
        if (s < slice_.size()) {
            if (slice_[s] >= minPotentials_.size() - 1)
                calculateSlice();
...
template<class _Automaton>
typename _Automaton::ConstRef pruneSync(typename _Automaton::ConstRef f, const typename _Automaton::Weight& threshold) {
