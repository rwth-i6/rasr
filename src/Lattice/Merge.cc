...
struct Context : public std::pair<Lm::History, WordBoundary> {
    ...
    Context()
            : Precursor(Lm::History(), WordBoundary()) {}
...
typedef std::unordered_map<Context, Fsa::StateId, ContextHash, ContextEquality>        IndexMap;
typedef std::unordered_set<ArcWithContext, ArcWithContextHash, ArcWithContextEquality> ArcsWithContext;
typedef std::unordered_map<Context, ArcsWithContext, ContextHash, ContextEquality>     ArcsWithContextMap;
typedef std::unordered_set<Context, ContextHash, ContextEquality>                      Contexts;
...
private:
    ...
    typedef std::unordered_set<Hypothesis, HypothesisHash, HypothesisEquality> CorrectHypotheses;
...
public:
    ...
    virtual std::string describe() const {
        return "turn-off-competing(" + fsa_->describe() + ")";
    }
};
