...
class RemoveEpsilons : public ModifyAutomaton<_Automaton> {
private:
...
public:
    RemoveEpsilons(_ConstAutomatonRef f)
            : Precursor(cache<_Automaton>(sort<_Automaton>(f, Fsa::SortTypeByInputAndTarget))) {
        if (f->type() != Fsa::TypeAcceptor)
            std::cerr << "epsilon-removal does only work for acceptors at the moment" << std::endl;
    }
    ...
    virtual void modifyState(_State* sp) const {
            ...
            for (typename _State::const_iterator a = _sp->begin(); a != _sp->end(); ++a) {
                ...
                if ((a->input() == Fsa::Epsilon) && (!done[a->target()])) {
                    if (!distancesCalculated) {
                        distances           = sssp(Precursor::fsa_, sp->id(), IsInputLabel<_Automaton>(Fsa::Epsilon));
                        distancesCalculated = true;
                    }
                    done[a->target()] = 1;
                    closure.insert(EpsPath(a->target(), distances[a->target()]));
                    S.push(a->target());
                }
            }
        }
        ...
        for (typename std::set<EpsPath, EpsPathCmp>::const_iterator qw = closure.begin(); qw != closure.end(); ++qw) {
            ...
            for (typename _State::const_iterator a = _sp->begin(); a != _sp->end(); ++a) {
                if (a->input() != Fsa::Epsilon) {
                    ...
                    typename _State::iterator pos = sp->lower_bound(tmp, byArc<_Automaton>(this->semiring()));
                    if ((pos == sp->end()) || (pos->input() != a->input()) || (pos->target() != a->target()))
                        sp->insert(pos, tmp);
                    else
 ...
 */
template<class _Automaton>
class SimpleRemoveEpsilonsAutomaton : public ModifyAutomaton<_Automaton> {
private:
    typedef ModifyAutomaton<_Automaton> Precursor;
...
private:
    ...
    public:
        ...
        bool operator()(const Arc& a) const {
            return (a.input() == Fsa::Epsilon) && (a.output() == Fsa::Epsilon) && (semiring_->compare(a.weight(), semiring_->one()) == 0);
        }
    };
    ConstSemiringRef  semiring_;
...
public:
    ...
    virtual void modifyState(State* sp) const {
        ...
        for (typename State::iterator a = sp->begin(); a != sp->end(); ++a) {
            if (inDegree_[a->target()] == 1) {
                ...
                if ((!_sp->isFinal()) && (_sp->nArcs() == 1)) {
                    ...
                    if ((_a->input() == Fsa::Epsilon) && (_a->output() == Fsa::Epsilon)) {
                        a->target_ = _a->target_;
                        a->weight_ = semiring_->extend(a->weight(), _a->weight());
                    }
                }
            }
        }
        ...
        while (!S.empty()) {
            ...
            for (typename State::const_iterator a = _sp->begin(); a != _sp->end(); ++a)
                if ((a->input() == Fsa::Epsilon) && (a->output() == Fsa::Epsilon) && (Precursor::fsa_->semiring()->compare(a->weight(), Precursor::fsa_->semiring()->one()) == 0)) {
                    mergeStates.push_back(a->target());
                    S.push_back(a->target());
                }
        }
        sp->remove(IsEpsilonArc(Precursor::fsa_->semiring()));
        for (std::vector<Fsa::StateId>::const_iterator i = mergeStates.begin(); i != mergeStates.end(); ++i) {
