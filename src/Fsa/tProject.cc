...
public:
    ProjectInputAutomaton(_ConstAutomatonRef f)
            : Precursor(f) {
    ...
    virtual Fsa::ConstAlphabetRef getOutputAlphabet() const {
        return Precursor::fsa_->getInputAlphabet();
    }
...
public:
    ProjectOutputAutomaton(_ConstAutomatonRef f)
            : Precursor(f) {
    ...
    virtual Fsa::ConstAlphabetRef getInputAlphabet() const {
        return Precursor::fsa_->getOutputAlphabet();
    }
    virtual void modifyState(_State* sp) const {
        for (typename _State::iterator a = sp->begin(); a != sp->end(); ++a)
            a->input_ = a->output_;
...
public:
    InvertAutomaton(_ConstAutomatonRef f)
            : Precursor(f) {
