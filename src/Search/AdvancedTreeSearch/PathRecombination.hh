...
private:
  ...
  struct RecombinationState {
    RecombinationState() : state( 0 ), loop( false ) {
    }
    ...
    struct Successor {
      Successor() : shortestDistance( Core::Type<u32>::max ), longestDistance( 0 ), state( 0 ) {
      }
  ...
  struct DistanceState {
    struct DistanceItem {
      DistanceItem() : shortestDistance( Core::Type<u32>::max ) {
      }
      u32 shortestDistance;
    };
