...
public:
    ...
    void setMinimumFrequency(Frequency f) {
        if (minimumFrequency_ != f) {
            minimumFrequency_ = f;
            needInit_         = true;
        }
    }
    void setMaximumFrequency(Frequency f) {
        if (maximumFrequency_ != f) {
            maximumFrequency_ = f;
            needInit_         = true;
        }
    }
    ...
    bool shouldWarpDifferentialUnit() const {
        return shouldWarpDifferenctialUnit_;
    }
    void setWarpCenterPositions(bool should) {
        if (shouldWarpCenterPositions_ != should) {
            shouldWarpCenterPositions_ = should;
            needInit_                  = true;
        }
    }

    void setNormalizationType(NormalizationType type) {
        if (normalizationType_ != type) {
            normalizationType_ = type;
            needInit_          = true;
        }
    }
...
private:
    ...
    void setFilteringIntervalStart(f64 interval) {
        if (filteringIntervalStart_ != interval) {
            filteringIntervalStart_ = interval;
            needInit_               = true;
        }
    }
    void setFilteringInterval(f64 interval) {
        if (filteringInterval_ != interval) {
            filteringInterval_ = interval;
            needInit_          = true;
        }
    }
