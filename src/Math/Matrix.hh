...
#if 0
    template<class T, class P>
    T Matrix<T, P>::determinant()const  {
        require(nRows()==nColumns());

        double d=1.0;
        std::vector<int> indx(nRows());

        Matrix<T> tmp_mat= ludcmp(indx, d);
        for (u32 i=0; i< nRows(); i++) d*=tmp_mat[i][i];

        return d;
    }

    template<class T, class P>
    T Matrix<T, P>::logDeterminant() const{
        require(nRows()==nColumns());
        ...
        Matrix<T> tmp_mat=ludcmp(indx, d);
        d=0.0;
        for (u32 i=0; i< nRows(); i++) d+=log(fabs(tmp_mat[i][i]));
        return d;
    }

    template<class T, class P>
    Matrix<T, P> Matrix<T, P>::invert() const{
        require(nRows()==nColumns());

        Matrix<T, P> inv_mat(nRows()),tmp_mat(nRows());
        double d=1.0;
        std::vector<int> indx(nRows());
        std::vector<double> col (nRows(),0.0);

        tmp_mat=ludcmp(indx, d);
        for (u32 j=0; j<nRows(); j++){
            for(u32 i=0; i<nRows(); i++) col[i]=0.0;
            col[j]=1.0;
            tmp_mat.lubksb(indx,col);
            for(u32 i=0; i<nRows(); i++) inv_mat[i][j]=col[i];
        }
        return inv_mat;
    }
#endif
