...
public:
    ...
    void setPreconditioner(CgPreconditioner<S, T>* preconditioner) {
    ...
    CgPreconditioner<S, T>* getPreconditioner() {
...
int LinearConjugateGradient<S, T>::terminationCriterionApplies(u32 iter) {
    ...
    if (configuration.terminateBasedOnAverageObjectiveFunction_ && iter >= configuration.averagingHistoryLength_) {
        u32 historyLength = configuration.dynamicAveragingHistoryLength_ ? std::max(configuration.averagingHistoryLength_, (u32)std::ceil(1.0 / configuration.averagingHistoryLength_ * iter)) : configuration.averagingHistoryLength_;
