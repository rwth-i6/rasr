...
struct LinearCombination {
    ScoreList scales;
    LinearCombination(const ScoreList& scales)
            : scales(scales) {}
...
struct Constant {
    ...
    Constant(const Fsa::Weight& constWeight)
    ...
    if (!l->getTopologicalSort()) {
        ...
        if (topologicalSort.maxSid == Fsa::InvalidStateId) {
            ...
            for (StateMap::const_iterator itSid = topologicalSort.begin(), endSid = topologicalSort.end();
                 itSid != endSid; ++itSid, ++order)
                topologicalOrder[*itSid] = order;
...
public:
    TopologicalOrderLattice(ConstLatticeRef l)
            : Precursor(l) {
