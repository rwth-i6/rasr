...
class ComposeAutomaton : public Lm::ComposeAutomaton {
protected:
...
class LemmaComposeAutomaton : public ComposeAutomaton {
...
class LemmaPronunciationComposeAutomaton : public ComposeAutomaton {
    typedef ComposeAutomaton Precursor;
...
public:
    LeftComposeMapping(Core::Ref<const Lm::ComposeAutomaton> f)
            : fsa_(f) {
        require(fsa_);
    }
    Fsa::StateId map(Fsa::StateId target) const {
        return fsa_->leftStateId(target);
    }
};
