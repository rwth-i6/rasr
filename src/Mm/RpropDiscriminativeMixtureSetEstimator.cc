...
bool RpropDiscriminativeMixtureSetEstimator::accumulateMixture(Core::BinaryInputStreams& is,
...
void RpropDiscriminativeMixtureSetEstimator::loadStepSizes() {
    Core::Ref<MixtureSet> stepSizes = Mm::Module::instance().readMixtureSet(select("old-step-sizes-mixture-set"));
    if (stepSizes) {
        ...
        distributeStepSizes(paramInitialStepSizeWeights(config),
...
void RpropDiscriminativeMixtureSetEstimator::storeStepSizes() {
    ...
    Mm::Module::instance().writeMixtureSet(paramNewStepSizesFilename(config), stepSizes);
}

bool RpropDiscriminativeMixtureSetEstimator::distributeStepSizes(const MixtureSet& stepSizes) {
    ...
    for (MeanIndex i = 0; i < stepSizes.nMeans(); ++i) {
        required_cast(RpropDiscriminativeMeanEstimator*, indexMaps.meanMap()[i].get())
                ->setStepSizes(stepSizes.mean(i));
    }
    if (stepSizes.nCovariances() != indexMaps.covarianceMap().size()) {
        return false;
    }
    for (CovarianceIndex i = 0; i < stepSizes.nCovariances(); ++i) {
        required_cast(RpropDiscriminativeCovarianceEstimator*, indexMaps.covarianceMap()[i].get())
                ->setStepSizes(stepSizes.covariance(i));
    }
    log("distribute step sizes ...done");
    return true;
}

bool RpropDiscriminativeMixtureSetEstimator::distributeStepSizes(Weight stepSizeWeights,
                                                                 Weight stepSizeMeans,
                                                                 Weight stepSizeVariances) {
    MixtureSetEstimatorIndexMap indexMaps(*this);
    ...
    for (MeanIndex i = 0; i < indexMaps.meanMap().size(); ++i) {
        required_cast(RpropDiscriminativeMeanEstimator*, indexMaps.meanMap()[i].get())
                ->setStepSizes(stepSizeMeans);
    }
    for (CovarianceIndex i = 0; i < indexMaps.covarianceMap().size(); ++i) {
        required_cast(RpropDiscriminativeCovarianceEstimator*, indexMaps.covarianceMap()[i].get())
                ->setStepSizes(stepSizeVariances);
    }
...
void RpropDiscriminativeMixtureSetEstimator::collectStepSizes(MixtureSet& stepSizes) {
    stepSizes.clear();
    ...
    for (DensityIndex i = 0; i < indexMaps.densityMap().size(); ++i) {
        stepSizes.addDensity(i, required_cast(RpropDiscriminativeGaussDensityEstimator*,
                                              indexMaps.densityMap()[i].get())
                                        ->collectStepSizes(indexMaps.meanMap(), indexMaps.covarianceMap()));
    }
    for (MeanIndex i = 0; i < indexMaps.meanMap().size(); ++i) {
        stepSizes.addMean(i, required_cast(RpropDiscriminativeMeanEstimator*,
                                           indexMaps.meanMap()[i].get())
                                     ->collectStepSizes());
    }
    for (CovarianceIndex i = 0; i < indexMaps.covarianceMap().size(); ++i) {
...
void RpropDiscriminativeMixtureSetEstimator::loadPreviousToPreviousMixtureSet() {
    Core::Ref<MixtureSet> p2pMixtureSet = Mm::Module::instance().readMixtureSet(select("previous-to-previous-mixture-set"));
    if (p2pMixtureSet) {
...
bool RpropDiscriminativeMixtureSetEstimator::distributePreviousToPreviousMixtureSet(
    ...
    for (DensityIndex i = 0; i < p2pMixtureSet.nDensities(); ++i) {
        required_cast(RpropDiscriminativeGaussDensityEstimator*, indexMaps.densityMap()[i].get())
                ->setPreviousToPreviousDensity(p2pMixtureSet.density(i));
    }
    ...
    for (MeanIndex i = 0; i < p2pMixtureSet.nMeans(); ++i) {
        required_cast(RpropDiscriminativeMeanEstimator*, indexMaps.meanMap()[i].get())
                ->setPreviousToPreviousMean(p2pMixtureSet.mean(i));
    }
    ...
    for (CovarianceIndex i = 0; i < p2pMixtureSet.nCovariances(); ++i) {
        required_cast(RpropDiscriminativeCovarianceEstimator*, indexMaps.covarianceMap()[i].get())
                ->setPreviousToPreviousCovariance(p2pMixtureSet.covariance(i));
    }
...
void RpropDiscriminativeMixtureSetEstimator::initialize(const MixtureSetEstimatorIndexMap& indexMaps,
...
void RpropDiscriminativeMixtureSetEstimator::finalize(MixtureSet&                        toEstimate,
 ...
 */
RpropDiscriminativeMixtureSetEstimatorWithISmoothing::RpropDiscriminativeMixtureSetEstimatorWithISmoothing(const Core::Configuration& c)
        : Core::Component(c),
          ISmoothing(c),
          Precursor(c) {
    ISmoothing::set(this);
...
RpropDiscriminativeMixtureEstimatorWithISmoothing* RpropDiscriminativeMixtureSetEstimatorWithISmoothing::createMixtureEstimator() {
    return new RpropDiscriminativeMixtureEstimatorWithISmoothing(mixtureEstimatorConfig_);
}

RpropDiscriminativeGaussDensityEstimatorWithISmoothing* RpropDiscriminativeMixtureSetEstimatorWithISmoothing::createDensityEstimator() {
    return new RpropDiscriminativeGaussDensityEstimatorWithISmoothing;
}

RpropDiscriminativeGaussDensityEstimatorWithISmoothing* RpropDiscriminativeMixtureSetEstimatorWithISmoothing::createDensityEstimator(const GaussDensity&) {
    return new RpropDiscriminativeGaussDensityEstimatorWithISmoothing;
}

RpropDiscriminativeMeanEstimatorWithISmoothing* RpropDiscriminativeMixtureSetEstimatorWithISmoothing::createMeanEstimator() {
    return new RpropDiscriminativeMeanEstimatorWithISmoothing;
}

RpropDiscriminativeMeanEstimatorWithISmoothing* RpropDiscriminativeMixtureSetEstimatorWithISmoothing::createMeanEstimator(const Mean& mean) {
    return new RpropDiscriminativeMeanEstimatorWithISmoothing(mean.size());
}

RpropDiscriminativeCovarianceEstimatorWithISmoothing* RpropDiscriminativeMixtureSetEstimatorWithISmoothing::createCovarianceEstimator() {
    return new RpropDiscriminativeCovarianceEstimatorWithISmoothing;
}

RpropDiscriminativeCovarianceEstimatorWithISmoothing* RpropDiscriminativeMixtureSetEstimatorWithISmoothing::createCovarianceEstimator(const Covariance& covariance) {
...
void RpropDiscriminativeMixtureSetEstimatorWithISmoothing::finalize(MixtureSet&                        toEstimate,
                                                                    const MixtureSetEstimatorIndexMap& indexMaps,
                                                                    const CovarianceToMeanSetMap&      meanSetMap) {
...
void RpropDiscriminativeMixtureSetEstimatorWithISmoothing::setIMixture(Core::Ref<AbstractMixtureEstimator> estimator,
...
void RpropDiscriminativeMixtureSetEstimatorWithISmoothing::setIDensity(Core::Ref<GaussDensityEstimator> estimator,
                                                                       const GaussDensity*              density) {
    required_cast(RpropDiscriminativeGaussDensityEstimatorWithISmoothing*, estimator.get())->setIDensity(density);
}

void RpropDiscriminativeMixtureSetEstimatorWithISmoothing::setIMean(Core::Ref<AbstractMeanEstimator> estimator,
                                                                    const Mean*                      mean,
                                                                    Weight                           iSmoothing) {
    RpropDiscriminativeMeanEstimatorWithISmoothing* _estimator = required_cast(RpropDiscriminativeMeanEstimatorWithISmoothing*, estimator.get());
    _estimator->setIMean(mean);
    _estimator->setConstant(iSmoothing);
}

void RpropDiscriminativeMixtureSetEstimatorWithISmoothing::setICovariance(Core::Ref<AbstractCovarianceEstimator> estimator,
                                                                          const Covariance*                      covariance,
                                                                          Weight                                 iSmoothing) {
    RpropDiscriminativeCovarianceEstimatorWithISmoothing* _estimator = required_cast(RpropDiscriminativeCovarianceEstimatorWithISmoothing*, estimator.get());
