...
TreeBuilder::TreeBuilder( Core::Configuration config, const Bliss::Lexicon& lexicon, const Am::AcousticModel& acousticModel, Search::PersistentStateTree& network, bool initialize, bool arcBased ) :
  lexicon_( lexicon ),
  ...
  if( allowCrossWordSkips_ )
  {
    ...
    for ( u32 t = 0; t < acousticModel_.nStateTransitions(); ++t ) {
      ...
      if( modelPenalty != skipPenalty )
        Core::Application::us()->warning() << "Inconsistency for forward/backward decoding: Transition model " << t << ": skip penalty differs from previous value: " << modelPenalty << " (previous value " << skipPenalty << ")";
      if( modelForwardPenalty != forwardPenalty )
        Core::Application::us()->warning() << "Inconsistency for forward/backward decoding: Transition model " << t << ": forward penalty differs from previous value: " << modelForwardPenalty << " (previous value " << forwardPenalty << ")";
    }
  }
...
void TreeBuilder::hmmFromAllophone( TreeBuilder::HMMSequence& ret, Bliss::Phoneme::Id left, Bliss::Phoneme::Id central, Bliss::Phoneme::Id right, u32 boundary, bool allowNonStandard )
{
  verify( ret.length == 0 );
...
void TreeBuilder::build() {
  ...
  log() << "coarticulated initial phones: " << coarticulatedInitial << " uncoarticulated: " << uncoarticulatedInitial << ", coarticulated final phones: " << uncoarticulatedFinal << " uncoarticulated: " << uncoarticulatedFinal;
...
{
//   verify( ( left != Bliss::Phoneme::term && right != Bliss::Phoneme::term ) ||
//     ( left == Bliss::Phoneme::term && right == Bliss::Phoneme::term ) );
...
std::pair<TreeBuilder::StateId, TreeBuilder::StateId> TreeBuilder::extendPhone( StateId currentState, u32 phoneIndex, const std::vector<Bliss::Phoneme::Id>& phones, Bliss::Phoneme::Id left, Bliss::Phoneme::Id right )
{
  u8 boundary = 0;
  if( phoneIndex != 0 )
...
u32 TreeBuilder::addExit( StateId prePredecessor, StateId predecessor, Bliss::Phoneme::Id leftPhoneme, Bliss::Phoneme::Id rightPhoneme, int depth, Bliss::LemmaPronunciation::Id pron )
{
  PersistentStateTree::Exit exit;
...
//   if(allowCrossWordSkips_)
//   {
//     if( network_.rootTransitDescriptions.count(prePredecessor) )
//     {
//       std::cout << "not adding skip-transition on root-node " << prePredecessor << " for " << lexicon_.lemmaPronunciation(pron)->lemma()->symbol() << std::endl;
//     }else{
//       PersistentStateTree::Exit skipExit;
//       skipExit.transitState = createSkipRoot( exit.transitState );
//       skipExit.pronunciation = pron;
//
//       u32 skipExitIndex = createExit( skipExit );
//
//       bool foundSkipExit = false;
//       for( HMMStateNetwork::SuccessorIterator target = network_.structure.successors( prePredecessor ); target; ++target )
//         if( target.isLabel() && target.label() == skipExitIndex )
//           foundSkipExit = true;
//
//       if(!foundSkipExit)
//         network_.structure.addOutputToNode( prePredecessor, ID_FROM_LABEL( skipExitIndex ) );
//     }
//   }
...
TreeBuilder::StateId TreeBuilder::extendBodyState( StateId state, Bliss::Phoneme::Id first, Bliss::Phoneme::Id second, Search::StateTree::StateDesc desc )
{
