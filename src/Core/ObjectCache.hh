...
private:
    ...
    ObjectCacheItem<Data>& operator=(const ObjectCacheItem<Data>& v) {
        return *this;
    }
...
public:
    ...
    void write(BinaryOutputStream& os) {
        data_->write(os);
        dirty_ = false;
    }
...
public:
    ...
    typedef std::unordered_map<Key, Item*, _HashFunction, _EqualKey> HashMap;
...
public:
    ...
    Iterator last() {
        Iterator result = find(mruList_.back());
        ensure(result != end());
        return result;
    }
...
public:
    ...
    Iterator last() {
        ...
        Iterator result      = find(keys_[randomIndex]);
        ensure(result != end());
        return result;
    }
...
const ParameterInt    parameterObjectCacheSize("size", "max number of objects in the cache memory", 1, 1);
const ParameterString parameterObjectCacheDirectory("directory", "directory of cached objects");
const ParameterBool   parameterObjectCacheDump("dump", "on/off dumping of objects", false);
 ...
 */
template<class List>
class ObjectCache : public virtual Component {
private:
...
template<class List>
ObjectCache<List>::ObjectCache(const Configuration& c, u8 mode)
        : Component(c),
          maxSize_(0),
          mode_(mode),
          dump_(false) {
