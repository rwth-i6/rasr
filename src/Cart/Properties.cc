...
void PropertyMapDiff::logKey(Core::XmlWriter&   xml,
...
void PropertyMapDiff::logUniq(Core::XmlWriter& xml,
                              std::string      key,
                              u16              diff) {
...
void Properties::write(std::ostream& os) const {
    ...
    for (size_t i = 0; i < map_->size(); ++i)
        os << std::setw(3) << std::right << (i + 1) << ". "
           ...
           << ((map_->isDefined(id = operator[](i))) ? (*map_)[i][id] : "<undefined>")
           << std::endl;
}
...
// ============================================================================
StoredProperties::StoredProperties(const Properties& props)
...
StoredProperties::StoredProperties(PropertyMapRef    map,
