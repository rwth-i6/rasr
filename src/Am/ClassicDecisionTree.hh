 ...
 * + Conditions
 */
class PropertyMap : public Cart::PropertyMap {
protected:
 ...
 */
class PropertyMapDiff : Core::Component {
private:
    /**
...
private:
    void logDiff(Core::XmlWriter&   xml,
                 PropertyMap::Index l,
                 PropertyMap::Index r);

public:
    PropertyMapDiff(const Core::Configuration& config,
...
class Properties : public Cart::Properties {
protected:
...
public:
    ...
    Properties(Cart::PropertyMapRef map, const AllophoneState& alloState)
            : Precursor(map),
              map_(dynamic_cast<const PropertyMap&>(*(map.get()))),
              alloState_(),
              cond_(0) {
        ...
        switch (keyIndex) {
            ...
            default:
                ...
                if (contextIndex < Index(map_.historySize())) {
                    return (contextIndex < maxHistorySize_) ? Index(allo.history()[Index(map_.historySize()) - contextIndex - 1]) : map_.termIndex;
                    //			map_.undefinedIndex;
                }
                ...
                if (contextIndex < Index(map_.futureSize())) {
                    return (contextIndex < maxFutureSize_) ? Index(allo.future()[contextIndex]) : map_.termIndex;
                    //			map_.undefinedIndex;
                }
                if (keyIndex < Index(map_.size()))
                    return (cond_) ? Index(cond_->hasCondition(map_.key(keyIndex))) : Index(false);
...
class DecisionTree : public Cart::DecisionTree {
protected:
    typedef DecisionTree       Self;
    typedef Cart::DecisionTree Precursor;

public:
    DecisionTree(const Core::Configuration& config, ClassicStateModelRef stateModel)
