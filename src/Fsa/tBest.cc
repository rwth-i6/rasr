...
public:
    ...
    virtual std::string describe() const {
        return "best(" + Precursor::fsa_->describe() + ")";
    }
};
...
template<class _Automaton>
typename _Automaton::ConstRef best(typename _Automaton::ConstRef f, const StatePotentials<typename _Automaton::Weight>& backward) {
    ...
    NBestTraces(_ConstAutomatonRef f, size_t n, bool bestSequences)
        ...
        while (!S.empty()) {
            ...
            if (sp->isFinal()) {
                ...
                if (finals_.size() == n + 1)
                    break;  // stop after n final traces
            }
...
public:
    ...
    virtual std::string describe() const {
        return "nbest(" + Precursor::fsa_->describe() + ")";
    }
    ...
    virtual _ConstStateRef getState(Fsa::StateId s) const {
        ...
        if (s == 0) {
            ...
            if (trace.predecessor_) {
                ...
                sp = new _State(s, Fsa::StateTagFinal, Precursor::fsa_->semiring()->one());
        }
        return _ConstStateRef(sp);
    }
...
private:
    ...
    class TracePool : public std::vector<Trace> {
    public:
    ...
    struct Hyp {
        ...
        struct Key {
            Fsa::StateId operator()(const Hyp& hyp) const {
                return hyp.state;
            }
        };
...
public:
    ...
    virtual std::string describe() const {
        return "firstbest(" + Precursor::fsa_->describe() + ")";
    }
