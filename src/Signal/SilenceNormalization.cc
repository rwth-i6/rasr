...
{
public:
    GaussianDensity(f32 bias = 1.0) : mean_(0), variance_(0), sigma_(0), offset_(0), energySum_(0), energySquareSum_(0), energyCount_(0), bias_(bias) {
    }
...
bool SilenceNormalization::flush(Vector<Sample> &out) {
    ...
    std::sort(sortedBlocks.begin(), sortedBlocks.end(), Core::composeBinaryFunction(std::less<f64>(),
            Core::select1st<std::pair<f64, u32> >(), Core::select1st<std::pair<f64, u32> >()));
    ...
    for(u32 it = 0; it < iterations_; ++it)
    {
        if(it > 0)
        {
            ...
            {
                isSpeech[block] =
                    (blocks[block] - silence.mean() >= (speech.mean() - silence.mean()) * silenceThreshold_) &&
                    blocks[block] > absoluteSilenceThreshold_;
            }
        }
...
/*    for( u32 block = 0; block < blocks.size(); ++block )
        std::cout << block << "   " << blocks[block] << "   " << (isSpeech[block] ? "speech" : "silence") << std::endl;    */
...
//       msg << "silence-normalization warping map:";
//       u32 outIdx = 0;
//       Time bufferStartTime = buffer_[0].first;
//       for(u32 block = 0; block < isSpeech.size(); ++block)
//       {
//         if(isSpeech[block])
//         {
//           if(block == 0 || !isSpeech[block-1])
//             msg << " " << (((f32)(outIdx * blockSizeSamples_)) / sampleRate_ + bufferStartTime) << ":" << (((f32)(block * blockSizeSamples_)) / sampleRate_ + bufferStartTime);
//           ++outIdx;
//         }
//       }
//     }
