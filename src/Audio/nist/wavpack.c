...
static int evaluate_strat(int flags);
static void evaluate_chan(int flags, struct strat_eval *f);
static void output_strat(int flags, FILE *ofile);
static void output_chan(int flags, FILE *ofile);
static int readchan(FILE *ifile, struct hdr *hp, unsigned char *ibuf);
static void decode_data(struct hdr *hp, unsigned char *ibuf, register short unsigned int *sobuf, int *nsobuf);
static void write_data(struct hdr *hdr, short unsigned int (*sobuf)[257], int *nsobuf, FILE *ofile);
static void printstrat(int x);
void perr(char *s);
...
static void evaluate_chan(int flags, struct strat_eval *f)		/* evaluate channel n */
