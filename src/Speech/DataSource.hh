 ...
 */
class DataSource : public Flow::DataSource {
protected:
...
public:
     ...
     */
    template<class T>
    bool getData(Flow::DataPtr<T>& d) {
        return getData(mainPortId_, d);
    }
     ...
     */
    virtual bool getData(Core::Ref<Feature>& f) {
        return getData(mainPortId_, f);
    }
    /** Pulls data from main port of the network
     *  @return is false if no output could be delivered.
     */
    virtual bool getData() {
        Flow::DataPtr<Flow::Data> d;
        return getData(mainPortId_, d);
    }
    ...
    Flow::PortId mainPortId() const {
        return mainPortId_;
    }

    /** @return number of frames per port */
    const std::vector<size_t>& nFrames() const {
        return nFrames_;
    }
    Flow::Time realTime() const {
        return endTime_ - startTime_;
    }

    void setProgressIndication(bool enabled) {
        noProgressIndication_ = !enabled;
    }
};
