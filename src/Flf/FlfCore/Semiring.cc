...
// -------------------------------------------------------------------------
constexpr Score               Semiring::Zero;
constexpr Score               Semiring::One;
constexpr Score               Semiring::Max;
constexpr Score               Semiring::Invalid;
constexpr Score               Semiring::DefaultScale;
constexpr Score               Semiring::UndefinedScale;
constexpr ScoreId             Semiring::InvalidId;
constexpr Semiring::Tolerance Semiring::DefaultTolerance;
...
bool Semiring::operator==(const Semiring& semiring) const {
    if (type() != semiring.type())
        return false;
    if (scales() != semiring.scales())
        return false;
    if (keys() != semiring.keys())
        return false;
    return true;
}
...
union f32_u32_union {
    f32        f;
    u32        u;
    inline u32 operator()(f32 x) {
        f = x;
        return u;
    }
    inline f32 operator()(u32 x) {
        u = x;
        return f;
    }
...
public:
    Internal(Core::XmlContext* context)
            : context_(context) {
        reset();
        build();
