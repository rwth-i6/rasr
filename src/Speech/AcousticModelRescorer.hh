...
namespace Speech {
    ...
    template<class T> class AcousticModelRescorer : public Core::Configurable {
    private:
    ...
    public:
        AcousticModelRescorer(const Core::Configuration &c) : Core::Configurable(c), scorer_(0), rescorer_(0) {
            beamCount_ = 0;
        }
        ...
        void setAcousticModel(const Mm::FeatureScorer *scorer) {
            scorer_ = scorer;
            if (rescorer_) rescorer_->restart();
        }
        ...
        void setBeamCount(typename T::Weight beamThreshold) { beamThreshold_ = beamThreshold; }
        void setBeamThreshold(u32 beamCount) { beamCount_ = beamCount; }
        ...
        template<class Trace> void getBestPath(Trace &t) const {
            Fsa::write(trace_, "/tmp/trace.fsm.gz"); // DEBUG
        }
    };
