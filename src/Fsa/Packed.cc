...
void PackedAutomaton::setState(State* sp) {
    ...
    if (arcs_.size() > 0)
        arcs_.pop_back();  // rewind as we can overwrite the automaton end marker
    states_[sp->id()].start_ = arcs_.size();
    u8 partition             = (sp->tags() >> StateIdBits) << 5;
    if (sp->hasTags(StateTagFinal))
        if (!semiring()->isDefault(sp->weight_))
            partition |= 0x01;
    arcs_.push_back(partition | 0x1c);
    if (partition & 0x01)
        appendBytes(arcs_, sp->weight_, sizeof(Weight));
    ...
    for (State::const_iterator a = sp->begin(); a != sp->end(); ++a) {
        ...
        if (!semiring()->isDefault(a->weight()))
            partition |= 0x20;
        int i = 0;
        if (a->input() != Epsilon)
            i = estimateBytes(a->input());
        int o = 0;
        if (type() != TypeAcceptor)
            if (a->output() != Epsilon)
                o = estimateBytes(a->output());
        partition |= (5 * o) + i;
        arcs_.push_back(partition);

        appendBytes(arcs_, a->target(), (partition >> 6) + 1);
        if (partition & 0x20)
            appendBytes(arcs_, a->weight(), sizeof(Weight));
        appendBytes(arcs_, a->input(), i);
        if (type() != TypeAcceptor)
            appendBytes(arcs_, a->output(), o);
        nArcs_++;
    }
...
ConstStateRef PackedAutomaton::getState(StateId s) const {
    if (s < states_.size()) {
        ...
        if (partition & 0x01)
            sp->weight_ = Weight(getBytesAndIncrement(a, sizeof(Weight)));
        else
        ...
        for (; ((partition = *(a++)) & 0x1c) != 0x1c;) {
            ...
            if (partition & 0x20)
                arc->weight_ = Weight(u32(getBytesAndIncrement(a, sizeof(Weight))));
            else
                arc->weight_ = semiring()->one();
            u8 io = partition & 0x1f;
            if (io % 5)
                arc->input_ = getBytesAndIncrement(a, io % 5);
            else
                arc->input_ = Epsilon;
            if (type() != TypeAcceptor) {
                if (io / 5)
                    arc->output_ = getBytesAndIncrement(a, io / 5);
                else
                    arc->output_ = Epsilon;
            }
            else
                arc->output_ = arc->input();
        }
...
size_t PackedAutomaton::getMemoryUsed() const {
    size_t size = 0;
    if (input_)
        size += input_->getMemoryUsed();
    if (output_)
        size += output_->getMemoryUsed();
