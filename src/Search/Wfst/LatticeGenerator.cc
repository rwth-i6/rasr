...
LatticeTraceRecorder::LatticeTraceRecorder(const StateSequenceList &hmms) :
        hmmsBegin_(&hmms.front()), lattice_(new Lattice),
        silence_(OpenFst::InvalidLabelId), silenceOutput_(OpenFst::InvalidLabelId),
        mergeSilence_(true),
...
{
    if (mergeSilence_) reviseSilenceLabels();
}
...
#if 0
static int DBG_LAT_CNT = 0;
void WriteLattice(const Search::Wfst::Lattice &lattice, const std::vector<bool> &active, const std::string &filename) {
    typedef Search::Wfst::Lattice Lattice;
    Search::Wfst::Lattice *tmp = lattice.Copy();
    OpenFst::StateId final = tmp->AddState();
    tmp->SetFinal(tmp->Start(), Lattice::Weight::One());
    tmp->SetStart(final);
    for (FstLib::StateIterator<Lattice> siter(*tmp); !siter.Done();
            siter.Next()) {
        OpenFst::StateId s = siter.Value();
        if (active[s]) tmp->AddArc(final,
                Lattice::Arc(0, 0, Lattice::Weight::One(), s));
    }
    FstLib::Connect(tmp);
    Lattice out;
    FstLib::Reverse(*tmp, &out);
    delete tmp;
    out.Write(filename);
}
#endif
...
void HmmLatticeTraceRecorder::prune(const std::vector<TraceRef> &finalStates)
{
    ...
    while (!queue.empty()) {
        ...
        if (deletedArcs) lattice_->DeleteArcs(s, deletedArcs);
    }
