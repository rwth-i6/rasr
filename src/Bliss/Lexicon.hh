...
public:
    ...
    const Pronunciation* pronunciation() const {
        return pronunciation_;
    }

    /** Probability of pronunciation given lemma. */
    f32 pronunciationProbability() const {
        return exp(-score_);
    }
    f32 pronunciationScore() const {
        return score_;
    }
    ...
    const LemmaPronunciation* nextForThisLemma() const {
        return nextForThisLemma_;
    }
...
public:
    ...
    Phoneme::Id leftContext() const {
        return leftContext_;
    }
    Phoneme::Id rightContext() const {
...
std::string Coarticulated<Object>::format(Core::Ref<const PhonemeInventory> phonemeInventory) const {
    const std::string l(leftContext() == Phoneme::term ? "#" : phonemeInventory->phoneme(leftContext())->symbol().str());
    const std::string r(rightContext() == Phoneme::term ? "#" : phonemeInventory->phoneme(rightContext())->symbol().str());
...
protected:
    friend class Lexicon;
    SyntacticToken(Bliss::Symbol _symbol)
            : Token(_symbol) {}
...
protected:
    friend class Lexicon;
    EvaluationToken(Bliss::Symbol _symbol)
            : Token(_symbol) {}
};
...
class Lexicon : public Core::ReferenceCounted,
...
public:
    ...
    const Core::Dependency& getDependency() const {
        return dependency_;
    }
    ...
    Core::Ref<const PhonemeInventory> phonemeInventory() const {
        return phonemeInventory_;
    }
     ...
     * suppressed.
     */
    Core::Ref<LemmaToSyntacticTokenTransducer> createLemmaToSyntacticTokenTransducer(bool useEmptySyntacticTokenSequences = true, size_t nDisambiguators = 0) const;
