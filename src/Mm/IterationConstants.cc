...
#if 0
    const CovarianceToMeanSetMap::MeanSet &meanSet =
        meanSetMap_[Core::Ref<AbstractCovarianceEstimator>(covarianceEstimator)];
    const std::vector<Sum> Gamma_x2_sl = covarianceEstimator->dtSum(meanSet);
    const std::vector<Sum> Gamma_x_sl = meanEstimator->dtSum();
    Weight Gamma_1_sl = meanEstimator->dtWeight();
#else
    require(false);
