...
SemiTiedEstimator::SemiTiedEstimator(
    ...
    stopCriterion_(paramIterationStop_(config))
{
    ...
    log("minimum number of observations for semi-tied MLLR estimation: ") <<
            minSemiTiedAdaptationObservations_;
};
...
void SemiTiedEstimator::estimateWMatrices()
{
        ...
        p!=tree_->leafList().end(); ++p)
    {
        ...
        if (qId!=id) tyingScheme[qId].insert(id); //else: qId is already leaf
    }
...
void SemiTiedEstimator::solveEstimationEquations(
    const IdSet& idSet,
    NodeId qId,
    ...
    const Math::Vector<Matrix>& g)
{
    ...
    log("number of conjugate gradient iterations for semi-tied MLLR estimation: ")
        << iterations;
    ...
    for (IdSetConstIterator c=idSet.begin(); c!=idSet.end(); ++c) {
        ...
        if (status!=0) error("Lapack routine solveLinearLeastSquares failed! status = ") << status;
        lambda[c->id]= x;
    }
...
// AdaptorEstimator* SemiTiedEstimator::clone() const
// {
//     return new SemiTiedEstimator(getConfiguration(),dimension_,
...
bool SemiTiedEstimator::read(Core::BinaryInputStream &i)
{
    ...
    minSemiTiedAdaptationObservations_=
        paramMinSemiTiedAdaptationObservations_(config, minSemiTiedAdaptationObservations_);
...
////////////////////////////////////////////////////////////////////
SemiTiedOptimizationFunction::SemiTiedOptimizationFunction(
    ComponentIndex d, const Math::Vector<Matrix> &G,
    const Math::Vector<Matrix>&Z,
    const std::map<NodeId,Math::Vector<double> >&l)
    :
    dimension_(d), G_(G), Z_(Z), lambda_(l), f1_(dimension_), f2_(dimension_) {}

SemiTiedOptimizationFunction::ResultType SemiTiedOptimizationFunction::operator()(
        const ArgumentType &x) const
{
...
////////////////////////////////////////////////////////////////////
SemiTiedOptimizationGradient::SemiTiedOptimizationGradient(
    ComponentIndex d,
    const Math::Vector<Matrix>& G,
    const Math::Vector<Matrix>& Z,
    const std::map<NodeId,Math::Vector<double> >& l)
...
Math::Vector<double> Mm::convert2Vector(
        const std::map<NodeId,Math::Vector<double> >& l,
