...
public:
    struct Statistics {
        ...
        void dump(Core::XmlChannel& xml) {
            xml << Core::XmlFull("count", nArc) + Core::XmlAttribute("name", "arc");
            xml << Core::XmlFull("count", nSlot) + Core::XmlAttribute("name", "slot");

            if (nSlot > 0) {
                xml << Core::XmlOpen("slot-statistic");
                xml << Core::XmlFull("min", minSlotSize) + Core::XmlAttribute("name", "arcs per slot");
                xml << Core::XmlFull("avg", f32(nArc) / f32(nSlot)) + Core::XmlAttribute("name", "arcs per slot");
                xml << Core::XmlFull("max", maxSlotSize) + Core::XmlAttribute("name", "arcs per slot");
                xml << Core::XmlClose("slot-statistic");
            }

            xml << Core::XmlOpen("operation-statistic");
            xml << Core::XmlFull("count", nNullArc) + Core::XmlAttribute("name", "null arc match");
            xml << Core::XmlFull("count", nNoMatch) + Core::XmlAttribute("name", "no match");
            xml << Core::XmlFull("count", nFwdMatch) + Core::XmlAttribute("name", "fwd. match");
            xml << Core::XmlFull("count", nPseudoBwdMatch) + Core::XmlAttribute("name", "pseudo bwd. match");
            xml << Core::XmlFull("count", nBwdMatchWithoutSplit) + Core::XmlAttribute("name", "bwd. match w/o split");
            xml << Core::XmlFull("count", nBwdMatchWithSplit) + Core::XmlAttribute("name", "bwd. match w/ split");
...
protected:
    const ArcIdList& sortArcsByLabel() const {
        if (sortedAids_.empty()) {
            ...
            for (ArcId aid = 0, endAid = arcs_.size(); aid < endAid; ++aid, ++itArc)
                *itArc = aid;
    ...
    inline Score similarity(const Arc& hyp, Score beginTime) const {
        Score overlapScore = hyp.end - beginTime;
        Score diffScore    = beginTime - hyp.begin;
    ...
    inline void updateBwdMatch(const ArcId hypId, const Arc& hyp, const ArcId refId, const Arc& ref, Match& bestBwdMatch) const {
        ...
        if (hyp.begin < ref.end) {
            Score hypLen  = hyp.end - hyp.begin;
            Score refLen  = ref.end - ref.begin;
            Score overlap = ref.end - hyp.begin;
            if (((hyp.label == ref.label) &&
                 (overlap > 0.25 * hypLen) &&
                 (overlap > 0.25 * refLen)) ||
                ((overlap > 0.5 * hypLen) &&
    ...
    if (newBwdArcCluster->prev->prev) {
        ...
        if (leftMerge)
            for (ArcCluster::const_iterator itLeftBwdArc  = newBwdArcCluster->prev->prev->begin(),
                 ...
                 itLeftBwdArc != endLeftBwdArc; ++itLeftBwdArc)
    ...
    if (newBwdArcCluster->next->next) {
        ...
        if (rightMerge)
            for (ArcCluster::const_iterator itRightBwdArc  = newBwdArcCluster->next->next->begin(),
                 ...
                 itRightBwdArc != endRightBwdArc; ++itRightBwdArc) {
    ...
    if (rightMerge || leftMerge) {
        ...
        if (leftMerge) {
            ...
            for (ArcCluster::iterator itBwdArc = newBwdArcCluster->begin(), endBwdArc = newBwdArcCluster->end();
                 itBwdArc != endBwdArc; ++itBwdArc)
                leftBwdArcCluster->add(*itBwdArc, arcs_[*itBwdArc]);
            ...
            for (StateCluster::iterator itBwd = newBwdArcCluster->next->begin(), endBwd = newBwdArcCluster->next->end();
                 itBwd != endBwd; ++itBwd)
                leftBwdCluster->add(*itBwd, states_[*itBwd]);
            ...
            for (ArcCluster::iterator itRightBwdArc = rightBwdArcCluster->begin(), endRightBwdArc = rightBwdArcCluster->end();
                 itRightBwdArc != endRightBwdArc; ++itRightBwdArc)
                newBwdArcCluster->add(*itRightBwdArc, arcs_[*itRightBwdArc]);
            ...
            for (StateCluster::iterator itRightBwd = rightBwdArcCluster->next->begin(), endRightBwd = rightBwdArcCluster->next->end();
                 itRightBwd != endRightBwd; ++itRightBwd)
                newBwdCluster->add(*itRightBwd, states_[*itRightBwd]);
...
void ClusteredLattice::cluster(ConstLatticeRef l, ConstFwdBwdRef fb, bool allowBwdMatch, ClusterBuilder::Statistics& statistics) {
    ...
    for (StateMap::const_iterator itSid  = chronologicalSort->begin(),
                                  endSid = chronologicalSort->end();
         itSid != endSid; ++itSid, pi.notify()) {
        // state
        ...
        u32 aid          = 0;
        for (Flf::State::const_iterator a = sr->begin(), end_a = sr->end(); a != end_a; ++a, ++aid, ++arcId) {
            ...
            if (bestBwdMatch.score > bestFwdMatch.score) {
                ...
                if ((bestFwdMatch.id != InvalidArcId) && (states_[arcs_[bestBwdMatch.id].to].cluster->id >= arcs_[bestFwdMatch.id].cluster->id)) {
                    ...
                    if (bestStayScore == Core::Type<Score>::min) {
                        ...
                        if (fromCluster->next)
                            fromCluster->next->prev = rightStateCluster;
                        if (cluster == fromCluster)
                            cluster = rightStateCluster;
                        ArcCluster* newArcCluster = leftStateCluster->next = rightStateCluster->prev = new ArcCluster(leftStateCluster, rightStateCluster);
                        ...
                        for (ArcCluster::iterator itBwdArc = bwdArcCluster->begin(), endBwdArc = bwdArcCluster->end(); itBwdArc != endBwdArc; ++itBwdArc) {
                            ...
                            if (bwdArc.cluster != 0)
                                ...
                                verify((bwdArc.to == sid) || (states_[bwdArc.to].cluster->id >= newArcCluster->id));
                                newArcCluster->add(*itBwdArc, bwdArc);
                            }
                        }
                ...
                if (op == ClusterBuilder::Statistics::PseudoBwdMatch) {
                    ...
                    for (ArcIdList::const_iterator itAid2 = fromState.bwd.begin(), endAid2 = fromState.bwd.end();
                         itAid2 != endAid2; ++itAid2)
                        if (arcs_[*itAid2].cluster->id > newFromClusterId) {
                            isConsistent = false;
                            break;
                        }
                    if (!isConsistent) {
                        ...
                        for (; *itFromCluster != fromSid; ++itFromCluster)
                            verify(itFromCluster != fromCluster->end());
        ...
        if (sum != 1.0) {
            ...
            if (dEpsConfidence < 0.0) {
                if (dEpsConfidence < -0.01)
                    Core::Application::us()->warning("PivotCnBuilder: Expected 1.0, got %f", (1.0 - dEpsConfidence));
                normalize = true;
            }
            else {
