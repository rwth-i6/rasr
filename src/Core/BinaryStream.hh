...
public:
    ...
    template<typename T>
    bool write(const T* v, std::streamsize n = 1);
...
public:
    ...
    void open(const std::string& name, std::ios_base::openmode mode = std::ios::in) {
        BinaryStreamIos::open(name, mode);
    }
    ...
    template<typename T>
    bool read(T* v, std::streamsize n = 1);
    template<typename T>
    std::streamsize readSome(T* v, std::streamsize n = 1);
...
public:
    ...
    BinaryInputStreams(const std::vector<std::string>& filenames,
                       std::ios_base::openmode         mode      = std::ios::in,
                       BinaryStreamIos::Endianess      endianess = BinaryStreamIos::defaultEndianess);
    BinaryInputStreams(std::vector<std::istream*>* istreams,
                       BinaryStreamIos::Endianess  endianess = BinaryStreamIos::defaultEndianess);

    ~BinaryInputStreams();

    BinaryInputStream& front() {
        return *Precursor::front();
    }
    BinaryInputStream& operator[](u32 n) {
        return *Precursor::operator[](n);
    }
    const BinaryInputStream& operator[](u32 n) const {
        return *Precursor::operator[](n);
    }
    bool areOpen() const;
};
