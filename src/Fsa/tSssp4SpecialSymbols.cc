            ...
            _Weight totalFlow() const {
                ...
                struct AddArcTropical : AddArc {
                        inline static _Weight failArc(const _Arc *arc, DirectHits &directHits, _Accumulator *collector, const bool firstLevel, _SsspBackward4SpecialSymbols *owner){
                                ...
                                _Weight failWeight, update;
#if 0
                                // This trick does not work, since failWeight would be wrong if the if condition is not given
                                failWeight=owner->potential_[ts];
                                update = owner->semiring_->extend(
                                                sssp4SpecialSymbolsHelper::convertWeight<_AutomataWeight,_Weight>(arc->weight()),
                                                failWeight);
                                /* if at least one path behind the fail path would change the best path,
                                 * check if there is a valid path which would change the best path.
                                 */
                                _Weight currentGet = collector->get();
                                if (owner->semiring_->collect(currentGet, update) != currentGet) {
#endif
...
#else
                                const ForwardPotential& forward=this->getFinalizedForwardPotential(*s);
                                std::cout << "State " << *s << " : ";
                                _ConstStateRef state = this->fsa_->getState(*s);
                                for (typename _State::const_iterator a=state->begin(); a!= state->end(); ++a)
                                        std::cout << a->input() << ":" << a->output() << "=" << sssp4SpecialSymbolsHelper::weight2str(forward.getForwardPotential(*a, sorter_)) << " ";
                                std::cout << std::endl;
#endif
                        }
