...
namespace Signal {
    ...
    class SegmentwiseFormantExtraction :
        public FormantExtraction
    {
    public:
        typedef f32 _float;
    ...
    public:
        SegmentwiseFormantExtraction(const Core::Configuration &c) :
            Core::Component(c), FormantExtraction(c),
            order_(0), max_nr_segment_(0), step_(0), segmentwise_estimator_(0), segment_estimator_(0) {}
        virtual ~SegmentwiseFormantExtraction()
            { if (segmentwise_estimator_) delete segmentwise_estimator_;
            if (segment_estimator_) delete segment_estimator_; }

        void setMaxNumberOfSegments(u8 max_nr_segment) {
            if (max_nr_segment_ != max_nr_segment) {
                max_nr_segment_ = max_nr_segment; setMaxNrFormant(max_nr_segment); need_init_ = true;
            }
        }
        u8 getMaxNumberOfSegments() { return max_nr_segment_; }

        void setMaxFrequency(u32 max_frequency) {
            if (max_frequency_ != max_frequency) {
                max_frequency_ = max_frequency; ; need_init_ = true;
            }
        }

        void setOrder(u8 order) { if (order_ != order) { order_ = order; need_init_ = true; } }
        ...
        bool work(const std::vector<_float>& in, std::vector<s32>& segments)
            {
                if (need_init_ && !init()) return false;

                if (!segmentwise_estimator_->setSignal(in)) return false;
    ...
    class SegmentwiseFormantExtractionNode :
        public SleeveNode, public SegmentwiseFormantExtraction
    {
    public:
    ...
    public:
        static std::string filterName() { return "signal-formant-segmentwise"; }
