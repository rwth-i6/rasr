...
public:
     ...
     */
    template<class T>
    class TypeSpecificFormats : public StringHashMap<Ref<const Format<T>>>,
...
private:
    ...
    template<class T>
    bool getFormat(const std::string& qualifier, Ref<const Format<T>>& format) const;
    bool checkFile(const std::string& filename, AccessMode mode) const;

public:
    FormatSet(const Configuration& c)
            : Component(c) {}
    ~FormatSet() {}
...
void FormatSet::registerFormat(const std::string& qualifier,
                               const Format<T>*   format,
                               bool               isDefault) {
    Ref<TypeSpecificFormats<T>> formats;
    getTypeSpecificFormats(formats);
...
struct CompressedBinaryFormat : public FormatSet::Format<T> {
    virtual bool read(const std::string& filename, T& o) const {
        ...
        BinaryInputStream     is(cis);
        is >> o;
        return is.good();
    }
    virtual bool write(const std::string& filename, const T& o) const {
        CompressedOutputStream cos(filename);
        BinaryOutputStream     os(cos);
        os << o;
        return os.good();
    }
};
