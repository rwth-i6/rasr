...
NetworkParser::NetworkParser(Network& n, const Core::Configuration& c)
    ...
    XmlElement* parameter_decl =
            collect(new XmlEmptyElementRelay("param", this, startHandler(&Self::start_parameter)));
    XmlElement* input_decl =
            collect(new XmlEmptyElementRelay("in", this, startHandler(&Self::start_input)));
    XmlElement* output_decl =
            collect(new XmlEmptyElementRelay("out", this, startHandler(&Self::start_output)));
    XmlElement* node_decl =
            collect(new XmlEmptyElementRelay("node", this, startHandler(&Self::start_node)));
    XmlElement* link_decl =
            collect(new XmlEmptyElementRelay("link", this, startHandler(&Self::start_link)));
    XmlElement* networknode_decl =
            collect(new XmlMixedElementRelay("network-node", this,
    ...
    XmlElement* network_decl =
            collect(new XmlMixedElementRelay("network", this,
...
void NetworkParser::start_parameter(const XmlAttributes atts) {
    const char* name = atts["name"];
    if (!name)
        error("network parameter has no name");
    else
        nodeBuilder_.currentTemplate().declareParameter(name);
}
...
void NetworkParser::start_input(const XmlAttributes atts) {
    const char* name = atts["name"];
    if (!name)
        error("network input has no name");
    else
        nodeBuilder_.currentTemplate().addInput(name);
}
...
void NetworkParser::start_output(const XmlAttributes atts) {
    const char* name = atts["name"];
    if (!name)
        error("network output has no name");
    else
        nodeBuilder_.currentTemplate().addOutput(name);
}
...
void NetworkParser::start_link(const XmlAttributes atts) {
    ...
    if (pos == std::string::npos)
        ...
        from_port = tmp.substr(pos + 1, tmp.size());
    ...
    if (pos == std::string::npos)
        ...
        to_port = tmp.substr(pos + 1, tmp.size());
    ...
    if (!status)
        error("could not add link to network");
}
