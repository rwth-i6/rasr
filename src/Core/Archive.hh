...
public:
    ...
    public:
        ...
        Size uncompressed() const {
            return uncompressed_;
        }
        void setCompressed(u32 compressed) {
            compressed_ = compressed;
        }
        Size compressed() const {
            return compressed_;
        }
    };
    ...
    public:
        const_iterator(_const_iterator* iter)
                : iter_(iter) {}
        const_iterator(const const_iterator& i)
                : iter_(i.iter_) {}
        const_iterator()
                : iter_(Ref<_const_iterator>()) {}
...
public:
    ...
    bool hasAccess(AccessMode a) const {
        return (a & access_);
    }
     ...
     **/
    static Archive* create(const Configuration& config, const std::string& path = "", AccessMode access = AccessModeReadWrite);
};
