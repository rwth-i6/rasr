...
namespace Lattice {
    ...
    public:
        Mapping(const ShortPauses &shortPauses) : shortPauses_(shortPauses) {}
        virtual ~Mapping() {}
    ...
    private:
        class AccumulatorMap :
            public std::unordered_map<Fsa::LabelId, Fsa::Accumulator*>
        {
        public:
            ~AccumulatorMap() {
    ...
    private:
        ...
        f32 tolerance_;
    public:
        TimeframeErrorDfsState(
            ConstWordLatticeRef lattice, const Mapping &mapping) :
            DfsState(lattice),
    ...
    public:
        TimeframeErrorAutomaton(
            ConstWordLatticeRef, const ShortPauses &, bool useLemmata);
        virtual ~TimeframeErrorAutomaton() { delete mapping_; }

        std::string describe() const { return "timeframe-error"; }
        void modifyState(Fsa::State *) const;
    };

    TimeframeErrorAutomaton::TimeframeErrorAutomaton(
        ConstWordLatticeRef lattice,
        const ShortPauses &shortPauses,
        bool useLemmata)
        :
        ...
        bool useLemmata)
    {
        ...
        // remove epsilons before posterior calculation?
        Core::Ref<TimeframeErrorAutomaton> tfe(
            new MaximumTimeframeErrorAutomaton(
                posterior(total), shortPauses, useLemmata));
        Core::Ref<WordLattice> result(new WordLattice);
        result->setWordBoundaries(tfe->wordBoundaries());
    ...
    public:
        ...
        virtual ~WordTimeframeAccuracyAutomaton() { delete mapping_; }

        std::string describe() const { return "frame-word-accuracy"; }
        void modifyState(Fsa::State *) const;
    };

    WordTimeframeAccuracyAutomaton::WordTimeframeAccuracyAutomaton(
        ConstWordLatticeRef lattice,
