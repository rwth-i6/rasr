...
struct Extender : public Accumulator<_Weight> {
    _Value v;
    Extender(const _Weight& initial)
            : v(_Value(initial)) {}
    virtual ~Extender() {}
...
struct LogCollector : public Accumulator<_Weight> {
    ...
    _Value                      min;
    _ValueList                  values;
    LogCollector(const _Weight& initial)
            : min(_Value(initial)) {}
...
int LogSemiring<_Weight, _Type>::compare(const _Weight& a, const _Weight& b) const {
    if (_Type(a) == _Type(b))
        return 0;
    if (Core::isAlmostEqualUlp(_Type(a), _Type(b), tolerance_))
        return 0;
    if (_Type(a) < _Type(b))
        return -1;
    return 1;
...
struct TropicalCollector : public Accumulator<_Weight> {
    _Value v;
    TropicalCollector(const _Weight& initial)
            : v(_Value(initial)) {}
    virtual ~TropicalCollector() {}
    void feed(const _Weight& a) {
        v = std::min(v, _Value(a));
    }
    _Weight get() const {
        return _Weight(v);
    }
...
struct ProbabilityExtender : public Accumulator<_Weight> {
    _Value v;
    ProbabilityExtender(const _Weight& initial)
            : v(_Value(initial)) {}
...
struct ProbabilityCollector : public Accumulator<_Weight> {
    _Value v;
    ProbabilityCollector(const _Weight& initial)
            : v(_Value(initial)) {}
...
_Weight ProbabilitySemiring<_Weight, _Type>::invert(const _Weight& a) const {
    if (Core::isAlmostEqualUlp(_Type(a), _Type(0), Precursor::tolerance_))
        return Precursor::max();
