...
namespace Search
{
    ...
    u32 levenshteinDistance ( const std::vector<LabelId>& A, const std::vector<LabelId>& B )
    {
        ...
        for ( m=0; m<M; ++m ) { D[m] = new u32 [N]; D[m][0] = m; }//end for m
        for ( n=0; n<N; ++n ) { D[0][n] = n; }//end for n
    ...
    std::set<StateId> getContour ( std::set<StateId> oldContour, ConstAutomatonRef fsa )
    {
        ...
        for ( std::set<StateId>::const_iterator stateId = oldContour.begin ( ); stateId != oldContour.end ( ); ++stateId )
        {
            ...
            {
                if ( arc->target ( ) != *stateId ) { contour.insert ( arc->target ( ) ); }//end if
            }//end for arc
        }
        return contour;
    }//end getContour
    ...
    ConstAutomatonRef createLinearAutomatonFromVector
    ( const std::vector<LabelId>& sequence,
      const Weight&               score,
...
#if 0
    HypothesisVector* getNbestlistAsHypothesesVector ( ConstAutomatonRef nbestlist )
    {
        HypothesisVector* hypotheses = new HypothesisVector ( );

        Fsa::ConstStateRef initialState = nbestlist->getState ( nbestlist->initialStateId ( ) );
        for ( Fsa::State::const_iterator arc = initialState->begin ( );
              arc != initialState->end ( );
              ++arc )
        {
            Fsa::ConstStateRef state = nbestlist->getState ( arc->target ( ) );
            f32 probability = exp ( - f32 ( arc->weight ( ) ) );
            std::vector<Fsa::LabelId> sentence;
            while ( state->hasArcs ( ) )
            {
                sentence.push_back ( state->begin ( )->input ( ) );
                state = nbestlist->getState ( state->begin ( )->target ( ) );
            }//end while
            hypotheses->push_back ( Hypothesis ( sentence, probability ) );
        }//end for trueArc
        return hypotheses;
    }//end getNbestlistAsHypothesesVector
#endif
    ...
    public:
        ...
        virtual std::string describe () const { return "partialNbest(" + fsa_->describe() + ")"; }
    ...
    public:
        ...
        virtual std::string describe ( ) const { return "partialNbest(" + fsa_->describe() + ")"; }//end describe
    };
