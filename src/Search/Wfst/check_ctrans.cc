...
public:
  ...
  std::string getUsage() const { return "test c transducer\n"; }
  ...
  void createPhoneSequence(const Am::Allophone &a, OpenFst::VectorFst *seq) const
  {
      ...
      /*
      std::cout << (history == Bliss::Phoneme::term ? "#" : std::string(pi_->phoneme(history)->symbol())) << " "
                << pi_->phoneme(central)->symbol() << " "
                << (future == Bliss::Phoneme::term ? "#" : std::string(pi_->phoneme(future)->symbol())) << std::endl;
      */
