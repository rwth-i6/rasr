...
public:
    ...
    inline bool isDataAvailable() {
        if (is_fast_)
            return (!isEmpty(fast_data_) || (!queue_.isEmpty()));
        return (!queue_.isEmptyAtomar());
    }
    template<class T>
    inline bool getData(DataPtr<T>& d) {
        if (is_fast_) {
    ...
    bool areAttributesAvailable() const {
    ...
    inline void setFromNode(AbstractNode* n) {
        from_node_ = n;
    }
    inline void setFromPort(PortId id) {
        from_port_ = id;
    }
    inline void setToNode(AbstractNode* n) {
        to_node_ = n;
    }
    inline void setToPort(PortId id) {
        to_port_ = id;
    }
    inline void setBuffer(u32 size) {
        buffer_ = size;
    }
    inline u32 getBuffer() {
        return buffer_;
    }

    inline AbstractNode* getFromNode() const {
        return from_node_;
    }
    inline PortId getFromPort() const {
        return from_port_;
    }
    inline AbstractNode* getToNode() const {
        return to_node_;
    }
    inline PortId getToPort() const {
        return to_port_;
    }
