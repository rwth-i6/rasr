...
public:
    ...
    public:
        ...
        const T& operator*() const {
            return std::vector<Element>::const_iterator::operator*().data_;
        }
        const T* operator->() const {
            return &std::vector<Element>::const_iterator::operator*().data_;
        }
...
public:
    ...
    bool has(const T& d) const {
        return find(d) != InvalidCursor;
    }
    size_t size() const {
        return elements_.size();
    }
    const T& operator[](const Cursor p) const {
        return elements_[p].data_;
    }
    ...
    size_t getMemoryUsed() const {
        return bins_.getMemoryUsed() + sizeof(Element) * elements_.size();
    }
};
