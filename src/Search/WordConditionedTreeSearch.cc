...
#if 0
        /* The following functions are only useful for debugging purposes. */
public:
        /**
         * Checks if all state- and prospect members of the StateHypotheses in
         * the vector stateHypotheses are within reasonable bounds.
         */
        bool checkStateHypothesisScores() {
                int total = stateHypotheses.size();
                int nAbnormProspects = 0;
                int nAbnormScores	= 0;
                for (StateHypothesesList::const_iterator sh = stateHypotheses.begin();
                         sh != stateHypotheses.end(); ++sh) {
                        if ( (sh->score > 1.0e+20) || (sh->score < -1.0e+20) )
                                ++nAbnormScores;
                        if ( (sh->prospect > 1.0e+20) || (sh->prospect < -1.0e+20) )
                                ++nAbnormProspects;
                }
                if (nAbnormScores)
                        std::cout << "<info> abnormal scores: " << nAbnormScores << " / " << total
                                          << "</info>" << std::endl;
                else
                        std::cout << "all scores ok" << std::endl;

                if (nAbnormProspects)
                        std::cout << "<info> abnormal prospects: " << nAbnormProspects << " / " << total
                                          << "</info>" << std::endl;
                else
                        std::cout << "all prospects ok" << std::endl;

                return( (nAbnormProspects == 0) && (nAbnormScores == 0) );
        }
...
#if 0
                        //  fan-out pruning penalty is hard-coded
                        if (tree_->stateDepth(sh->state) < 0 && tree_->stateDepth(sh->state) >= -6)
                                sh->prospect += 140.0;
#endif
                }
