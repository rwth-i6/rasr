...
AllophoneStateFsaExporter::ExportedAutomaton AllophoneStateFsaExporter::exportFsaForOrthography(std::string const& orthography) const {
    ...
    graphModifier_.modify(graph, states, edges);
...
// HMM topology
Core::ParameterInt paramMinOccur(
        "label-min-occurance", 
        "speech only: minimum duration", 
        1, 1);
...
//       cross-word ratio is further applied to the speech forward transition
...
// mainly for label-dependent loop, e.g. constant shift of nClasses
Core::ParameterInt paramLoopIdShift(
        "loop-id-shift",
        "emission index shift for label loop",
        0);


FsaGraphModifier::FsaGraphModifier(const Core::Configuration& config, Core::Ref<Am::AcousticModel> am) :
        Precursor(config),
        am_(am) {

    silenceIndex_ = am_->emissionIndex(am_->silenceAllophoneStateIndex());
    ...
    loopShift_ = paramLoopIdShift(config);
    if (loopShift_ > 0) {
        verify(blankIndex_ == Core::Type<u32>::max); // HMM only
        log() << "shift emission index of label loop by " << loopShift_;
    }
}

void FsaGraphModifier::modify(Speech::AllophoneStateGraphRef graph, std::vector<Fsa::StateId>& states, std::vector<Edge>& edges) const {
        ...
        if (automaton->hasState(s)) {
            ...
            for (Fsa::State::const_iterator a = state->begin(); a != state->end(); ++a) {
                ...
                if (!labelLoop_)
                    ...
                    if (s == a->target_ && am_->emissionIndex(a->input_) == silenceIndex_)
                        silLoopStates.insert(s);
                }
                edges.push_back(Edge(s, a->target_, am_->emissionIndex(a->input_), Speech::Score(a->weight_)));
            }
            if (state->isFinal()) {
        ...
        if (!labelLoop_) {
            ...
            for (u32 idx = 0; idx < nEdges; ++idx) {
                ...
                if (e.from == e.to || e.emission_idx == blankIndex_)
                    continue; // skip loop and blank arcs
            ...
            // TODO transitionWeights_ ? also applicable
        }
    } 
    else { // HMM topology
        if (!transitionWeights_.empty())
            modifyTransitionWeights(edges, silLoopStates);
        // shift emission index of loop transitions (no interference with further minDur handling)
        if (loopShift_ > 0) {
            for (u32 idx = 0; idx < edges.size(); ++idx) {
                Edge& e = edges[idx];
                if (e.from == e.to)
                    e.emission_idx += loopShift_;
            }
        }
    }

    if (minOccur_ > 1)
        modifyMinDuration(edges, states, automaton); 
...
void FsaGraphModifier::modifyTransitionWeights(std::vector<Edge>& edges, const std::unordered_set<Fsa::StateId>& silLoopStates) const {
    ...
    for (u32 idx = 0; idx < nEdges; ++idx) {
        ...
        else { // speech forward + optional forward ratio to speech/silence
...
// Note: no additional check w.r.t. audio length (automaton-wise always correct)
void FsaGraphModifier::modifyMinDuration(std::vector<Edge>& edges, std::vector<Fsa::StateId>& states, Core::Ref<const Fsa::StaticAutomaton> automaton) const {
    verify(silenceIndex_ != Core::Type<u32>::max || blankIndex_ != Core::Type<u32>::max);
    Fsa::StateId maxStateId = std::max(automaton->maxStateId(), states.back());
    u32 nEdges = edges.size();
    for (u32 idx = 0; idx < nEdges; ++idx) {
        // expand each speech forward transition to minOccur_
        Edge e = edges[idx];
        if (e.from == e.to || e.emission_idx == silenceIndex_ || e.emission_idx == blankIndex_)
            continue;
        Fsa::StateId target = e.to;
        for (u32 rp = 1; rp < minOccur_; ++rp) {
            states.push_back(++maxStateId);
            edges.emplace_back(maxStateId, target, e.emission_idx, 0); // no weight here
            target = maxStateId;
        }
