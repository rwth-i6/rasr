...
namespace Search
{
     ...
     *
     */
    inline void computeLevenshteinColumn(
...
#if 0
            levenshteinColumns_.erase(levenshteinColumns_.begin() + oldNodePosition);
            levenshteinScores_.erase(levenshteinScores_.begin() + oldNodePosition);
#endif
    ...
    void MinimumBayesRiskAStarSearch::SearchNode::addState(
        const Fsa::StateId stateId,
        ...
        const Fsa::Weight inverseNormalizationConstant)
    {
        ...
        logTotalProbabilities_.push_back(
            Fsa::LogSemiring->extend(inverseNormalizationConstant,
                                     Fsa::LogSemiring->extend(
                                         logForwardProbability,
    ...
    void MinimumBayesRiskAStarSearch::SearchNode::dump(
        Core::XmlWriter& out, Fsa::ConstAlphabetRef alphabet) const
    {}
    ...
    void MinimumBayesRiskAStarSearch::dump(Core::XmlWriter &out, Fsa::ConstAlphabetRef alphabet) const
    {
        ...
        if (secondMinimalNode_) std::cerr << " | 2nd-Minimal: " << secondMinimalNode_->index_;
        if (minimalIncompleteNode_) std::cerr << " | minimal incomplete: " << minimalIncompleteNode_->index_;
        std::cerr << " |" << std::endl;
        std::cerr << std::endl;
...
#if 1
            std::cerr << "georg3d" << std::endl;
#endif

            overestimateFinal(expansionNode);
#if 1
            std::cerr << "georg3e" << std::endl;
#endif
        } else {
#if 1
            std::cerr << "georg3f" << std::endl;
#endif
    ...
    std::vector<SearchNodeRef> MinimumBayesRiskAStarSearch::createNewSearchNodes (
        ConstSearchNodeRef expansionNode)
    {
        ...
        for (; stateId != expansionNode->end(); ++ stateId) {
            ...
            for (; arc != state->end (); ++ arc) {
                ...
                if (mapEntry == newNodes.end()) {
                    mapEntry = newNodes.insert(
                        mapEntry,
                        std::make_pair(
                            arc->input(),
                            SearchNodeRef(
                                new SearchNode(
                                    expansionNode->hypothesis_,
                                    arc->input(),
        ...
        SearchNodeRef expansionNode, std::vector<SearchNodeRef>& newNodes)
    {
        ...
        for (u32 n = 0; n < stack_->size(); ++ n) {
            ...
            for (; newSearchNode != newNodes.end(); ++ newSearchNode) {
...
#if 1
                std::cerr << "levenshtein-column-size: " << levenshteinColumn.size() << std::endl;
                std::cerr << "competing-levenshtein-column-size: " << competingLevenshteinColumn.size() << std::endl;
#endif
                ...
                if (competingSearchNode->isFinal()) {
                    if (!exactEstimate_) {
                        ...
                        Fsa::Weight levenshteinScoreOverestimate =
                            Fsa::LogSemiring->extend(
                                LogWeights.at(newLevenshteinColumn.back()),
                                (*newSearchNode)->totalProbability_);
                        competingSearchNode->levenshteinScoresOverestimate_.push_back(levenshteinScoreOverestimate);
                    }
                }
#if 0
                if (competingSearchNode->isExplorable_) {
                    if (!minimalIncompleteNode_ || competingSearchNode < minimalIncompleteNode_) {
                        minimalIncompleteNode_ = competingSearchNode;
                    }
                }
#endif

                Fsa::Weight levenshteinScore =
                    Fsa::LogSemiring->extend(
                        LogWeights.at(minimum(levenshteinColumn)),
                        (*newSearchNode)->totalProbability_);
                ...
                (*newSearchNode)->levenshteinScores_.at(n) =
                    Fsa::LogSemiring->extend(
                        Fsa::Weight(LogWeights.at(minimum((*newSearchNode)->levenshteinColumns_.at(n)))),
            ...
            if (competingSearchNode->isFinal()) {
#if 0
                if (!expansionNode->isFinal_) {
                    competingSearchNode->levenshteinScoresOverestimate_.erase(
                        competingSearchNode->levenshteinScoresOverestimate_.begin() + expansionNode->index_);
                }
#endif
                Fsa::Accumulator* collector = Fsa::LogSemiring->getCollector(Fsa::LogSemiring->zero());
    ...
    void MinimumBayesRiskAStarSearch::generateNewGridNodes(
        SearchNodeRef expansionNode, std::vector<SearchNodeRef>& newNodes)
    {
    ...
    void MinimumBayesRiskAStarSearch::performSearch(Fsa::ConstAutomatonRef fsa)
    {
...
#if 0
        fsa_ = Fsa::changeSemiring(fsa_, Fsa::TropicalSemiring);
        Fsa::ConstAutomatonRef bestAutomaton = Fsa::removeEpsilons(Fsa::nbest(fsa_, 1));
        require(bestAutomaton);
        Fsa::Weight bestScore = Fsa::getLinearWeight(bestAutomaton);
        std::vector<Fsa::LabelId> bestSentence;
        Fsa::getLinearInput(bestAutomaton, bestSentence);

        if (exp(-f32(Fsa::LogSemiring->extend(bestScore, Fsa::LogSemiring->invert(backwardPotentials_.at(fsa_->initialStateId()))))) > 0.5) {
            bestAutomaton_ = bestAutomaton;
            return;
        }
        Fsa::ConstAutomatonRef levenshteinAutomaton = Fsa::levenshtein(bestAutomaton, fsa_);
        Fsa::ConstAutomatonRef m1 = Fsa::composeMatching(Fsa::multiply(levenshteinAutomaton, Fsa::Weight(0.0)), fsa_);
        Fsa::ConstAutomatonRef m2 = Fsa::composeMatching(levenshteinAutomaton, Fsa::multiply(fsa_, Fsa::Weight(0.0)));

        m1 = Fsa::changeSemiring(m1, Fsa::LogSemiring);
        f32 risk = Fsa::expectation(m1, m2);
        ...
        //	Fsa::write(fsa_, "bin:hypotheses.binfsa.gz");
...
#if 1
        std::cerr << "hello8a" << std::endl;
#endif
            expand();
#if 1
        std::cerr << "hello8b" << std::endl;
#endif
        }
