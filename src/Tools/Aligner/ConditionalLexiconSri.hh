...
namespace Translation {
        ...
        ConditionalLexiconSri(const Core::Configuration& config, Fsa::ConstAlphabetRef alphabet) :
            ...
            {}
                  ...
                  virtual Translation::Cost getCost(const size_t index, const std::vector<Fsa::LabelId>& key) const {
                                //std::cerr << "getCost (" << index << ",[" << Core::vector2String(key) << "])" << std::endl;
                                ...
                                if (key.size()>2) {
                                         ...
                                         //std::cerr << "sriKey = " << Core::vector2String(sriKey) << std::endl;
    ...
    private:
                  VocabIndex mapToTupleIndexString_(Fsa::LabelId s, Fsa::LabelId t) const {
                                ...
                                //std::cerr << "converting symbols '" << tokens_->symbol(s)
                                //			 << "' and '" << tokens_->symbol(t)
                                //			 << "' to '" << tupleSymbol
                                //			 << "' which has index " << sriTupleVocabulary_.getIndex(VocabString(tupleSymbol.c_str()));
                  ...
                  VocabIndex mapToTupleIndex_(Fsa::LabelId s, Fsa::LabelId t) const {
                                ...
                                if ( i != vocabMap.end() ) {
                                         //	 std::cerr << " mapIndex=" << i->second << std::endl;
                                         return i->second;
                                } else {
