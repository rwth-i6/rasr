...
// ===========================================================================
class FileArchive::_const_iterator : public Archive::_const_iterator {
private:
        ...
        *stream_ >> checksum;
        if (name.empty()) {
            ...
            *stream_ >> tmp;
            ...
            // regular file
            if (compressed)
                stream_->BinaryInputStream::seek(compressed, std::ios::cur);
            else
                stream_->BinaryInputStream::seek(size, std::ios::cur);
            tmp = 0;
            *stream_ >> tmp;
...
bool FileArchive::test(const std::string& path) {
    ...
    if (stat64(path.c_str(), &fs) < 0) {
        ...
        if ((stream) && (memcmp(header, _header, 8) == 0))
            return true;
    }
    return false;
}
