...
AdaptationTree::AdaptationTree(const Core::Configuration&     c,
    ...
    switch (ClassicAcousticModel::paramType(config, ClassicAcousticModel::cartTying)) {
...
void AdaptationTree::loadDecisionTree(const Core::Configuration&     config,
                                      const Am::ClassicStateModelRef stateModel,
                                      Bliss::Phoneme::Id             silencePhoneme) {
    ...
    Core::BinaryTree::TreeStructure structure;
    {
        ...
        verify((root.leftChild().info<Cart::TrainingInformation>().order == 1) && (root.rightChild().info<Cart::TrainingInformation>().order == 2));
        std::stack<const Cart::BinaryTree::Node*> S;
        S.push(&root.rightChild());
        while (!S.empty()) {
            const Cart::BinaryTree::Node* node = S.top();
            S.pop();
            u32 order = node->info<Cart::TrainingInformation>().order - 2;
            if (node->isLeaf()) {
                Mm::MixtureIndex leafNumber = node->id_;
                if (leafNumber > siMixtureId)
                    --leafNumber;
