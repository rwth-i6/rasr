...
SegmentwiseAlignmentGenerator::SegmentwiseAlignmentGenerator(const Core::Configuration& c, Core::Ref<const ModelCombination> modelCombination)
        : Core::Component(c),
...
void SegmentwiseAlignmentGenerator::setSegmentwiseFeatureExtractor(Core::Ref<SegmentwiseFeatureExtractor> segmentwiseFeatureExtractor) {
...
void SegmentwiseAlignmentGenerator::update(const std::string& segmentId) {
    ...
    if (featureScorer->hasTimeIndexedCache() && featureScorer->isBuffered()) {
        // We need to provide all features now.
        featureScorer->reset();
        size_t t = 0;
        for (SegmentwiseFeatures::const_iterator f = features()->begin(), featureEnd = features()->end(); f != featureEnd; ++f, ++t) {
            if (featureScorer->bufferFilled()) {
                criticalError("Buffer filled after %zu/%zu frames.", t, features()->size());
                break;
            }
            featureScorer->addFeature(*f);
        }
    }
}
...
void SegmentwiseAlignmentGenerator::getScorers(std::vector<Mm::FeatureScorer::Scorer>& scorers) const {
...
void SegmentwiseAlignmentGenerator::getScorers(TimeframeIndex tbeg, TimeframeIndex tend, std::vector<Mm::FeatureScorer::Scorer>& scorers) const {
...
namespace {
const Core::ParameterString paramPath("path", "path", "");
}  // namespace
...
Core::Archive* OrthographyAlignmentGenerator::createAlignmentCache(const Core::Configuration& config) {
    Core::Archive*      alignmentCache = Core::Archive::create(config, paramPath(config), Core::Archive::AccessModeReadWrite);
    ...
    if (ar.isOpen()) {
        ...
        if (!archiveDependencies.read(config, ar)) {
            error("Failed to read dependencies from alignment cache \"%s\".", alignmentCache->path().c_str());
        }
        else if (archiveDependencies == dependencies()) {
            warning("Archive \"%s\" exists already and will be overwritten.", alignmentCache->path().c_str());
        }
    }
    else {
...
Fsa::ConstAutomatonRef OrthographyAlignmentGenerator::getAlignmentFsa() {
    ...
    if (!isValid_) {
        ...
        Fsa::ConstAutomatonRef transducer = transducerCache_->get(allophoneStateGraphBuilder_->createTransducerFunctor(segmentId_, orthography_));
        wordLatticeBuilder_->setModelTransducer(transducer);
        // ???
        Fsa::ConstAutomatonRef modelAcceptor = modelAcceptorCache_->get(allophoneStateGraphBuilder_->createFinalizationFunctor(segmentId_, transducer));
...
const Alignment* OrthographyAlignmentGenerator::getAlignment(const std::string& orthography) {
    ...
    if (alignmentCache_) {
        Core::ArchiveReader reader(*alignmentCache_, segmentId_);
        if (reader.isOpen()) {
            Alignment*              alignment = new Alignment;
            Core::BinaryInputStream bi(reader);
            bi >> *alignment;
            if (!bi)
                error("Failed to read alignment \"%s\" from cache \"%s\".", segmentId_.c_str(), alignmentCache_->path().c_str());
            return alignment;
        }
    }
    ...
    if (alignmentCache_) {
        Core::ArchiveWriter writer(*alignmentCache_, segmentId_);
        if (writer.isOpen()) {
            Core::BinaryOutputStream bo(writer);
            bo << *alignment;
            if (!bo)
                error("Failed to write alignment \"%s\" to cache \"%s\".", segmentId_.c_str(), alignmentCache_->path().c_str());
        }
        else {
            error("Could not open file \"%s\" for writing in cache \"%s\".", segmentId_.c_str(), alignmentCache_->path().c_str());
        }
    }
    return alignment;
}
