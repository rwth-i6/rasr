...
/**** Added byt JGF  ****/
#if defined(OS_SunOS) || defined(OS_HPUX)	/* cth */
#include <sys/types.h>
...
extern int read_shortpacked_data(short int *, int, FOB *);
extern int write_shortpacked_data(short int *, int, FOB *);

int compute_checksum(short int *wav, int len);

/*uses htons to check byte ordering of machine*/

/*if check_checksum = 1 the checksum will be checked if = 0 it won't*/
int read_wav_data(FOB *fp, short int **pwaveform, header_t *header, int check_checksum)
{ int i;
...
void write_wav_data(FOB *fp, short int *waveform, header_t *header)
{ int i;
  char sample_byte_format[MAX_HEADER_FIELD];
  int swap; 
  int string_len;
...
void update_checksum_unless_its_already_there(short int *waveform, header_t *header)
{ int field_len;
  int checksum;
  int wav_len;
...
void update_checksum_even_if_its_already_there(short int *waveform, header_t *header)
{ int field_len;
  int checksum;
  int wav_len;
