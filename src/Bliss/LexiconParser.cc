...
void LexiconElement::addOrth(const std::string& _orth) {
    ...
    if (!lemma_) {
        ...
        if (!specialLemmaName_.empty()) {
            ...
            // that each piece keeps its own orthography. These categories are
            // intentionally multi-valued; all other special names remain unique
            // as before. The first pair describes a continuation-marked subword
            // inventory, the second a word-start-marked one.
            bool multiValued = specialLemmaName_ == "unknown-continuation" || specialLemmaName_ == "unknown-final" ||
                               specialLemmaName_ == "unknown-word-start" || specialLemmaName_ == "unknown-word-internal";
