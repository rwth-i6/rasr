...
InputStream::InputStream(const std::string& path, Core::Archive* archive)
    ...
    if (archive) {
        ...
        if (!(*ar) || !ar->isOpen()) {
            Core::Application::us()->warning("Could not open archive entry \"%s\".", filename.c_str());
            delete ar;
            ar = 0;
        }
        else
        ...
        if (!(*cis)) {
            Core::Application::us()->warning("Could not open file \"%s\".", filename.c_str());
            delete cis;
            cis = 0;
        }
        else
...
OutputStream::OutputStream(const std::string& path, Core::Archive* archive)
    ...
    if (archive) {
        ...
        if (!(*aw)) {
            Core::Application::us()->error("Could not open archive entry \"%s\"", filename.c_str());
            delete aw;
            aw = 0;
        }
        else
        ...
        if (!(*cos)) {
            Core::Application::us()->error("Could not open file \"%s\"", filename.c_str());
            delete cos;
            cos = 0;
        }
        else
    ...
    switch (version) {
        ...
        case 4: {
            for (StaticBoundaries::iterator it = b->begin(); (it != b->end()) && (bis); ++it) {
                ...
                u16 final, initial;
                u8  boundary;
                if (!(bis >> time >> final >> initial >> boundary))
                    return false;
                it->setTime(time);
...
private:
    ...
    typedef std::unordered_map<std::string, CtmRecording, Core::StringHash> CtmCorpus;
...
public:
    ...
    virtual void init(const std::vector<std::string>& arguments) {
        ...
        if (!semiring_) {
            ...
            for (ScoreId id = 0; id < keys.size(); ++id)
                ids_.push_back(id);
        }
        else {
