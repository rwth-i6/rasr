...
{
public:
    BufferedAlignedFeatureProcessor *processor_;
    u32 nClasses_;
...
void TestBufferedAlignedFeatureProcessor::setUp(){
    ...
    processor_ = new BufferedAlignedFeatureProcessor(config);
    processor_->acousticModelNeedInit_ = false;
    processor_->classLabelWrapper_ = new Nn::ClassLabelWrapper(config, nClasses_);
...
{
    processor_->processAlignedFeature(feature1_, 3);
    EXPECT_EQ(1u, processor_->nBufferedFeatures_);
    EXPECT_EQ(0u, processor_->segmentIndexBuffer_.at(0));
    processor_->processAlignedFeature(feature2_, 1);
    EXPECT_EQ(2u, processor_->nBufferedFeatures_);
    EXPECT_EQ(0u, processor_->segmentIndexBuffer_.at(1));
    processor_->processAlignedFeature(feature3_, 9);
    EXPECT_EQ(3u, processor_->nBufferedFeatures_);
    EXPECT_EQ(0u, processor_->segmentIndexBuffer_.at(2));
    EXPECT_EQ(processor_->classLabelWrapper_->getOutputIndexFromClassIndex(3), processor_->alignmentBuffer_.at(0));
    EXPECT_EQ(processor_->classLabelWrapper_->getOutputIndexFromClassIndex(1), processor_->alignmentBuffer_.at(1));
    EXPECT_EQ(processor_->classLabelWrapper_->getOutputIndexFromClassIndex(9), processor_->alignmentBuffer_.at(2));

    f32 val = 1.0f;
    for (u32 i = 0; i < 3; i++){
        for (u32 j = 0; j < 2; j++ ){
            EXPECT_EQ(val, processor_->featureBuffer_.at(0).at(j,i));
            val = val + 1.0f;
        }
    }
}
