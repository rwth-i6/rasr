 ...
 **/
class StateTree : public virtual Core::Component {
private:
...
public:
    ...
    struct Exit {
        ...
        bool operator<(const Exit& rhs) const {
            return (pronunciation < rhs.pronunciation) || (pronunciation == rhs.pronunciation && transitEntry < rhs.transitEntry);
        }

        bool operator==(const Exit& rhs) const {
            return pronunciation == rhs.pronunciation && transitEntry == rhs.transitEntry;
        }
    };
...
public:
    ...
    struct StateDesc {
        ...
        bool operator<(const StateDesc& rhs) const {
            return (acousticModel < rhs.acousticModel) || (acousticModel == rhs.acousticModel && transitionModelIndex < rhs.transitionModelIndex);
        }

        bool operator==(const StateDesc& rhs) const {
            return (acousticModel == rhs.acousticModel) && (transitionModelIndex == rhs.transitionModelIndex);
        }
...
private:
    ...
    struct BuildRequest;
    struct BatchRequest;
    friend struct StateTree::BatchRequest;
    struct LeapRequest;
    friend struct StateTree::LeapRequest;
    class RequestQueue;
    class StraightRequestQueue;
    friend class StateTree::StraightRequestQueue;
    class RecombiningRequestQueue;
    friend class StateTree::RecombiningRequestQueue;
...
public:
    ...
    StateId nStates() const {
        return states_.size();
    }
    size_t memSizeStates() const {
        return states_.size() * sizeof(State);
    }
    StateId nWordHeadStates() const {
        return wordHeadsEnd_;
    }
    StateId root() const {
        return root_;
    }
    StateId ciRoot() const {
        return ciRoot_;
    }
