                ...
                if (lexiconFilename_ != "") {
                  ...
                  virtual Translation::Cost getProb(const size_t index, const std::vector<std::string>& key) const {
                                ...
                                for (size_t i=0; i<key.size(); ++i) {
                                         ...
                                         //std::cerr << "mapping " << key[i] << " to " << mappedKey.back() << std::endl;
                                }
                                //reverse(mappedKey.begin(),mappedKey.end());
                                //std::cerr << "getProb " << getProb(index, mappedKey) << std::endl;
...
//			        std::cerr << "KEY : " << std::endl;
//				for (uint i=0; i < reverseKey.size(); i++) {
//					std::cerr << tokens_->symbol(reverseKey[i])<< " ";
//				}
//				std::cerr << std::endl;
...
//			 * existence has to be memorize in order to decide whether to store or
//			 * to increase the current key.
//			 */
//
//			if (lexica_.size()<index+1) {
//            		std::cerr << "resizing lexicon to " << index+1 << std::endl;
//
...
//			std::vector<Fsa::LabelId>::const_iterator i = key.begin();
//			while (i != key.end()) {
//				Lexicon::Node* newNode;
//
//				if (exists) {
//					newNode = currentNode->follow(*i);
//					if (newNode == 0) {
//						exists=false;
...
//			}
//			if (exists) {
//				//! \todo make addToValue method in PrefixTree
                        ...
                        for (uint index=0; index<lexica_.size(); ++index) {
                                for (Lexicon::iterator it = lexica_[index]->begin(); it != lexica_[index]->end(); ++it) {
                                        std::vector<Fsa::LabelId> path = it.getIndexPath();
                                                if (it->isLeaf()) {
                                                out << index << " " << "prob: " << exp(-(it->getData())) << " cost: " << it->getData() << " ";
