...
namespace Signal {
    ...
    public:
        typedef f32 Float;
        enum SilenceType { silence, speech, unsure };
    ...
    protected:
        ...
        public:
            Frame() : energy_(0), silence_(unsure) {}
            ...
            SilenceType& silence() { return silence_; }
        };
    ...
    private:
        ...
        Float energyScalingFactor() { return (Float)energyHistogram_.size() / 90.0; }

    public:
        SilenceDetection() : energyHistogram_(270), need_init_(true) {}
        ~SilenceDetection() {}

        void setHistogramBufferSize(const u32 size) {
            if (histogramSlidingWindowSize_ != size) {
                histogramSlidingWindowSize_ = size; need_init_ = true;
            }
        }
        void setHistogramBufferDelay(const u32 delay) {
            if (histogramSlidingWindowRight_ != delay) {
                histogramSlidingWindowRight_ = delay; need_init_ = true;
            }
        }

        void setBlockSize(const u32 size) { if (blockSize_ != size) { blockSize_ = size; need_init_ = true; } }

        void setSparseEventRatio(const Float sparseEventRatio) {
            if (sparseEventRatio_ != sparseEventRatio) { sparseEventRatio_ = sparseEventRatio; need_init_ = true; }
        }
        void setThresholdInterpolationFactor(const Float thresholdInterpolationFactor) {
            if (thresholdInterpolationFactor_ != thresholdInterpolationFactor) {
                thresholdInterpolationFactor_ = thresholdInterpolationFactor; need_init_ = true;
            }
        }
        void setMinSnr(const Float minSnr) {
            if (minSnr_ != (minSnr * energyScalingFactor())) {
                minSnr_ = minSnr * energyScalingFactor(); need_init_ = true;
            }
        }

        void setMinSpeechLength(const u32 minSpeechLength) {
            if (minSpeechLength_ != minSpeechLength) { minSpeechLength_ = minSpeechLength; need_init_ = true; }
        }
        void setMinSilenceLength(const u32 minSilenceLength) {
            if (minSilenceLength_ != minSilenceLength) { minSilenceLength_ = minSilenceLength; need_init_ = true; }
        }
        void setEndDelay(const u32 endDelay) {
            if (endDelay_ != endDelay) { endDelay_ = endDelay; need_init_ = true; }
        }

        void setBeginDelay(const u32 delay) {
            if (beginDelay_ != delay) { beginDelay_ = delay; need_init_ = true; }
        }

        bool update(const Flow::DataPtr<Flow::Vector<Float> > &in, Frame &out);
        bool flush(Frame &out);
        void reset() { need_init_ = true; }
    };
    ...
    private:
        ...
        bool send(Frame &out) {
            if (!out) return false;
            if (out.silence() == unsure) criticalError("Unsure frame");
    ...
    public:
        static std::string filterName() { return "signal-silence-detection"; }
        ...
        virtual bool setParameter(const std::string &name, const std::string &value) {
            if (paramHistogramBufferSize.match(name)) setHistogramBufferSize(paramHistogramBufferSize(value));
            else if (paramHistogramBufferDelay.match(name)) setHistogramBufferDelay(paramHistogramBufferDelay(value));
            else if (paramBlockSize.match(name)) setBlockSize(paramBlockSize(value));
            else if (paramSparseEventRatio.match(name)) setSparseEventRatio(paramSparseEventRatio(value));
            ...
            else if (paramMinSnr.match(name)) setMinSnr(paramMinSnr(value));
            else if (paramMinSpeechLength.match(name)) setMinSpeechLength(paramMinSpeechLength(value));
            else if (paramMinSilenceLength.match(name)) setMinSilenceLength(paramMinSilenceLength(value));
            else if (paramEndDelay.match(name)) setEndDelay(paramEndDelay(value));
            else if (paramBeginDelay.match(name)) setBeginDelay(paramBeginDelay(value));
            else return false;
            return true;
        }
        ...
        virtual void reset() { SilenceDetection::reset(); }
    };
}
