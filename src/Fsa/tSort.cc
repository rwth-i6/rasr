...
public:
    SortByArcAutomaton(_ConstAutomatonRef f)
            : Precursor(f) {
...
public:
    SortByInputAutomaton(_ConstAutomatonRef f)
            : Precursor(f) {
        this->setProperties(Fsa::PropertySorted, Fsa::PropertySortedByInput);
    }
    virtual void modifyState(_State* sp) const {
        sp->sort(byInput<_Automaton>());
    }
    virtual std::string describe() const {
...
public:
    SortByInputAndOutputAutomaton(_ConstAutomatonRef f)
            : Precursor(f) {
        this->setProperties(Fsa::PropertySorted, Fsa::PropertySortedByInputAndOutput);
    }
    virtual void modifyState(_State* sp) const {
        sp->sort(byInputAndOutput<_Automaton>());
    }
    virtual std::string describe() const {
...
public:
    SortByInputAndTargetAutomaton(_ConstAutomatonRef f)
            : Precursor(f) {
        this->setProperties(Fsa::PropertySorted, Fsa::PropertySortedByInput);
    }
    virtual void modifyState(_State* sp) const {
        sp->sort(byInputAndTarget<_Automaton>());
    }
    virtual std::string describe() const {
...
public:
    SortByInputAndOutputAndTargetAutomaton(_ConstAutomatonRef f)
            : Precursor(f) {
        this->setProperties(Fsa::PropertySorted, Fsa::PropertySortedByInputAndOutputAndTarget);
    }
    virtual void modifyState(_State* sp) const {
        sp->sort(byInputAndOutputAndTarget<_Automaton>());
    }
    virtual std::string describe() const {
...
public:
    SortByOutputAutomaton(_ConstAutomatonRef f)
            : Precursor(f) {
        this->setProperties(Fsa::PropertySorted, Fsa::PropertySortedByOutput);
    }
    virtual void modifyState(_State* sp) const {
        sp->sort(byOutput<_Automaton>());
    }
    virtual std::string describe() const {
...
public:
    SortByWeightAutomaton(_ConstAutomatonRef f)
            : Precursor(f) {
        this->setProperties(Fsa::PropertySorted, Fsa::PropertySortedByWeight);
    }
    virtual void modifyState(_State* sp) const {
        sp->sort(byWeight<_Automaton>(this->semiring()));
    }
    virtual std::string describe() const {
    ...
    switch (type) {
        ...
        case Fsa::SortTypeByArc:
            if (f->hasProperty(Fsa::PropertySortedByArc))
                ...
                return typename _Automaton::ConstRef(new SortByArcAutomaton<_Automaton>(f));
        case Fsa::SortTypeByInput:
            if (f->hasProperty(Fsa::PropertySortedByArc) ||
                ...
                return typename _Automaton::ConstRef(new SortByInputAutomaton<_Automaton>(f));
        case Fsa::SortTypeByInputAndOutput:
            if (f->hasProperty(Fsa::PropertySortedByArc) ||
                ...
                return typename _Automaton::ConstRef(new SortByInputAndOutputAutomaton<_Automaton>(f));
        case Fsa::SortTypeByInputAndTarget:
            if (f->hasProperty(Fsa::PropertySortedByInputAndTarget))
                return f;
            else
                return typename _Automaton::ConstRef(new SortByInputAndTargetAutomaton<_Automaton>(f));
        case Fsa::SortTypeByInputAndOutputAndTarget:
            if (f->hasProperty(Fsa::PropertySortedByInputAndOutputAndTarget))
                return f;
            else
                return typename _Automaton::ConstRef(new SortByInputAndOutputAndTargetAutomaton<_Automaton>(f));
        case Fsa::SortTypeByOutput:
            if (f->hasProperty(Fsa::PropertySortedByOutput))
                return f;
            else
                return typename _Automaton::ConstRef(new SortByOutputAutomaton<_Automaton>(f));
        case Fsa::SortTypeByWeight:
            if (f->hasProperty(Fsa::PropertySortedByWeight))
                return f;
            else
                return typename _Automaton::ConstRef(new SortByWeightAutomaton<_Automaton>(f));
        default:
            defect();
    }
    return f;
...
public:
    ...
    virtual void exploreNonTreeArc(_ConstStateRef from, const _Arc& a) {
        if (Precursor::color(a.target()) == Precursor::Gray)
            isCyclic_ = true;
    }
};
...
Fsa::StateMap topologicallySort(typename _Automaton::ConstRef f, bool progress) {
    ...
    --v.time_;
    if (v.isCyclic_)
        v.map_.resize(0);  // return empty map in case of cyclic automaton
    // states in decreasing finishing times of dfs define topological order
    for (Fsa::StateId i = 0; i < v.map_.size(); ++i)
        if (v.map_[i] != Fsa::InvalidStateId)
            v.map_[i] = v.time_ - v.map_[i];
    return v.map_;
}
