...
Score AcousticLookAhead::calculateDistance( const AcousticLookAhead::AcousticFeatureVector& mean1, const AcousticLookAhead::AcousticFeatureVector& mean2 ) {
  ...
  switch ( ( dimension - 1 ) % 8 ) {
    while ( cmp < dimension ) {
      case 7: df = ( mean1[cmp] - mean2[cmp] ); score += df * df; ++cmp;
      case 6: df = ( mean1[cmp] - mean2[cmp] ); score += df * df; ++cmp;
      case 5: df = ( mean1[cmp] - mean2[cmp] ); score += df * df; ++cmp;
      case 4: df = ( mean1[cmp] - mean2[cmp] ); score += df * df; ++cmp;
      case 3: df = ( mean1[cmp] - mean2[cmp] ); score += df * df; ++cmp;
      case 2: df = ( mean1[cmp] - mean2[cmp] ); score += df * df; ++cmp;
      case 1: df = ( mean1[cmp] - mean2[cmp] ); score += df * df; ++cmp;
      case 0: df = ( mean1[cmp] - mean2[cmp] ); score += df * df; ++cmp;
    }
  }
...
Score AcousticLookAhead::calculateDistance( const Mm::SimdGaussDiagonalMaximumFeatureScorer::QuantizedType* mean1, const Mm::SimdGaussDiagonalMaximumFeatureScorer::QuantizedType* mean2, u32 dimension ) {
  int df, score = 0;
  u32 cmp = 0;
  switch ( ( dimension - 1 ) % 8 ) {
    while ( cmp < dimension ) {
...
void AcousticLookAhead::AcousticLookAheadModel::estimate( const std::vector<u32>& assigned, const std::vector<AcousticFeatureVector>& means, u32 splits ) {
  if( assigned.empty() )
    return;
  ...
  for( int i = 0; i < iterations_; ++i )
  {
    ...
    if( ( splittingThreshold_ != 0.0 || splitEmpty_ ) && i + 1 != iterations_ )
    {
      ...
      for( u32 i = 0; i < splitUntil; ++i )
      {
        ...
        ++splitted;
//         sorted[ sorted.size() - 1 - i ].first;
//         std::cout << "eliminating state " << eliminate << " with " << sorted[i].second << ", splitting " << split << " with " << sorted[ sorted.size() - 1 - i ].second << std::endl;
...
void AcousticLookAhead::getSuccessorMixtures( const PersistentStateTree& network, StateId state, std::set<Am::AcousticModel::EmissionIndex>& target, int depth, bool includeCurrent ) {
...
void AcousticLookAhead::setLookAhead( std::vector<Mm::FeatureVector> lookahead ) {
  ...
  {
    acousticLookAhead_.push_back( std::make_pair( acousticLookAheadScorer_->multiplyAndQuantize( lookahead.at( a ) )[0], acousticLookAheadScorer_->getScorer( lookahead.at( a ) ) ) );
...
struct Stats {
  ...
  Stats() : cached( 0 ), computed( 0 ), cacheSizeBefore( 0 ), cacheSize( 0 ), cacheSizeSamples( 0 ) {
  }
  ~Stats() {
    if( cached || computed )
    {
...
void AcousticLookAhead::fillCacheForTimeframe( int timeframe ) {
  ...
  if( cache.simpleCacheTimeframe_ != timeframe )
  {
    ...
    for( int lookaheadId = 0; lookaheadId < acousticLookAheadModels_.size(); ++lookaheadId )
      cache.simpleScoreCache_[lookaheadId] = calculateDistance( means_.data() + ( dimension * lookaheadId ), acousticLookAhead_[timeframe - currentTimeFrame_].first.data(), dimension );
  }
}
...
void AcousticLookAhead::computeAllLookAheadScoresInternal() {
  ...
  if( useAverage )
  {
    ...
    if( len == 1 )
    {
      ...
      for( int lookaheadId = 0; lookaheadId < acousticLookAheadModels_.size(); ++lookaheadId )
        preCachedLookAheadScores_[lookaheadId] = calculateDistance( means_.data() + ( dimension * lookaheadId ), acousticLookAhead_[0].first.data(), dimension ) * divFac;
      return;
    }
